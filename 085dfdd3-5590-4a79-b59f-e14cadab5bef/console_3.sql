select
blaze7.dbo.Guid_To_ExecId(tr.reportid),
tr.Status,tor.SystemID,tor.SystemOrderTypeID,
tor.ID,
tr.TransactionDateTime,
	dateadd(hour, 1, tr.TransactionDateTime) as trade_record_time,
	...
	Login,
	   ...
	   case when  ISNULL(u.AORSUsername,Login)='BBNTRST' then 'NTRSCBOE'
              								   							else  ISNULL(u.AORSUsername,Login)
		                                      					end)   as account_name,
	CONCAT ( coalesce(nullif(torm.[DashAlias], ''), case when  ISNULL(u.AORSUsername,Login)='BBNTRST' then 'NTRSCBOE'
              								                   else  ISNULL(u.AORSUsername,Login)
		                                                       end),
                    isnull(tor.GiveUpFirm, tr.[ExecutingBroker]),
			case when isnull(tor.GiveUpFirm, '') = '792'
			     then case when isnull(nullif(tor.CMTAFirm,''), '949') = '949'
				      then 'PTA'
					  else null
					  end
                 else null
...
--    case when ISNULL(u.AORSUsername,Login)='BBNTRST' then 'NTRSCBOE' else ISNULL(u.AORSUsername,Login) end as account_name,
/*	case when u.Login='BBNTRST' then 'NTRSCBOE' end as account_name,*/
	...
	den.last_mkt as den_last_mkt,
	den1.last_mkt as den1_last_mkt,
	tr.ExDestination,
	nullif(coalesce(den.last_mkt,den1.last_mkt,tr.ExDestination),'') as chk,

	case when nullif(coalesce(den.last_mkt,den1.last_mkt,tr.ExDestination),'') in ('CBOE-CRD NO BK','PAR','CBOIE') then 'W'
		 when nullif(coalesce(den.last_mkt,den1.last_mkt,tr.ExDestination),'') in ('XPAR','PLAK','PARL') then 'LQPT'
		 when nullif(coalesce(den.last_mkt,den1.last_mkt,tr.ExDestination),'') in ('SOHO','KNIGHT','LSCI','NOM') then 'ECUT'
		 ...
		 when nullif(coalesce(den.last_mkt,den1.last_mkt,tr.ExDestination),'') in ('ACT','BOE','OTC','lp','VOL')  then 'BRKPT'
	...
	case when nullif(coalesce(den1.mic_code,tr.ExDestination),'') in ('CBOE-CRD NO BK','PAR','CBOIE') then 'XCBO'
...
/*	isnull(nullif(tr.[ExecutingBroker],''),tor.GiveUpFirm) as exec_broker,*/
	...
	case when tor.ForWhom in (1,25,32,78) then '0'
...
tl.TypeCode,
   ...
   CASE
WHEN tor.ORIGOrderID <> '00000000-0000-0000-0000-000000000000' or tor.ContraOrderID <> '00000000-0000-0000-0000-000000000000' then 26
WHEN tor.ParentOrderID <> '00000000-0000-0000-0000-000000000000'  or (tor.ParentOrderID = '00000000-0000-0000-0000-000000000000' and tor.ChildOrders!=0) then  10
...
ELSE 50 end as  strategy_decision_reason_code,
	   ...
	   case
		...
		case when nullif(tl.[ExpirationDate],'1900-01-01 00:00:00.000') is not null and nullif(tl.[Strike],0.00) is not null then 'O'
				else 'E'
...
--   case when u.Login= 'TRAFXPSC' and coalesce(nullif(tor.AccountAlias,''),uam.Alias) is null then tor.Account else coalesce(nullif(tor.AccountAlias,''),uam.Alias) end as AccountAlias,
   --tor.AccountAlias,
   /*case  when  tor.SystemID = 8 and tor.SystemOrderID like 'f_%' then 1
			when tr.Status = 151 then coalesce([EDW_Billing].[dbo].[f_get_is_trade_SOR_routed_daily_temp](tr.ID),0)
        else 0
   end as is_sor_routed,*/
	tr.ReportID,
	(case
        when lag(c.companyname,1) over (partition by tr.ExchangeTransactionID order by tor.generation) <> c.companyname
         and lag(tor.OrderID,1) over (partition by tr.ExchangeTransactionID order by tor.generation) = tor.ParentORdeRID  then 1
        else 0
   ...
   -- tor.generation,
	...
	case when replace(replace(tl.[BaseCode],'.','/'),'-','/') like '[0-9]%' then 1
		 else 0
	end as  real_flex_order,
	convert(numeric(16,8), (convert(numeric(16,0), tr.LastPrice)/100000000)) as last_px_temp, tor.SystemID, tor.SystemOrderID, tr.Status
...
----outer apply (select top 1 Alias from #TUserAccountAlias_temp uam with(nolock)--FlexTran.dbo.TUserAccountAlias uam with(nolock)
----						where  u.UserID =uam.UserID and u.SystemID = 2 and torm.DashAlias =uam.[DashAlias] ) uam
left join [LiquidPoint_EDW].[dbo].[TCompany] c with (nolock) on tor.CompanyID = c.ID
left join [EDW_Billing].[dbo].[dash_exchange_names] den with (nolock) on den.mic_code=replace(replace(tr.ExDestination,'DIRECT-',''),' Printer','') and den.real_exchange_id=den.exchange_id and den.mic_code!='' and den.is_active='TRUE'
left join [EDW_Billing].[dbo].[dash_exchange_names] den1 with (nolock) on den1.exchange_id=replace(replace(tr.ExDestination,'DIRECT-',''),' Printer','')  and den1.real_exchange_id=den1.exchange_id and den1.mic_code!='' and den1.is_active='TRUE'
left join (select r.ID
			...
			) bust on bust.ID = tr.ID
where 1=1
--and tr.Status in (151, 156, 239)
--and tor.SystemID in(2,3,8)
--  and tor.SystemOrderTypeID <> 87
    and tr.TransactionDateTime>= '2024-10-24'
  and tr.TransactionDateTime<  '2024-10-25'
and tr.orderid = '00000000-000F-0000-0000-0006F313AD80'