CREATE OR REPLACE FUNCTION dash360.report_billing_belved01_ecr(in_start_date_id int4, in_end_date_id int4)
    RETURNS TABLE
            (
                roe text
            )
...
round((coalesce(tcb."CATTF$", 0.0))::numeric, 8)                                                         as "FINRA CAT Fees"
    from billing.billing_data.tcustomer_billing_detail_all tcb
             join billing.billing_data.fdw_dash_trade_record tr
                  on (tr.date_id between :start_date_id and :end_date_id and
    ...
    where true
--    and tcb.date between :start_date_id and :end_date_id --convert(date, getdate()-day(getdate()-1)) and convert(date, getdate()-2)
