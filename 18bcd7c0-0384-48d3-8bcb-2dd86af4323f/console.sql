...
$$
    ...
    exception
        when others then raise notice 'Cannot set not nulls. Check it manually';
    end;
$$;

-- Uncomment it if you install it the first time
/*
alter function staging.db_cleanup_table_trigger_part(varchar, varchar, int4) set schema trash;
alter function trash.db_cleanup_table_trigger_part(varchar, varchar, int4) rename to db_cleanup_table_trigger_part_old;
alter function staging.db_cleanup_table_native_part(varchar, varchar, int4) set schema trash;
alter function trash.db_cleanup_table_native_part(varchar, varchar, int4) rename to db_cleanup_table_native_part_old;
alter function staging.db_cleanup_table (varchar, varchar, int4) set schema trash;
alter function trash.db_cleanup_table (varchar, varchar, int4) rename to db_cleanup_table_old;
alter function staging.db_cleanup_data_rolloff() set schema trash;
...
insert into db_management.table_retention(schema_name, table_name, retention_period, cleanup_schedule, key_field, is_active)
