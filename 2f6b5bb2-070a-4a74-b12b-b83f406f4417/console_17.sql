...
(
	...
	location_name VARCHAR(150) UNIQUE
);

CREATE TABLE IF NOT EXISTS t_apartments
(
	apartment_id serial NOT NULL PRIMARY KEY,
	hotel_id integer NOT NULL,
    capacity integer,
    type_id integer NOT NULL,
	status CHAR DEFAULT 'U' CHECK (status IN ('B','F','U')),
...
(
	...
	beg_date timestamp with time zone,
	end_date timestamp with time zone,
	full_price numeric(9,2)
);
...
CREATE OR REPLACE FUNCTION hotel_add
	...
	DECLARE
		new_id integer := 0;
	BEGIN
		IF (p_hotel_name IS NULL) THEN
		RAISE EXCEPTION 'ERROR: parameter p_hotel_name does not set'
			USING HINT = 'Please set parameter correctly';
		END IF;

		IF (p_hotel_location_id IS NULL) THEN
		RAISE EXCEPTION 'ERROR: parameter p_hotel_location_id does not set'
			USING HINT = 'Please set parameter correctly';
		END IF;

		IF (p_hotel_score IS NULL) THEN
		RAISE EXCEPTION 'ERROR: parameter p_hotel_score does not set'
			USING HINT = 'Please set parameter correctly';
		END IF;
...
CREATE OR REPLACE FUNCTION apartment_add
	...
	BEGIN
		...
		IF (p_capacity IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_capacity does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;

		IF (p_type_id IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_type_id does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;

		IF (p_status IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_status does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;

		IF (p_score IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_score does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;

		IF (p_price IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_price does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;
...
CREATE OR REPLACE FUNCTION client_add
	...
	BEGIN
		IF (p_full_name IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_full_name does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;
...
CREATE OR REPLACE FUNCTION booking_add
	...
	BEGIN
		...
		IF (p_end_date IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_end_date does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;
		IF (p_full_price IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_full_price does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;
...
CREATE OR REPLACE FUNCTION payment_add
	...
	BEGIN
		IF (p_payment_date IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_payment_date does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;
		IF (p_pay_sum IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_pay_sum does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;
...
CREATE OR REPLACE FUNCTION payment_upd
	...
	BEGIN
		...
		IF (p_payment_info IS NULL) THEN
			RAISE EXCEPTION 'ERROR: parameter p_payment_info does not set'
				USING HINT = 'Please set parameter correctly';
		END IF;
...
SELECT * FROM hotel_add(p_hotel_location_id := 4, p_hotel_score := 1); -- не можна передати null