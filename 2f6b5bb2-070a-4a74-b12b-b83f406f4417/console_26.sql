...
create or replace function loader.fixer (in_action text default 'check'::text, in_date_id int4 default current_date::text::int4)
returns int
    language plpgsql
as $fn$
declare
...
begin
    ...
    if in_action = 'check' then
       ...
       dl3.start_position,
       dl3.end_position,
       case when dl3.loading_comment = 'Not matched' then dl3.loading_comment else 'Before the mismatched batch' end
from loader.daily_load as dl1
         join lateral (select *
                       from loader.daily_load dl2
                       where dl2.file_id = dl1.file_id
                         and dl2.batch_id < dl1.batch_id
...
where dl1.loading_comment = 'Not matched'
        on conflict (batch_id) do nothing;

        insert into loader.fixer (date_id, batch_id, file_id, start_position, end_position, reason)
        select date_id, batch_id, file_id, start_position, end_position, 'no data in the batch'
        from loader.daily_load
        where coalesce(loading_confirmed, true) = false
        on conflict (batch_id) do nothing;

        with to_del as (
            select batch_id from loader.fixer
                            where date_id = in_date_id
                            and processed is null
        )
        delete from loader.daily_load dl using to_del
        where date_id = in_date_id
        and batch_id = to_del.batch_id;

        get diagnostics l_row_cnt = row_count;
elseif in_action = 'fix'
