...
select GENESIS2_QA_20100601.GET_ACCOUNT_STATIC_DATA(263359, 'O', 'V', 'CBOE', 'Y')
from dual;

select GENESIS2_QA_20100601.GET_ACCOUNT_STATIC_DATA(
    263201,
    'O',
    null,
    'CBOE',
...
{"ACCOUNT": "katecross_9","OPT_EXEC_BROKER": "019","OPT_IS_FIX_EXECBROK_PROCESSED": "Y","OPT_CLEARING_FIRM": "","OPT_IS_FIX_CLFIRM_PROCESSED": "Y","OPT_CUST_OR_FIRM": "","OPT_IS_FIX_CUSTFIRM_PROCESSED": "Y","OPT_OCC_ID": "","OPT_SUB_ACC": "","EQ_MPID": "DFIN","EQ_ORDER_CAPACITY": ""}
CREATE OR REPLACE FUNCTION GENESIS2_QA_20100601.GET_ACCOUNT_STATIC_DATA2(
    in_account_id NUMBER,
    in_instrument_type CHAR,
    in_opt_exec_broker VARCHAR2,
    in_exchange_id VARCHAR2,
    in_is_cross CHAR default 'N'
) RETURN varchar2
    is
...
-- III. ClearingFirm(439)
    ...
    if in_instrument_type = 'O' then
        select case
                   when l_opt_customer_or_firm in
                        (select CUSTOMER_OR_FIRM_ID from CUSTOMER_OR_FIRM where CAPACITY_GROUP_ID = 4)
                       and in_is_cross = 'Y' then e.home_mmid_cross_tag
                   when l_opt_customer_or_firm in
                        (select CUSTOMER_OR_FIRM_ID from CUSTOMER_OR_FIRM where CAPACITY_GROUP_ID = 4)
                       and in_is_cross = 'N' then e.home_mmid_tag
                   when l_opt_customer_or_firm in
                        (select CUSTOMER_OR_FIRM_ID from CUSTOMER_OR_FIRM where CAPACITY_GROUP_ID = 5)
                       and in_is_cross = 'Y' then e.away_mmid_cross_tag
                   when l_opt_customer_or_firm in
                        (select CUSTOMER_OR_FIRM_ID from CUSTOMER_OR_FIRM where CAPACITY_GROUP_ID = 5)
                       and in_is_cross = 'N' then e.away_mmid_tag
                   else -1
                   end
        into l_tag
        from exchange e
...
-- III. ClearingFirm(439)
	...
	SELECT COUNT(*)
		INTO l_is_group4
	FROM customer_or_firm
		WHERE customer_or_firm_id = l_opt_customer_or_firm
  			AND capacity_group_id = 4;

	IF in_instrument_type = 'O' AND l_is_group4 > 0 THEN
   	SELECT max(stv.TAG_VALUE)
        into l_opt_sub_acc
        FROM ACCOUNT2EXCHANGE ae
                 join SPECIFIC_TAG_SET sts
