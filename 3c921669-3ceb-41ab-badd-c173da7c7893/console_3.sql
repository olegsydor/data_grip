CREATE TABLE "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP"
(
    "DASH_EXEC_BROKER" varchar2(20) NOT NULL,
    "OPT_EXEC_BROKER"  varchar2(20) NOT NULL,
    CONSTRAINT "PK_SG_DASH_EXEC_BROKER_MAP" PRIMARY KEY ("DASH_EXEC_BROKER")
);
alter table "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" add "TRADING_FIRM_ID" VARCHAR2(9) references "GENESIS2_QA_20100601"."SG_TRADING_FIRM" ("TRADING_FIRM_ID");
commit;
update "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP"
    set TRADING_FIRM_ID = '360sga'
where TRADING_FIRM_ID is null;
select * from "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP";

ALTER TABLE GENESIS2_QA_20100601.SG_DASH_EXEC_BROKER_MAP DROP CONSTRAINT PK_SG_DASH_EXEC_BROKER_MAP;
ALTER TABLE GENESIS2_QA_20100601.SG_DASH_EXEC_BROKER_MAP ADD CONSTRAINT SG_DASH_EXEC_BROKER_MAP_UNIQUE UNIQUE (DASH_EXEC_BROKER,TRADING_FIRM_ID) ENABLE;

insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('BAYCRESTAMUS', 'BAYC');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('EURO-STOCK', 'BGCF');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('BTIGSANFAMUS', 'BTIG');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('CANACCORD-A', 'CAAC');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('DASHFINAFIUS', 'DFIN');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('DYTRLLCFIUS', 'MEDX');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('EXANE-NYSDUS', 'EXNE');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('FGCSECSFIUS', 'FGCS');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('FOGSECUS', 'FOGS');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('GARBAN-INTER', 'GARB');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('ELI-GFISECNY', 'GFIS');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('HAMILTON', 'HAME');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('ENGLANDER', 'IAEC');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('LAKESHOR-SEC', 'LAKE');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('LIBUCKI-BK', 'LBKI');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('LOUIS-CM-NY', 'LOUI');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('MPSGLOBAFIUS', 'MPSG');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('OSCARNY', 'OGRU');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('OTCEXFIUS', 'OTCT');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('PENSERRACOUS', 'PNSA');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('PUMACLLCFIUS', 'PUMA');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('SUNRISBKUS', 'SUBR');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('TFSDERNY', 'TFAS');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('TJM', 'TJMI');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('TULIBSECFIUS', 'TPIS');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('WALLSTACSDUS', 'WABR');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('WOLVERINFIUS', 'WEXX');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('X-CHANGE', 'XCHF');
insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER)
VALUES ('XPSECURFIUS', 'XPUS');

commit;
select * from "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP";

CREATE OR REPLACE FUNCTION GENESIS2_QA_20100601.get_sg_account(
    in_account_id NUMBER,
    in_instrument_type CHAR,
    in_opt_exec_broker VARCHAR2,
    in_exchange_id VARCHAR2,
    in_dash_broker varchar2
) RETURN varchar2
    is

    l_opt_customer_or_firm   varchar2(10);
    l_opt_is_fix_execbrok_pr varchar2(10);
    l_opt_is_fix_clfirm_pr   varchar2(10);
    l_opt_is_fix_custfirm_pr varchar2(10);
    l_new_model              char;
    l_trading_firm_id        varchar2(9);
    l_eq_mpid                varchar2(4);
    l_opt_occ_id             varchar2(40);
    l_clearing_firm          varchar2(40);
    l_sg_sub_account         varchar2(10);
    l_sg_mint_account        varchar2(20);
    l_sg_sales_trader_id     varchar2(20);
    l_client_cust_or_firm    varchar2(20) := '';
    l_opt_exec_broker        varchar2(20);
    l_return                 varchar2(1500);
    l_parent_sub_account     char;
    l_parent_mint_account    char;
    l_parent_sales_trader_id char;
    l_opt_dash_broker        varchar2(20);

begin

    SELECT
        -- acc.ACCOUNT_NAME accountName,
        max(acc.OPT_CUSTOMER_OR_FIRM),
        max(coalesce(acc.OPT_IS_FIX_EXECBROK_PROCESSED,'N')),
        max(coalesce(acc.OPT_IS_FIX_CLFIRM_PROCESSED,'N')),
        max(coalesce(acc.OPT_IS_FIX_CUSTFIRM_PROCESSED,'N')),
        max(acc.TRADING_FIRM_ID),
        max(acc.eq_mpid),
        max(acc.opt_occ_id),
        max(coalesce(acc.USE_NEW_MODEL,'N'))
    into l_opt_customer_or_firm, l_opt_is_fix_execbrok_pr, l_opt_is_fix_clfirm_pr, l_opt_is_fix_custfirm_pr, l_trading_firm_id,
        l_eq_mpid, l_opt_occ_id, l_new_model
    FROM aCCOUNT acc
    WHERE 1 = 1
      and acc.ACCOUNT_ID = in_account_id;

    -- ExecBroker(76)

    if in_dash_broker is not null then
        select max(OPT_EXEC_BROKER)
        into l_opt_dash_broker
        from SG_DASH_EXEC_BROKER_MAP
        where DASH_EXEC_BROKER = IN_DASH_BROKER
          and TRADING_FIRM_ID = (select TRADING_FIRM_ID from SG_ACCOUNT where ACCOUNT_ID = in_account_id);
    end if;

    IF l_opt_is_fix_execbrok_pr = 'N' OR in_opt_exec_broker IS NULL  THEN
    -- The OPT_EXEC_BROKER view needs to be used. All the logic regarding new/old model hiden inside

            SELECT max(OPT_EXEC_BROKER)
            into l_opt_exec_broker
            FROM OPT_EXEC_BROKER OE
            WHERE OE.IS_DELETED = 'N'
              AND OE.ACCOUNT_ID = in_account_id
              AND OE.IS_DEFAULT = 'Y';

--        if l_new_model = 'Y' then
--            SELECT max(OE.OPT_EXEC_BROKER)
--            into l_opt_exec_broker
--            FROM GENESIS2_QA_20100601.OPT_EXEC_BROKER_CONFIG OE
--                     INNER JOIN GENESIS2_QA_20100601.ACCOUNT2OPT_EXEC_BROKER_CONFIG AO
--                                on AO.OPT_EXEC_BROKER_CONFIG_ID = OE.OPT_EXEC_BROKER_CONFIG_ID
--            WHERE OE.IS_DELETED = 'N'
--              AND AO.ACCOUNT_ID = in_account_id
--              AND AO.IS_DEFAULT = 'Y';
--        else
--            SELECT max(OPT_EXEC_BROKER)
--            into l_opt_exec_broker
--            FROM OPT_EXEC_BROKER OE
--            WHERE OE.IS_DELETED = 'N'
--              AND OE.ACCOUNT_ID = in_account_id
--              AND OE.IS_DEFAULT = 'Y';
--        end if;
    ELSE
        -- l_opt_exec_broker := 'null';
        l_opt_exec_broker:=in_opt_exec_broker;

    end if;

-- II. CustomerOrFirm(204)
    IF l_opt_is_fix_custfirm_pr =  'Y' THEN
        -- l_opt_is_fix_custfirm_pr := 'null';
        -- l_client_cust_or_firm := 'null';
       l_opt_customer_or_firm := '';
    end if;

-- III. ClearingFirm(439)

    IF (in_instrument_type = 'E' or l_opt_is_fix_clfirm_pr = 'Y' or l_opt_exec_broker is null) then
        l_clearing_firm := '';
    else
        if l_new_model = 'Y' then
            select max(tv.TAG_VALUE)--, abc.ACCOUNT_ID, ocp.EXCHANGE_ID,  obc.OPT_EXEC_BROKER
            into l_clearing_firm
            from OPT_EXEC_BROKER_CONFIG obc
            join ACCOUNT2OPT_EXEC_BROKER_CONFIG abc on abc.OPT_EXEC_BROKER_CONFIG_ID = obc.OPT_EXEC_BROKER_CONFIG_ID
            join EXEC_BROKER_CONFIG2EXCH_PARAM ocp  on ocp.OPT_EXEC_BROKER_CONFIG_ID = obc.OPT_EXEC_BROKER_CONFIG_ID
            join SPECIFIC_TAG_VALUE tv              on tv.SPECIFIC_TAG_SET_ID = ocp.SPECIFIC_TAG_SET_ID
            where 1 = 1
              and abc.ACCOUNT_ID = in_account_id
              and obc.OPT_EXEC_BROKER = l_opt_exec_broker
              and ocp.EXCHANGE_ID = in_exchange_id
              and abc.IS_DEFAULT = 'Y'
              and tv.TAG_NUMBER = 439;
        else
            select max(stv.TAG_VALUE)
            into l_clearing_firm
            from OPT_EXEC_BROKER_OLD od
            join OPT_EXEC_BROKER2EXCH_PARAM_OLD ep   on ep.OPT_EXEC_BROKER_ID = od.OPT_EXEC_BROKER_ID
            join SPECIFIC_TAG_VALUE stv              on stv.SPECIFIC_TAG_SET_ID = ep.SPECIFIC_TAG_SET_ID
            where od.ACCOUNT_ID = in_account_id
              and od.OPT_EXEC_BROKER = l_opt_exec_broker
              and ep.EXCHANGE_ID = in_exchange_id
              and od.IS_DEFAULT = 'Y'
              and stv.TAG_NUMBER = 439
              and od.is_deleted = 'N';
        end if;
    end if;


    --     IV. ActionableID(10440)


-- V. SG Account fields

/*
 * this piece of code should be used instead of the next 7 rows when SG_NULL_ACCOUNT_PARAMETER is present in DB (OS)
 * https://dashfinancial.atlassian.net/browse/DS-10332
*/
select max(case when DB_FIELD_NAME = 'SG_SUB_ACCOUNT' then 'Y' end),
       max(case when DB_FIELD_NAME = 'SG_MINT_ACCOUNT' then 'Y' end),
       max(case when DB_FIELD_NAME = 'SG_SALES_TRADER_ID' then 'Y' end)
into l_parent_sub_account, l_parent_mint_account, l_parent_sales_trader_id
from SG_NULL_ACCOUNT_PARAMETER
where ACCOUNT_ID = in_account_id;

        select MAX(coalesce(ch.SG_SUB_ACCOUNT, case when l_parent_sub_account = 'Y' then pa.SG_SUB_ACCOUNT end)),
           MAX(coalesce(ch.SG_MINT_ACCOUNT, case when l_parent_MINT_ACCOUNT = 'Y' then pa.SG_MINT_ACCOUNT end)),
           MAX(coalesce(ch.SG_SALES_TRADER_ID, case when l_parent_SALES_TRADER_ID = 'Y' then pa.SG_SALES_TRADER_ID end))
    into l_sg_sub_account, l_sg_mint_account, l_sg_sales_trader_id
    from SG_ACCOUNT ch
    left join SG_ACCOUNT pa  on pa.ACCOUNT_ID = ch.SG_PARENT_ACCOUNT_ID
    where ch.ACCOUNT_ID = in_account_id;

/*
    select MAX(coalesce(ch.SG_SUB_ACCOUNT, pa.SG_SUB_ACCOUNT)),
           MAX(coalesce(ch.SG_MINT_ACCOUNT, pa.SG_MINT_ACCOUNT)),
           MAX(coalesce(ch.SG_SALES_TRADER_ID, pa.SG_SALES_TRADER_ID))
    into l_sg_sub_account, l_sg_mint_account, l_sg_sales_trader_id
    from SG_ACCOUNT ch
    left join SG_ACCOUNT pa  on pa.ACCOUNT_ID = ch.SG_PARENT_ACCOUNT_ID
    where ch.ACCOUNT_ID = in_account_id;
*/

    select '{' ||
           '"OPT_IS_FIX_CLFIRM_PROCESSED": "' || l_opt_is_fix_clfirm_pr || '",' ||
           '"OPT_CLEARING_FIRM": "' || l_clearing_firm || '",' ||
           '"OPT_IS_FIX_CUSTFIRM_PROCESSED": "' || l_opt_is_fix_custfirm_pr || '",' ||
           '"OPT_CUST_OR_FIRM": "' || l_opt_customer_or_firm || '",' ||
           '"CLIENT_CUST_OR_FIRM": "' || l_client_cust_or_firm || '",' ||
           '"OPT_IS_FIX_EXECBROK_PROCESSED": "' || l_opt_is_fix_execbrok_pr || '",' ||
           '"OPT_EXEC_BROKER": "' || l_opt_exec_broker || '",' ||
           '"OPT_OCC_ID": "' || l_opt_occ_id || '",' ||
           '"SG_SUB_ACCOUNT": "' || l_sg_sub_account || '",' ||
           '"SG_MINT_ACCOUNT": "' || l_sg_mint_account || '",' ||
           '"SG_SALES_TRADER_ID": "' || l_sg_sales_trader_id ||
           '"OPT_EXEC_BROKER_BY_DASH_BROKER": "' || l_opt_dash_broker ||
           '"}'
--select 'DATA'
    into l_return
    from dual;

    return l_return;

EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RETURN 'NO DATA';
    WHEN OTHERS THEN
        RAISE;
END;


insert into "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP" (DASH_EXEC_BROKER, OPT_EXEC_BROKER, TRADING_FIRM_ID)
select DASH_EXEC_BROKER, OPT_EXEC_BROKER, 'socgenps'
from "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP"
where TRADING_FIRM_ID = '360sga'
    ;
commit

    select *
from "GENESIS2_QA_20100601"."SG_DASH_EXEC_BROKER_MAP";

    select *
from "GENESIS2_QA_20100601"."SG_TRADING_FIRM";



INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '101', 'Added displayed volume', 1, 'N');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '153', 'Added displayed volume – NBBO improved', 1, 'N');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '154', 'Added displayed volume – NBBO joined', 1, 'N');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '162', 'Added displayed volume – price improvement', null, 'N');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '102', 'Removed volume', 2, 'N');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '160', 'Removed volume on entry', 2, 'N');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '161', 'Removed volume –immediate Midpoint removed', 1, 'N');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '103', 'Routed removed volume', 3, 'N');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '151', 'Added non-displayed volume', 1, 'Y');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '163', 'Added non-displayed volume – price improvement', 1, 'Y');
INSERT INTO LIQUIDITY_INDICATOR (EXCHANGE_ID, TRADE_LIQUIDITY_INDICATOR, DESCRIPTION, LIQUIDITY_INDICATOR_TYPE_ID,
                                 IS_GREY)
VALUES ('24X', '152', 'Added Midpoint', 1, 'Y');
