-- DROP FUNCTION data_marts.so_load_ats_cons_inc(_int8, int4);
select * from data_marts.load_ats_cons_inc(in_recalc_date_id := 20250307)
;
...
-- MOCK RFQ when recalc
  ...
  then
      ...
      l_local_rfq_id := (select coalesce(max(q.rfq_id), -1) as local_rfq_id
                        ...
                        1000; -- why this 10000 is here if we do not have on conflict update later in insert?
  else
      l_local_rfq_id := -1;

  end if;
  l_max_rfq_id := (select max(r.rfq_id)
...
--       , to_timestamp(v.rfq_transact_time, 'YYYYMMDD HH24:MI:SS.MS')::timestamp without time zone as rfq_transact_time
  ...
  limit 1000000 -- potential bug!!! (SO)
    ;
      ...
      left join lateral
        (
          select sum(tr.last_qty) as filled_qty
            ...
            , max(tr.trade_record_id) as max_trade_record_id
          from dwh.flat_trade_record tr
          where true -- try to calculate for both ATS and CONS (maybe CONS will be calculated on the street executions)
