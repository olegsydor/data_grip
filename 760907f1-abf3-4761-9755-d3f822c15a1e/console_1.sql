...
CREATE TABLE public.holiday_calendar (
	...
	date_start timestamp NULL DEFAULT now(),
...
CREATE OR REPLACE FUNCTION public.get_business_date_plpgsql(in_date date DEFAULT ('now'::text)::date, in_offset integer DEFAULT 0, ignore_banking_holiday boolean DEFAULT true)
 RETURNS date
 LANGUAGE plpgsql
 AS $function$
begin
...
CREATE OR REPLACE FUNCTION public.get_dateid_plpgsql(period date)
    RETURNS integer
    LANGUAGE plpgsql
...
CREATE OR REPLACE FUNCTION public.get_business_date_gth_plpgsql(in_timestamp timestamp without time zone DEFAULT (now())::timestamp without time zone)
    RETURNS integer
    LANGUAGE plpgsql
AS
$function$
begin
    return (select case
                       when to_char(in_timestamp, 'HH24')::int >= 19
                           then public.get_dateid_plpgsql(public.get_business_date_plpgsql(in_timestamp::date, 1, true))
                       else public.get_dateid_plpgsql(public.get_business_date_plpgsql(in_timestamp::date, 0, true))
                       end);
end;
$function$
;

create table training.like_l1_snapshot
(
...
select * from training.like_l1_snapshot


select $$insert into training.like_l1_snapshot (input_date) values (public.get_business_date_gth_plpgsql('$$||
       ('2023-11-30'::timestamp + interval '1 minute' * round(random()::numeric * 1440, 0))::text||$$'));$$
