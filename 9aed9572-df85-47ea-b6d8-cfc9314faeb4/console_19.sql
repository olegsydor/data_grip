SELECT id,
       ...
       crossingtypeid,
       round(stoplimit::bigint / 10000.0, 4)                                                       as stoplimit,
       catid,
       dashaliasid,
       minquantity::int,
       mindisplayqty::int,
       ...
       coalesce(boxqooannouncedtimev2::timestamptz at time zone 'US/Central',
                boxqooannouncedtime)                                                               as boxqooannouncedtime,
       ordernotes,
       cboeequitybroker,
       order_trade_date_id,
       autorouteruleid,
       ...
       IsTiedToCombo,
       IsMerged,
       public.clordid_to_guid(MergedOrderId)                                                       as MergedOrderId
FROM blaze7.tordermisc1_edw as x
where order_trade_date_id between %(date_id)s and public.get_dateid(public.get_business_date(%(date_id)s::text::date, 1))
AND x._last_mod_time::timestamp >  %(max_processed_time_misc)s::timestamp at time zone 'US/Central' - interval '1 second'