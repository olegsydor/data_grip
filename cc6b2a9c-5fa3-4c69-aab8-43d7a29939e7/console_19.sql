CREATE OR REPLACE FUNCTION get_client_commission_rate(
    ...
    IS
    l_touch_type_id      EX_DESTINATION_CODE.TOUCH_TYPE_ID%TYPE;
    l_rate               acct_comm_opt_rate.rate%TYPE;
    -- 20260119 SO https://dashfinancial.atlassian.net/browse/DS-10947 is expected to used instead of GET_COMMISSION_RATE

    -- GET symbol_list_id
    TYPE t_symbol_list_ids IS TABLE OF acct_comm_opt_rate.symbol_list_id%TYPE
...
BEGIN
    ...
    if in_forced_flow_type is null then
        ...
        FROM EXCHANGE E
                 JOIN EX_DESTINATION D ON D.EXCHANGE_ID = E.EXCHANGE_ID
                 JOIN EX_DESTINATION_CODE C ON C.EX_DESTINATION_CODE = D.EX_DESTINATION_CODE
                 JOIN RISK_MGMT_TOUCH_TYPE T ON C.TOUCH_TYPE_ID = T.TOUCH_TYPE_ID
    ...
    if in_instrument_type = 'O' then
        ...
        FROM acct_comm_opt_rate optr
                 JOIN acct_comm_symbol_list sl ON optr.symbol_list_id = sl.symbol_list_id
                 JOIN symbol2acct_comm_symbol_list asl ON asl.symbol_list_id = sl.symbol_list_id
                 JOIN tf2acct_comm_symbol_list tfsl ON tfsl.symbol_list_id = sl.symbol_list_id
        ...
        from (select *
              from (SELECT 1 as prioritet,
                           ...
                           prt.min_price
                    FROM acct_comm_opt_rate optr
                             JOIN acct_comm_opt_premium_tier prt ON prt.tier_id = optr.tier_id
                      ...
                      AND case
                              when l_symbol_list_id_cnt > 0 and optr.symbol_list_id IN
                                                                (SELECT optr.symbol_list_id
                                                                 FROM acct_comm_opt_rate optr
                                                                          JOIN acct_comm_symbol_list sl
                                                                               ON optr.symbol_list_id = sl.symbol_list_id
                                                                          JOIN symbol2acct_comm_symbol_list asl
                                                                               ON asl.symbol_list_id = sl.symbol_list_id
                                                                          JOIN tf2acct_comm_symbol_list tfsl
                                                                               ON tfsl.symbol_list_id = sl.symbol_list_id
                                                                          JOIN trading_firm tf
...
--                        when l_symbol_list_id_cnt > 0 and optr.symbol_list_id is null then 1
        ...
        from (SELECT 1 as prioritet,
                     optr.rate
              FROM acct_comm_eqt_rate optr
--                       JOIN sg_acct_comm_eqt_rate sacer prt ON prt.tier_id = optr.tier_id
              WHERE optr.account_id = in_account_id
                AND optr.touch_type_id = l_touch_type_id
                AND case
                        when l_symbol_list_id_cnt > 0 and optr.symbol_list_id IN
...
CREATE OR REPLACE FUNCTION get_data_for_delayed_drop(
    in_account_id NUMBER,
    in_instrument_type CHAR,
...
begin
	...
	-- 20260225 SO https://dashfinancial.atlassian.net/browse/DS-10997 (ticket in our board)
