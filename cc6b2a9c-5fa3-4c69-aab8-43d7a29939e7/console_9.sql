...
--   and CL.EXCH_ORDER_ID = '300044233420'
--		  when AC.TRADING_FIRM_ID <> 'imc01' then (select EXCH_ORDER_ID from CLIENT_ORDER where ORDER_ID = CL.PARENT_ORDER_ID) = 300044233420
--		  else (select EXCH_ORDER_ID from CLIENT_ORDER where ORDER_ID = (select max(PARENT_ORDER_ID) from CLIENT_ORDER where CROSS_ORDER_ID = CL.CROSS_ORDER_ID and IS_ORIGINATOR <> CL.IS_ORIGINATOR)) = 300044233420
--		end) --RFR_ID
  and trunc(EX.EXEC_TIME) = date '2024-04-23'
...
select *
from liquidity_indicator li
where li.trade_liquidity_indicator in ('AB', 'PC', 'CA');

select fm.* from client_order cl
         join EXECUTION EX on CL.ORDER_ID = EX.ORDER_ID
         left join FIX_MESSAGE_FIELD fm on cl.FIX_MESSAGE_ID = fm.FIX_MESSAGE_ID and fm.TAG_NUM = 9730
    where 1=1
  and trunc(EX.EXEC_TIME) = date '2024-04-23'
--   and CL.PARENT_ORDER_ID is null
and cl.CLIENT_ORDER_ID = 'CDAB2262-20240423';
