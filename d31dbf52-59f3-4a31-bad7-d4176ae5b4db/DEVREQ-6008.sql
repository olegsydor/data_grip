...
$function$
    ...
    -- 20250529 OS Changed Data source from hods to client_order
declare
    l_load_id       int;
    l_row_cnt       int;
    l_step_id       int;
...
begin
    ...
    from dwh.client_order co
             ...
             left join lateral (select fmj.fix_message ->> '10147' as t10147
...
create temp table t_os as
 ...
 select to_char(hods."StatusDate", 'Month') as "Month",
           ...
           count(distinct "StatusDate")        as "Actual Trading Days",
           tf.trading_firm_name::varchar       as "Firm",
           case when a.account_name = 'T3_BTIG_DESK' then 'TG814083'
               when a.account_name = 'T3_OPCO_DESK' then 'TG814083'
               else a.account_name end as "Account",
           cf.customer_or_firm_name::varchar   as "Capacity",
           count(distinct hods."ClOrdID")      as "Parent Order Count",
           hods."InstrumentType"
  , 'exc' as src
    ...
    where true
      ...
      and hods."CustomerOrderID" is null
      and hods."AccountID" = any (:l_account_ids)
    group by to_char(hods."StatusDate", 'Month'), to_char(hods."StatusDate", 'YYYY'), "Account",
             cf.customer_or_firm_name, tf.trading_firm_name,hods."InstrumentType"
...
select "Month", "Year", "Actual Trading Days", "Firm", "Account", "Capacity", "Parent Order Count", "InstrumentType" from t_os
where src = 'exc'
...
select * from t_os
where "Account" in('T3_OPCO_DESK','T3_BTIG_DESK','TG813803','TG814654','TG813938')
...
with base as (select distinct hods."ClOrdID"
 from dwh.historic_order_details_storage hods
    ...
    where true
      ...
      and hods."CustomerOrderID" is null
      and hods."AccountID" = any (:l_account_ids)
and a.account_name = 'T3_OPCO_DESK')
...
select *
from dwh.historic_order_details_storage hods
where "ClOrdID" = '525139001DT'
  and "Status_Date_id" >= :l_start_date_id
  and "Status_Date_id" <= :l_end_date_id
and "Status_Date_id" = 20250519;

select trans_type, * from dwh.client_order
where client_order_id = 'STE-CXL-2368028784'

select *
from dash360.report_compliance_avg_parent_order_count(in_start_date_id := 20250501, in_end_date_id := 20250528,
                                                      in_trading_firm_ids := '{OFP0058,OFP0077,t3trade01}');
