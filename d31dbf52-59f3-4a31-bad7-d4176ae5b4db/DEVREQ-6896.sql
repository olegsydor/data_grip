...
FROM dwh.gtc_order_status gos
    JOIN dwh.client_order cl ON cl.order_id = gos.order_id AND cl.create_date_id = gos.create_date_id
...
WHERE gos.close_date_id IS NULL
    ...
    AND case when i.last_trade_date >= CURRENT_DATE then true
    when i.last_trade_date IS null then true
    else false end
    AND ss.sub_system_id ='SEG3_OSR1'