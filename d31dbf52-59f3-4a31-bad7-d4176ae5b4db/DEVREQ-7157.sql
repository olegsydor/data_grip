...
create or replace function dash360.report_surveillance_socgen_parent_order_count(in_start_date_id integer DEFAULT get_dateid((date_trunc('month'::text, (CURRENT_DATE)::timestamp with time zone))::date),
                                                                               in_end_date_id integer DEFAULT get_dateid(CURRENT_DATE),
                                                                               in_instrument_type character varying DEFAULT NULL::character varying)
    RETURNS TABLE
            (
...
begin
    ...
    from dwh.d_account ac
    where true
      and trading_firm_id in ('socgenpsc', 'socgeneqd');
