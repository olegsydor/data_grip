-- https://dashfinancial.atlassian.net/browse/DEVREQ-7253

-- DROP FUNCTION trash.dash360_report_trades_v3(int4, int4, bpchar, _int4, _varchar);

CREATE OR REPLACE FUNCTION trash.dash360_report_trades_v3(in_start_date_id integer DEFAULT public.get_dateid(CURRENT_DATE), in_end_date_id integer DEFAULT public.get_dateid(CURRENT_DATE), in_instrument_type character DEFAULT NULL::bpchar, in_account_ids integer[] DEFAULT '{}'::integer[], in_trading_firm_ids character varying[] DEFAULT '{}'::character varying[])
 RETURNS TABLE("Trading Firm" character varying, "Account" character varying, "Street Cl Ord ID" character varying, "Cl Ord ID" character varying, "Secondary Ord ID" character varying, "Date" text, "Time" text, "Sec Type" text, "Ex Dest" character varying, "Sub Strategy" character varying, "Fee Sensitivity" smallint, "Status" character varying, "Side" text, "O/C" text, "Symbol" character varying, "Last Qty" integer, "Leaves Qty" integer, "Last Px" numeric, "Last Mkt" character varying, "Exchange Name" character varying, "MIC Code" character varying, "Bid Qty" text, "Bid Px" text, "Ask Px" text, "Ask Qty" integer, "Exec Bid Qty" integer, "Exec Bid Px" text, "Exec Ask Px" text, "Exec Ask Qty" integer, "Liquidity Ind" character varying, "Liq Ind Description" character varying, "Cust/Firm" character varying, "Exec Broker" character varying, "CMTA" character varying, "Client ID" character varying, "Expiration" text, "Root Symbol" character varying, "Put/Call" text, "Strike" numeric, "OSI Symbol" character varying, "Dash Exec ID" bigint, "Exch Exec ID" character varying, "Secondary Exch Exec ID" character varying, "Is Mleg" text, "Is Cross" character, "Sending Firm" character varying, "Sub System" character varying, "Free Text" character varying, "Principal Amount" numeric, "Commission" numeric, "Exchange Fees" text, "Exchange Fees/Unit" text, "Execution Cost" numeric, "Execution Cost/Unit" text, "Maker/Taker Fee" numeric, "Maker/Taker Fee/Unit" text, "Transaction Fee" text, "Trade Processing Fee" text, "Spread Transaction Fee" text, "QCC Rebate" text, "Royalty Fee" text, "Option Regulatory Fee" text, "OCC Fee" text, "SEC Fee" text, "CAT Fee" text, "Equity Clearing Fee" text, "Contra Broker" character varying, "Client Commission" numeric, "Dash Liq Ind Type" character varying, "OCC AID" character varying)
 LANGUAGE plpgsql
AS $function$
...
begin
    ...
    select public.load_log(l_load_id, l_step_id,
                           'dash360.MAREX - XFA for ' || in_start_date_id::text || ' - ' ||
                           in_end_date_id::text ||
                           ' STARTED===', 0, 'O')
    into l_step_id;
    ...
    return query
        select replace(tf.trading_firm_name, ',', '')::varchar                             as "Trading Firm",
               ...
               tr.secondary_order_id                                                       as "Secondary Ord ID",
               ...
               --case when tr.leaves_qty = 0 then 'Filled' else 'Partially filled' end       as "Status",
               case
               ...
               tr.secondary_exch_exec_id                                                   as "Secondary Exch Exec ID",
               case
               ...
               to_char(round(tr.spread_transaction_fee_amount, 4), 'FM999990.0000')        as "Spread Transaction Fee",
               to_char(round(qcc_rebate_amount, 8), 'FM999990.00999999')                   as "QCC Rebate",
        ...
        from dwh.flat_trade_record tr
                 ...
                 left join dwh.execution e on (e.exec_date_id between in_start_date_id and in_end_date_id and e.exec_date_id = tr.date_id and e.exec_id = tr.exec_id)
    ...
    select public.load_log(l_load_id, l_step_id,
                           'dash360.MAREX - XFA for ' || in_start_date_id::text || ' - ' || in_end_date_id::text ||
...
where account_name = 'TICKRSGTH';

select *
from trash.dash360_report_trades_v3(20251209, 20251209, 'O', '{74899}');