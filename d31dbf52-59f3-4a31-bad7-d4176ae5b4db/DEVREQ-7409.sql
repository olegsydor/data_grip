-- DROP FUNCTION trash.report_rps_s3(int4, int4, _int4, bpchar, _varchar);
create temp table t_os as
    insert into t_os
select ret_row, 'sos' as tp
from trash.report_rps_s3(in_start_date_id := 20251001, in_end_date_id := 20251001, in_is_multi_leg := 'Y',
                         in_trading_firm_ids := '{LPTF286,socgenpsc,socgen01}', in_exclude_blaze := true);

insert into t_os
select *, 'old'
from dash360.report_rps_s3(in_start_date_id := 20251001, in_end_date_id := 20251001, in_is_multi_leg := 'N',
                         in_trading_firm_ids := '{LPTF286,socgenpsc,socgen01}');

insert into t_os
select ret_row, 'sos' as tp
from trash.report_rps_s3(in_start_date_id := 20251001, in_end_date_id := 20251001, in_is_multi_leg := 'N',
                         in_trading_firm_ids := '{LPTF286,socgenpsc,socgen01}', in_exclude_blaze := true);
...
select * from t_os
where tp = 'mod'
--   and ret_row ilike '%00208058473ESNY1%'
and ret_row ilike '%blaze%'


select string_agg(account_id::text, ',')
from dwh.d_account
where trading_firm_id = any('{LPTF286,socgenpsc,socgen01}')

select * from dwh.d_trading_firm
where trading_firm_name = any('{"Societe Generale Financial","Societe Generale Financial Flow Desk","Societe Generale Financial PSC"}')



-- DROP FUNCTION trash.report_rps_s3(int4, int4, _int4, bpchar, _varchar, bool);

CREATE OR REPLACE FUNCTION trash.report_rps_s3(in_start_date_id integer, in_end_date_id integer, in_account_ids integer[] DEFAULT '{}'::integer[], in_is_multi_leg character DEFAULT 'N'::bpchar, in_trading_firm_ids character varying[] DEFAULT '{}'::character varying[], in_exclude_blaze boolean DEFAULT true)
...
--          on commit drop
    ...
    where true
      ...
      and case when in_exclude_blaze then cl.exchange_id not ilike 'blaze' else true end;
...
create table trash.check_s3_socgen as

insert into trash.check_s3_socgen
select ret_row, 'old' as report, :date_id as date_id
from dash360.report_rps_s3(in_start_date_id := :date_id, in_end_date_id := :date_id, in_is_multi_leg := 'N', in_trading_firm_ids := '{LPTF286,socgenpsc,socgen01}');
...
ac.broker_dealer_mpid,
     'NO'                                      as record_type,
           ...
           '|' || --OrderAction
           to_char(cl.process_time, 'YYYYMMDD') || 'T' || to_char(cl.process_time, 'HH24MISSFF3') || '|' ||
           coalesce(ot.order_type_short_name, '') || '|' || --order_type
           case
               when not :l_is_multileg then cl.order_qty::text
               else
                   case when cl.multileg_reporting_type = '3' then '' else cl.order_qty::text end
 ...
 array_to_string(ARRAY [
           ...
           case i.instrument_type_id when 'E' then i.display_instrument_id when 'O' then oc.opra_symbol else '' end ,
           '' , --primary Exchange
           case cl.side when '1' then 'B' when '2' then 'S' when '5' then 'SS' when '6' then 'SSE' else '' end , --OrderAction
           to_char(cl.process_time, 'YYYYMMDD') || 'T' || to_char(cl.process_time, 'HH24MISSFF3') ,
           coalesce(ot.order_type_short_name, '') , --order_type
           case
               when not :l_is_multileg then cl.order_qty::text
...
select cl.exchange_id, ex.exchange_id, *
from dwh.client_order cl
...
-- and cl.ex_destination ilike '%blaze%'
--   and cl.exchange_id not ilike '%blaze%'
--   and ex.exchange_id ilike '%blaze%'
  and ex.exec_type in ('4', '8', 'F')
  and ex.exec_date_id between :in_start_date_id and :in_end_date_id
and cl.order_id = 100000022969314819;
    ...
    select cl.ex_destination, cl.exchange_id,
           'NO'                                      as record_type,
           coalesce(cl.parent_order_id, cl.order_id) as order_id,
           to_char(cl.process_time, 'HH24MISSFF3')   as time_id,
           cl.client_order_id                        as record_id,
