...
select ret_row from trash.report_rps_s3(in_start_date_id := 20251001, in_end_date_id := 20251001, in_is_multi_leg := 'Y', in_trading_firm_ids := '{LPTF286,socgenpsc,socgen01}', in_exclude_blaze := true);


select *
from dash360.report_rps_s3(in_start_date_id := 20251001, in_end_date_id := 20251001, in_is_multi_leg := 'Y',
...
CREATE or replace FUNCTION trash.report_rps_s3(in_start_date_id integer, in_end_date_id integer,
...
AS $function$
    ...
    -- SO 20260108 https://dashfinancial.atlassian.net/browse/DEVREQ-7409 Add a new parameter: Exclude S3 EOD Blaze Orders
declare
...
select ret_row, 'old' as report, 20251008 as date_id
from dash360.report_rps_s3(in_start_date_id := 20251008, in_end_date_id := 20251008, in_is_multi_leg := 'N', in_trading_firm_ids := '{LPTF286,socgenpsc,socgen01}');

insert into trash.check_s3_socgen
select ret_row, 'new', 20251008
       from trash.report_rps_s3(in_start_date_id := 20251008, in_end_date_id := 20251008, in_is_multi_leg := 'N', in_trading_firm_ids := '{LPTF286,socgenpsc,socgen01}', in_exclude_blaze := true);
