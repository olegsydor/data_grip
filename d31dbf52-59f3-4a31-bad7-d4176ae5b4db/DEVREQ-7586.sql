CREATE or replace FUNCTION trash.report_obo_compliance_xls_with_clordid(in_date_begin_id integer,
                                                                        ...
                                                                        in_client_order_ids varchar(256)[] default '{}'::varchar(256)[])
    RETURNS TABLE
            (
                "OrderID"                   bigint,
...
-- SY: 20251013 left has been added to avoid ERROR SqlState: 22008 MessageText: date/time field value out of range: "20251013-19:25:16.657925405"
-- SO: 20251111 https://dashfinancial.atlassian.net/browse/DEVREQ-6896 add the in_fix_comp_ids parameter to the existing dash360.report_obo_compliance_xls function
-- SO: 20260203 https://dashfinancial.atlassian.net/browse/DEVREQ-7586 added in_client_order_id as an input parameter
declare
    l_load_id           int;
    l_step_id           int;
...
begin
    ...
    select public.load_log(l_load_id, l_step_id,
                           'dash360.report_obo_compliance_xls_new with cl_ord_id for ' || l_date_begin_id::text ||
                           ' - ' ||
                           l_date_end_id::text ||
                           ' STARTED===', 0, 'O')
    into l_step_id;
...
--                         and mleg.create_date_id >= cl.create_date_id
    ...
    where cl.parent_order_id is null
      ...
      and case when in_client_order_ids = '{}' then true else cl.client_order_id = any (in_client_order_ids) end
      and cl.trans_type <> 'F';
...
-- Account Details
               ...
               sender_type::varchar                                          as "Sender Type",
...
select *
from trash.report_obo_compliance_xls_with_clordid(in_date_begin_id := 20260106, in_date_end_id := 20260106, in_include_routes := 'N',
