...
where is_active
and fix_comp_id ilike '%TFNFSCSLO1%';

select fix_connection_id from dwh.client_order
where create_date_id != 20260428
...
CREATE OR REPLACE FUNCTION dash360.report_gtc_fidelity_retail(p_is_multileg_report character varying DEFAULT 'N'::character varying,
                                                              ...
                                                              in_fix_comp_ids character varying[] default '{}'::character varying[])
    RETURNS TABLE
            (
                export_row text
            )
...
AS
$function$
    -- 2024-12-09 OS added (78, 71) instead of 78 for sub_strategy (Slavko Dmytriv)
    -- 2026-04-28 OS https://dashfinancial.atlassian.net/browse/DEVREQ-7707 add the in_fix_comp_ids parameter
declare
    l_row_cnt                 integer;
...
--       and gtc.create_date_id >= :l_gtc_date_id
      ...
      and case
              when coalesce(in_fix_comp_ids, '{}') = '{}' then true
              else cl.fix_connection_id in
                   (select fix_connection_id from dwh.d_fix_connection where fix_comp_id = any (in_fix_comp_ids)) end
--   and cl.client_order_id in ('JZ/6232/X42/799840/24005HAEZH ')
    ;
    GET DIAGNOSTICS l_row_cnt = ROW_COUNT;
    --RAISE INFO 'Found: % rows', l_row_cnt;
    select public.load_log(l_load_id, l_step_id, 'insert into tmp_gtc_fidelity_retail_ord_open ',
    ...
    then
        ...
        from (select 'DASO'                                                          as daso_mm_firm
                   ...
                   , lpad(coalesce((oc.strike_price * 1000000000)::numeric(24, 0), 0)::varchar, 18,
                          '0')                                                       as daso_strike_price
                   , coalesce(upper(ot.order_type_short_name), '   ')                as daso_mkt_lmt
                   , lpad(coalesce((abs(co.price) * 10000000)::numeric(24, 0), 0)::varchar, 11,
                          '0')                                                       as daso_lmt_price
                   , lpad(coalesce((abs(co.stop_price) * 10000000)::numeric(24, 0), 0)::varchar, 11,
                          '0')                                                       as daso_stp_price
                ...
                --when coalesce(dwh.get_cum_qty_from_orig_orders(in_order_id => co.order_id, in_date_id => co.create_date_id), 0) >= co.order_qty then 1 -- order is filled -- too long
                         ...
                         when ex.max_order_status in ('2', '4', '5', '8')
                             then 1 -- order status: filled, canceled, replaced, rejected
                --when coalesce(di.last_trade_date, current_timestamp) <= current_timestamp  then 1 -- Expiration date of instrument is exceeded
                         when to_char(coalesce(di.last_trade_date, current_timestamp), 'YYYYMMDD')::integer <
                              in_start_date_id then 1 -- Expiration date of instrument is exceeded
                         else 0
        ...
        select s.dasm_mm_firm
                   ...
                   as roe
        -- select s.* --s.trading_firm_id, s.gts_is_closed, count(1) as cnt
        from (select 'DASM'                                                          as dasm_mm_firm
                   ...
                   , '2'                                                             as dasm_d_rec_id
                   , case
                         when close_date_id is null then 0
              ...
              from tmp_gtc_fidelity_retail_ord_open co
                  ...
                  where cl.order_id = co.order_id
                    and cl.sub_strategy_id in (71, 78)   --in (select dts.target_strategy_id from dwh.d_target_strategy dts where dts.target_strategy_name = 'RETAIL') -- only 9000 tag = RETAIL
                    and cl.time_in_force_id = '1'        -- in ('1','C','M') -- GTC hardcode - no GTC found for , but are found for
              ...
              where true
                ...
                and case
                        when (di.symbol in ('VIX', 'SPX') and
                              di.last_trade_date = in_start_date_id::text::date + interval '1 day') then false
                        else true end) s
        where true
--      and s.gts_is_closed = 0
        ;
    ...
    GET DIAGNOSTICS l_row_cnt = ROW_COUNT;
    --RAISE INFO 'Found: % rows', l_row_cnt;
    select public.load_log(l_load_id, l_step_id, 'insert into tmp_gtc_fidelity_retail_roe ', coalesce(l_row_cnt, 0),
                           'O')
    into l_step_id;
    ...
    then
        -- Report Name: Fidelity Single-leg Open Order Report
        RETURN QUERY
            select rpad('DASO'::varchar, 4, ' ') || -- DASO-FILE-ID
                   rpad(to_char(to_date(in_start_date_id::varchar, 'YYYYMMDD'), 'YYMMDD'), 6,
                        ' ') || -- DASO-FILE-DATE  l_current_date_id - Current business date
                   rpad(' '::varchar, 151, ' ') || -- FILLER
                   rpad('1'::varchar, 1, ' ') -- DASO-H-REC-ID
            union all
            select t.roe
            from tmp_gtc_fidelity_retail_roe t
            union all
            select lpad(l_row_cnt::varchar, 6, '0') || -- DASO-REC-COUNT
        ...
        -- Report Name: Fidelity Multi-leg Open Order Report
        RETURN QUERY
            select rpad('DASM'::varchar, 4, ' ') || -- DASM-FILE-ID
                   rpad(to_char(to_date(in_start_date_id::varchar, 'YYYYMMDD'), 'YYMMDD'), 6,
                        ' ') || -- DASM-FILE-DATE  l_current_date_id - Current business date
            ...
            from tmp_gtc_fidelity_retail_roe t
            union all
            select lpad(l_row_cnt::varchar, 6, '0') || -- DASM-REC-COUNT
