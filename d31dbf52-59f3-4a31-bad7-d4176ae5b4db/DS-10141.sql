...
CREATE OR REPLACE FUNCTION trash.report_obo_compliance_new(in_date_begin_id integer, in_date_end_id integer,
    ...
    RETURNS TABLE
            (
                ...
                "Event Type"                character varying, -- 5
                "Event Date"                text,
                "Event Time"                text,
...
select array_agg(distinct account_id) from dwh.client_order
where order_id in (19158555678,19159621489,19158555679,19159621495,19158555680,19159621501,19161719278,19161719279,19161719282,19163253234,19163264539,19168490738,19176656452,19176967344,19176978122,19176979238,19179142317,19179142678,19179143605,19179143648,19179143792,19191747171,19197256857,19191747172,19197256860,19191747174,19197256862,19197561973,19218836009,19218836011,19218836014,19222801424,19238029924,19245808594)



select *
from dash360.report_obo_compliance_xls(20250218, 20250221, null, '{58549,64894,68334,71776,71797,71827,71852,71871,72082,72991,73089}',
                                       '{19158555678,19159621489,19158555679,19159621495,19158555680,19159621501,19161719278,19161719279,19161719282,19163253234,19163264539,19168490738,19176656452,19176967344,19176978122,19176979238,19179142317,19179142678,19179143605,19179143648,19179143792,19191747171,19197256857,19191747172,19197256860,19191747174,19197256862,19197561973,19218836009,19218836011,19218836014,19222801424,19238029924,19245808594}');


select cl.order_id, cl.exec_instruction, fmj.fix_message ->> '18' as t18, fmj.fix_message ->> '9291' as t9291
  from client_order cl
  join lateral(select fix_message from fix_capture.fix_message_json fmj where fmj.fix_message_id = cl.fix_message_id and fmj.date_id = cl.create_date_id limit 1) fmj on true
where cl.create_date_id = 20250218
--   and cl.order_id in (19168306278, 19168315685, 19168315692, 19168326399)
and fmj.fix_message ->> '9291' is not null;


select cl.order_id, cl.exec_instruction, fmj.fix_message ->> '18' as t18, fmj.fix_message ->> '9291' as t9291
from client_order cl
         join lateral (select fix_message
                       from fix_capture.fix_message_json fmj
                       where fmj.fix_message_id = cl.fix_message_id
                         and fmj.date_id = cl.create_date_id
                       limit 1) fmj on true
where cl.create_date_id = 20250805
--   and cl.order_id in (19168306278, 19168315685, 19168315692, 19168326399)
  and fmj.fix_message ->> '9291' is not null;