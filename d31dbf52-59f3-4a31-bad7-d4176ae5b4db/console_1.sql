...
--       rt.is_active,
      ...
      end ::character varying routing_table_type,
...
from staging.routing_table rt
    left join (select
               case when :p_mode = 'A' then co.account_id else null end account_id,
           ...
           left join (select ftr.order_id, sum(ftr.last_qty) exec_qty, max(ftr.trade_record_time) trade_record_time, sum(ftr.principal_amount) principal_amount
                      from dwh.flat_trade_record ftr
                      where ftr.date_id between :p_start_status_date_id and :p_end_status_date_id and ftr.is_busted='N' and order_id > 0
