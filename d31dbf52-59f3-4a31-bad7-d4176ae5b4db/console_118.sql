...
CREATE FUNCTION trash.report_rps_s3(in_start_date_id integer, in_end_date_id integer, in_account_ids integer[] DEFAULT '{}'::integer[], in_is_multi_leg character DEFAULT 'N'::bpchar, in_trading_firm_ids character varying[] DEFAULT '{}'::character varying[])
...
begin
    ...
    if coalesce(in_account_ids, '{}') = '{}' and coalesce(in_trading_firm_ids, '{}') = '{}' then
        ...
        where true
          and case
                  when coalesce(in_trading_firm_ids, '{}') <> '{}'::varchar[] then trading_firm_id = ANY (in_trading_firm_ids)
                  else true end
    ...
    select public.load_log(l_load_id, l_step_id,
                           'report_rps_s3 for ' || in_start_date_id::text || ' - '||in_end_date_id::text || substr(l_account_ids::text, 1, 50) ||' STARTED ===',
                           0, 'O')
    into l_step_id;
...
--                                                'DAIN' || '|' ||
                                               (select coalesce(cat_imid, '')
                                                ...
                                                where true
                                                  and case when l_account_ids = '{}' then true else account_id = any(l_account_ids) end
...
select *
from dash360.report_rps_s3(in_start_date_id := 20250218, in_end_date_id := 20250218, in_is_multi_leg := 'Y',
                           in_trading_firm_ids := '{"OFP0016"}');
...
where trading_firm_name ilike '%OFP - ETrade Financial%';

select substr(:l_account_ids::text, 1, 50)