...
CREATE PROCEDURE dash_reporting.imc_report_making_commit(IN in_date_id integer DEFAULT (to_char((CURRENT_DATE)::timestamp with time zone, 'YYYYMMDD'::text))::integer)
    LANGUAGE plpgsql
AS
$procedure$
...
--                                   and cnl.create_date_id = in_date_id --??
    ...
    begin
        insert into dash_reporting.imc_base_ext_md
        select *
        from dash_reporting.imc_base_ext cl
                 left join lateral (select
                                        -- AMEX
