select *
from trash.report_risk_baml_peak_consumption_file(
        in_start_date_id := 20241001,
        in_end_date_id := 20241031,
        in_trading_firm_ids := array ['akuna','caerusats','davinciat','gtsats','hullats','imcats','plaxats','peak6','scalpats','simplex01','sigats','virtuats','walltaats','walleye01','webbats','wexats','imc01','imc02'],
        in_is_include_rejected_order := 'Y',
        in_is_include_10503_10502_tags := 'Y'
     );


-- DROP FUNCTION dash360.report_risk_baml_peak_consumption_file(int4, int4, _int8, _varchar, varchar, varchar);

CREATE FUNCTION trash.report_risk_baml_peak_consumption_file(in_start_date_id integer DEFAULT NULL::integer,
...
begin
  ...
  l_step_id:=1;
   ...
   select public.load_log(l_load_id, l_step_id, 'dash360.report_risk_baml_peak_consumption_file STARTED===', 0, 'O')
    ...
    l_is_include_10503_10502_tags := case when left(in_is_include_10503_10502_tags, 1) ilike '%Y%' then 'Y' else 'N' end;
    ...
    select public.load_log(l_load_id, l_step_id, left('in_account_ids = '|| in_account_ids::varchar, 200), 0, 'O')
      into l_step_id;
    select public.load_log(l_load_id, l_step_id, left('in_trading_firm_ids = '|| in_trading_firm_ids::varchar, 200), 0, 'O')
      into l_step_id;
    select public.load_log(l_load_id, l_step_id, left('in_is_include_rejected_order = '|| in_is_include_rejected_order::varchar||' , in_is_include_10503_10502_tags = '|| in_is_include_10503_10502_tags::varchar, 200), 0, 'O')
      into l_step_id;
    select public.load_log(l_load_id, l_step_id, left('l_is_include_rejected_order = '|| l_is_include_rejected_order::varchar||' , l_is_include_10503_10502_tags = '|| l_is_include_10503_10502_tags::varchar, 200), 0, 'O')
      into l_step_id;
...
select * from tmp_risk_peak_conumption;
    ...
    select ac.account_name as tag_1
      ...
      --, rlv.osr_param_name as rl_parameter
      , case when rlv.risk_mgmt_config_scope = 'H' then rlv.osr_param_name||'(EOS)'  else rlv.osr_param_name end as rl_parameter
      ...
      --
      , gs.val::int4 as peak_id
      , tf.trading_firm_name , tf.trading_firm_id, ac.account_id, ac.account_name
    ...
    from staging.risk_limits_osr_param_v rlv
      left join dwh.d_account ac
        on rlv.account_id = ac.account_id
      left join dwh.d_trading_firm tf
        on (tf.trading_firm_unq_id = ac.trading_firm_unq_id or (rlv.trading_firm_id = tf.trading_firm_id and tf.is_active = true))
      cross join generate_series(1,5) gs(val)
    where rlv.osr_param_code in (/*'CRMNVE',*/ 'CRMXNV', 'EMAXON', 'EMAXOS', 'OMAXNV', 'OMAXOC', 'DTNOPT', 'DTNEQT', 'CRTNOP')
      and case when :in_account_ids<>'{}' then ac.account_id = ANY(:in_account_ids) else true end
      and case when :in_trading_firm_ids<>'{}' then tf.trading_firm_id = ANY(:in_trading_firm_ids) else true end
      and case when :in_account_ids='{}' and :in_trading_firm_ids='{}' then rlv.entity in ('BLMDO161','BLMDOSS161') else true end
      --and rlv.entity in ('BLMDO161') --,'BLMDOSS161')
      and ac.account_name is not null;
--     order by 1, 3, 2, 8
    ;
        ...
        (
          select account_id
            ...
            , case
                ...
                when osr_param_code = 'CRMXNV_ML' and crmxnv_ml > 0 and rn_crmxnv_ml = peak_num then client_order_id
                when osr_param_code = 'EMAXON' and emaxon > 0 and rn_emaxon = peak_num          then client_order_id
                when osr_param_code = 'EMAXON_ML' and emaxon_ml > 0 and rn_emaxon_ml = peak_num then client_order_id
                when osr_param_code = 'EMAXOS' and emaxos > 0 and rn_emaxos = peak_num          then client_order_id
                when osr_param_code = 'EMAXOS_ML' and emaxos_ml > 0 and rn_emaxos_ml = peak_num then client_order_id
                when osr_param_code = 'OMAXNV' and omaxnv > 0 and rn_omaxnv = peak_num          then client_order_id
                when osr_param_code = 'OMAXNV_ML' and omaxnv_ml > 0 and rn_omaxnv_ml = peak_num then client_order_id
                when osr_param_code = 'OMAXOC' and omaxoc > 0 and rn_omaxoc = peak_num          then client_order_id
                when osr_param_code = 'OMAXOC_ML' and omaxoc_ml > 0 and rn_omaxoc_ml = peak_num then client_order_id
              end as peak_cl_ord_id
          from
            ( select op.osr_param_code, pn.peak_num
              from (select unnest(array['CRMXOC', 'CRMXCO', 'CRMNVE', /*'CRMNVE_ML',*/ 'CRMXNV', 'CRMXNV_ML', 'EMAXON', 'EMAXON_ML', 'EMAXOS', 'EMAXOS_ML', 'OMAXNV', 'OMAXNV_ML', 'OMAXOC', 'OMAXOC_ML']) as osr_param_code) op
                cross join
                   (select unnest(array[1,2,3,4,5]) as peak_num) pn
            ) as opc
            cross join
    --  select * from
            (
              select account_id
                ...
                , crmxnv_ml
                , row_number() over (partition by account_id order by crmxnv_ml desc nulls last) as rn_crmxnv_ml
                , emaxon
                , row_number() over (partition by account_id order by emaxon desc nulls last) as rn_emaxon
                , emaxon_ml
                , row_number() over (partition by account_id order by emaxon_ml desc nulls last) as rn_emaxon_ml
                , emaxos
                , row_number() over (partition by account_id order by emaxos desc nulls last) as rn_emaxos
                , emaxos_ml
                , row_number() over (partition by account_id order by emaxos_ml desc nulls last) as rn_emaxos_ml
                , omaxnv
                , row_number() over (partition by account_id order by omaxnv desc nulls last) as rn_omaxnv
                , omaxnv_ml
                , row_number() over (partition by account_id order by omaxnv_ml desc nulls last) as rn_omaxnv_ml
                , omaxoc
                , row_number() over (partition by account_id order by omaxoc desc nulls last) as rn_omaxoc
                , omaxoc_ml
                , row_number() over (partition by account_id order by omaxoc_ml desc nulls last) as rn_omaxoc_ml
              from
                (
                  ...
                  --, s.side, s.order_price
                    , case
                        when s.instrument_type_id = 'O' and coalesce(s.is_cross, false) = true and s.multileg_reporting_type = '1' -- single options
                          then s.order_qty
                      end as CRMXOC --CrossMaxOrderContracts -- OPT
                    , case
                        when s.instrument_type_id = 'O' and coalesce(s.is_cross, false) = true and s.multileg_reporting_type = '2' -- qty the same as mlrt=3
                          then s.order_qty
                      end as CRMXCO --CrossMaxOrderContracts -- MLEG
                    , case
                        when s.instrument_type_id = 'E' and coalesce(s.is_cross, false) = true and s.multileg_reporting_type = '2' -- multileg, cross eqyity legs. single cross equity legs are not present.
                    ...
                    , case
                        when s.instrument_type_id in ('O') and coalesce(s.is_cross, false) = true and s.multileg_reporting_type = '2' -- multileg, but what should be here, eq or opt legs? OPT
                          then s.cross_legs_order_notional
                      end as CRMXNV_ML --CrossMaxOrderNotional -- MLEG OPT
                    , case
                        when s.instrument_type_id = 'E' and coalesce(s.is_cross, false) = false and s.multileg_reporting_type = '1' --
                          then s.fix_order_notional --s.order_notional
                      end as EMAXON --EquityMaxOrderNotional -- EQ
                    , case
                        when s.instrument_type_id = 'E' and coalesce(s.is_cross, false) = false and s.multileg_reporting_type = '2' --
                          then s.fix_order_notional --s.order_notional
                    ...
                    , case
                        when s.instrument_type_id = 'O' and coalesce(s.is_cross, false) = false and s.multileg_reporting_type = '1' --
                          then s.fix_order_notional --s.order_notional
                      end as OMAXNV --OptionMaxOrderNotional -- OPT
                    , case
                        when s.instrument_type_id = 'O' and coalesce(s.is_cross, false) = false and s.multileg_reporting_type = '2' --
                          then s.fix_order_notional --s.order_notional
                  ...
                  from
                    (
                      select
                          src.account_id, src.client_order_id
                        , src.create_date_id
...
--                           EXPLAIN (ANALYZE, COSTS, VERBOSE, BUFFERS, FORMAT JSON)
                          select fyc.account_id , fyc.order_id, fyc.client_order_id -- fyc.order_id will be removed to group by cl_ord_id
                            ...
                            , fyc.order_qty
                            , case when fyc.instrument_type_id = 'E'
                                    then abs(fyc.order_qty *
                                          coalesce((case --when fyc.order_type_id <> '1' then fyc.order_price
                                                       ...
                                                       when fyc.side not in ('1','3') then fyc.nbbo_bid_price  -- fyc.order_type_id = '1' and
                                                     end), 0))
                                   when fyc.instrument_type_id = 'O'
                                    then fyc.order_qty * os.contract_multiplier *
                                          coalesce((case --when fyc.order_type_id <> '1' then fyc.order_price
                                                       when fyc.side in ('1','3') then abs(fyc.nbbo_ask_price) -- fyc.order_type_id = '1' and
                                                       when fyc.side not in ('1','3') then -abs(fyc.nbbo_bid_price) -- fyc.order_type_id = '1' and -- sell will summarizing as minus
                                                     end), 0)
                          ...
                          from --data_marts.f_yield_capture fyc
                            ( select distinct account_id
                              ...
                              --from trash.sdn_tmp_risk_peak_conumption --
                            ) ac
                            inner join lateral
                              (
                                select fyc.account_id , fyc.order_id, fyc.client_order_id
                                  , fyc.routed_time
                                  , fyc.multileg_reporting_type , fyc.instrument_id , fyc.order_fix_message_id, fyc.status_date_id
                                  , fyc.instrument_type_id, fyc.cross_order_id
                                  , fyc.order_qty
                                  , fyc.side, fyc.nbbo_ask_price, fyc.nbbo_bid_price
                                  , case when fyc.parent_order_id is not null then 1 else 0 end as is_prnt
                                from data_marts.f_yield_capture fyc
                                where 1=1
                                  and fyc.account_id = ac.account_id
                                ...
                                limit 1000000000
                              ) fyc on true and fyc.is_prnt = 0 --!!! avoid unnecessary index(parent_order_id) usage
                          ...
                          where 1=1
                            ...
                            --and fyc.status_date_id between :l_start_date_id and :l_end_date_id -- 20210701 and 20210731 --
                            --and to_char(fyc.routed_time, 'YYYYMMDD')::varchar = fyc.status_date_id::varchar
                            --and fyc.account_id in (select distinct account_id from tmp_risk_peak_conumption) -- (24649, 25239) --trash.sdn_tmp_risk_peak_conumption
    ...
    select public.load_log(l_load_id, l_step_id, 'max_order_notional and max_order_shares/contracts parameters are loaded', l_row_cnt, 'I')
       into l_step_id;
   ...
   update tmp_risk_peak_conumption trg
    ...
    from
      (
        select account_id, osr_param_code, peak_id, peak_value, peak_date, peak_cl_ord_id
        --from trash.sdn_tmp_risk_peak_max_order_notshar_contr
        from tmp_risk_peak_max_order_notshar_contr
      ) src
    ...
    select public.load_log(l_load_id, l_step_id, 'max_order_notional and max_order_shares/contracts parameters are updated to DM', l_row_cnt, 'U')
       into l_step_id;
    ...
    create temp table tmp_risk_peak_total_notionals with (parallel_workers = 8) ON COMMIT drop as
    select
        src.account_id
      , src.osr_param_code
      , src.peak_num as peak_id
      ...
      (
        select account_id
          ...
          , case
              ...
              when osr_param_code = 'DTNEQT_ML' and dtneqt_ml is not null and rn_dtneqt_ml = peak_num then dtneqt_ml
              ...
              when osr_param_code = 'DTNOPT_ML' and dtnopt_ml is not null and rn_dtnopt_ml = peak_num then dtnopt_ml
              when osr_param_code = 'CRTNOP_ML' and crtnop_ml is not null and rn_crtnop_ml = peak_num then crtnop_ml
            end as peak_value
          -- peak date
          , case
              ...
              when osr_param_code = 'DTNEQT_ML' and dtneqt_ml is not null and rn_dtneqt_ml = peak_num then date_id
              ...
              when osr_param_code = 'DTNOPT_ML' and dtnopt_ml is not null and rn_dtnopt_ml = peak_num then date_id
              when osr_param_code = 'CRTNOP_ML' and crtnop_ml is not null and rn_crtnop_ml = peak_num then date_id
            end as peak_date
        from
          ( select op.osr_param_code, pn.peak_num
          ...
          (
            ...
            from
              (
                select s.account_id
                  ...
                  , dtneqt_ml
                  , row_number() over (partition by account_id order by dtneqt_ml desc nulls last) as rn_dtneqt_ml
                  , dtnopt
                  ...
                  , dtnopt_ml
                  , row_number() over (partition by account_id order by dtnopt_ml desc nulls last) as rn_dtnopt_ml
                  , crtnop_ml
                  , row_number() over (partition by account_id order by crtnop_ml desc nulls last) as rn_crtnop_ml
                from
                  (
                    select src.account_id
                      , src.date_id
                    ...
                    from
                      (
                        select s.account_id
                          ...
                          , case
                              when s.instrument_type_id = 'E' and s.is_cross = true and s.multileg_reporting_type = '1'
                                then --s.principal_amount
                                  case when s.is_cross = true
                                         then s.principal_amount
                                       when s.is_10504_10505_exists = true -- If tags are available
                          ...
                          , case
                              when s.instrument_type_id = 'E' and s.is_cross = false and s.multileg_reporting_type = '1'
                                then --s.principal_amount
                                  case when s.is_cross = true
                                         then s.principal_amount
                                       when s.is_10504_10505_exists = true -- If tags are available
                          ...
                          , case
                              when s.instrument_type_id = 'O' and s.is_cross = true and s.multileg_reporting_type = '1'
                                then --s.principal_amount
                                  case when s.is_cross = true
                                         then s.principal_amount
                                       when s.is_10504_10505_exists = true -- If tags are available
                          ...
                          , case
                              when s.instrument_type_id = 'O' and s.is_cross = false and s.multileg_reporting_type = '1'
                                then --s.principal_amount
                                  case when s.is_cross = true
                                         then s.principal_amount
                                       when s.is_10504_10505_exists = true -- If tags are available
                          ...
                          , case
                              when s.instrument_type_id = 'O' and s.is_cross = true and s.multileg_reporting_type = '2'
                                then --s.principal_amount
                                  case when s.is_cross = true
                                         then s.principal_amount
                                       when s.is_10504_10505_exists = true -- If tags are available
                        ...
                        from
                          (
                                    ...
                                    from
                                      (
                                        ...
                                        where tr.date_id between :l_start_date_id and :l_end_date_id -- = 20211207 --   20211201 and 20211231 --
                                          and tr.account_id in (select distinct account_id from tmp_risk_peak_conumption) --  --(25239, 24649 /*, 27630*/)
                            ...
                            union all -- include orders with status NEW
                                      ...
                                      (
                                       select co.account_id
                                        ...
                                        where co.create_date_id between :l_start_date_id and :l_end_date_id -- 20211201 and 20211231 -- = 20211207 --
                                          and co.account_id in (select distinct account_id from tmp_risk_peak_conumption) -- in (24649, 25239, 29222, 24650,30214) --
            ...
            where ( dtneqt is not null and rn_dtneqt <= 5)
               ...
               or ( crtnop_ml is not null and rn_crtnop_ml <= 5)
          ) fl
      ) src
    group by src.account_id
      , src.osr_param_code
   ...
   update tmp_risk_peak_conumption trg
    ...
    from
      (
        select account_id, osr_param_code, peak_id, peak_value, peak_date
        --from trash.sdn_tmp_risk_peak_total_notionals
        from tmp_risk_peak_total_notionals
      ) src
    where trg.account_id = src.account_id
      and trg.osr_param_code = src.osr_param_code
      and trg.peak_id = src.peak_id
  ...
  then
   ...
   -- step 5: load total_notionals (10502 and 10503 values from latest order of the day) parameters
    ...
    create temp table tmp_risk_10502_10503_total_notionals with (parallel_workers = 8) ON COMMIT drop as
    ...
    from
      (
        select co.account_id, co.create_date_id, co.create_time, co.process_time, co.order_id, co.multileg_reporting_type --, di.instrument_type_id--, co.fix_message_id
          , co.client_order_id
          , fx.fix_message_id, fx.tag_10502_equity_total_day_notional, fx.tag_10503_option_total_day_notional, fx.tag_10504_equity_order_notional, fx.tag_10505_option_order_notional
          , row_number() over (partition by co.account_id, co.create_date_id order by co.create_time desc, co.order_id desc) rn
          , rj.*
        from dwh.client_order co
          --left join dwh.d_instrument di
          --  on co.instrument_id = di.instrument_id
  ...
  then
   ...
   RETURN QUERY
     ...
     from
       (
         select
            coalesce(t.tag_1::varchar, '')::varchar                     ||','|| -- Tag 1
            ...
            coalesce(t.tag_10502_equity_total_day_notional::varchar, '')::varchar          ||','|| -- Equity Total Notional
            coalesce(t.tag_10503_option_total_day_notional::varchar, '')::varchar                  -- Option Total Notional
            as roe
         from
           (
              select t.tag_1, t.security_type, t.rl_parameter, t.current_limit, t.peak_value
              , to_char(to_date(t.peak_date::varchar,'YYYYMMDD'), 'MM/DD/YYYY') as peak_date
              , t.cl_ord_id, t.peak_id
              , tn.tag_10502_equity_total_day_notional, tn.tag_10503_option_total_day_notional
              from --trash.sdn_tmp_risk_peak_conumption t
                ...
                left join tmp_risk_10502_10503_total_notionals tn
                  ...
                  and t.rl_parameter ilike '%TotalNotional%' -- reverted to print for all TotalNotionals
                  --and t.rl_parameter in ('EquityTotalNotional','OptionTotalNotional')
              --where t.rl_parameter ilike '%TotalNotional%'
           ) t
         order by t.tag_1, t.rl_parameter, t.security_type, t.peak_id
       ) s
    ;
 ...
 exception when others then
    select public.load_log(l_load_id, l_step_id, 'dash360.report_risk_baml_peak_consumption_file ERROR=====', 0, 'E')
     into l_step_id;
