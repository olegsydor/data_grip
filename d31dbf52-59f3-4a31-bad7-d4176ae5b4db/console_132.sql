-- DROP FUNCTION dash360.report_eod_alpaca_options_retail2(int4, int4);
...
begin
    ...
    return query
        select 'Account Name,Trade Date,Timestamp,Symbol,Buy/Sell,Quantity,Price,Rate Category,Rate,Commission,Order ID,Exec ID,Venue,Exchange Fees';
    return query
        select array_to_string(ARRAY [
                                   ac.account_name,
                                   to_char(tr.order_process_time, 'YYYY-MM-DD'), -- as Trade date
...
CREATE OR REPLACE FUNCTION dash360.report_eod_alpaca_algo_route(in_start_date_id integer, in_end_date_id integer, in_trading_firm_ids varchar[] default '{}'::varchar[], in_account_ids int4[] default '{}'::int4[])
 RETURNS TABLE(ret_row text)
 LANGUAGE plpgsql
AS $function$
-- 20240926 SO https://dashfinancial.atlassian.net/browse/DEVREQ-4936 added Account Name
-- 20241107 KK https://dashfinancial.atlassian.net/browse/DEVREQ-5065 Updated Account Name
-- 20250415 OS https://dashfinancial.atlassian.net/browse/DEVREQ-5905 added 2 new input parameters
declare
    l_load_id     int;
    l_row_cnt     int;
