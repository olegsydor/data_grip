...
where account_name ilike '%T3%';


select string_agg(account_id::text, ',')
    from dwh.d_account
    where trading_firm_id in ('OFP0058', 't3trade01')
    and (account_name in ('123456','234567','KTTNUNN','T3_BTIG_DESK','T3_OPCO_DESK') or
        account_name like 'T3GSX%' or
        account_name like 'T3T3G%' or
