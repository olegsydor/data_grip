...
begin
    ...
    select cl.transaction_id,
           ...
           case ascii(cl.counterparty_range)
               when 0 then repeat(' ', 1)
               else cof.customer_or_firm_id end                                                          as counterparty_range,
...
--CrossOrderID,AuctionType,RequestCount,BillingType,ContraBroker,ContraTrader,WhiteList,PaymentPerContract,ContraCrossExecutedQty
               coalesce(cl.cross_order_id::text, '') || ',' ||
    --CrossOrderID
...
--                   when CL.STRATEGY_DECISION_REASON_CODE in ('32', '62', '96', '99') then 'FLASH'
        ...
        coalesce(staging.trailing_dot(cl.cons_payment_per_contract), '') || ',' || coalesce(cl.contra_cross_exec_qty::text, '') || ',' || coalesce(cl.contra_cross_lp_id, '') || ',' || coalesce(cl.ac_account_demo_mnemonic, '') as rec from dash_reporting.imc_final cl left join lateral (select customer_or_firm_id from t_cust_or_firm cof where cof.exchange_id = cl.ex_exchange_id and cof.exch_customer_or_firm_id = cl.counterparty_range order by customer_or_firm_id limit 1) cof on cl.counterparty_range is not
    null left join lateral (select customer_or_firm_id
