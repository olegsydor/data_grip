...
CREATE OR REPLACE FUNCTION dash360.report_equity_tca_init_v2(in_date_begin integer, in_date_end integer, in_trading_firm_ids text[] DEFAULT '{}'::text[], in_client_ids text[] DEFAULT '{}'::text[], in_account_ids bigint[] DEFAULT '{}'::bigint[], in_sub_strategy character varying[] DEFAULT '{}'::character varying[], in_custom_configuration_algo boolean DEFAULT false, in_symbols character varying[] DEFAULT '{}'::character varying[])
 RETURNS integer
 LANGUAGE plpgsql
AS $function$
...
--	truncate  table  pre_pre_fetch_equity_tca;
--analyze pre_pre_fetch_equity_tca;
--	insert  into  pre_pre_fetch_equity_tca	
--	select  
--		yc.order_id,
--		yc.client_order_id,
--		yc.status_date_id  as  date_id,
...
--		ts.target_strategy_desc  as  algorithm,
--		yc.nbbo_bid_price  as  parent_nbbo_bid_price,
--		yc.nbbo_ask_price  as  parent_nbbo_ask_price,  
--		yc.nbbo_bid_quantity  as  parent_nbbo_bid_qty,
--		yc.nbbo_ask_quantity  as  parent_nbbo_ask_qty,
...
--		nullif(tca.eligible_pwp_5pc,  0)::float,
--		nullif(tca.eligible_pwp_10pc,  0)::float,
--		nullif(tca.eligible_pwp_15pc,  0)::float,
--		nullif(tca.eligible_pwp_20pc,  0)::float,
--		tca.trade_count,
...
--			when  'POV'  then  js.fix_message  ->>  '9023'  --target_pov
--			when  'VOLUME  PARTICIPATION'  then  js.fix_message  ->>  '9023'--target_pov
--			when  'PHANTOM'  then  js.fix_message  ->>  '9002'  --urgency
--			when  'CLOSE'  then  js.fix_message  ->>  '9126'  --close_aggression
--			when  'SENSOR  DARK'  then  'Default  (PI  =  '  ||  (js.fix_message  ->>  '9191')::text  ||  ')'
--			else  null
...
--		yc.order_qty
--		from  data_marts.f_yield_capture  yc
--			left  join  dwh.d_client  c  on  c.client_unq_id  =  yc.client_id
--			join  dwh.d_account  a  on  a.account_id  =  yc.account_id  and  a.is_active  
--			join  dwh.d_trading_firm  tf  on  tf.trading_firm_unq_id  =  yc.trading_firm_unq_id  and  tf.is_active  
...
--			left  join  eq_tca.algorithmic_order_analytic_v2  tca  on  tca.order_id  =  yc.order_id
--			left  join  eq_tca.daily_analytic_v2  da  on  da.symbol  =  i.activ_symbol  and  da.date_id  =  yc.status_date_id
--			left  join  lateral  (select  da.close_price  as  close_price  from  eq_tca.daily_analytic_v2  da  where  da.symbol  =  i.activ_symbol  and  da.date_id  <  yc.status_date_id  order  by  date_id  desc  limit  1)  prev  on  true
--			left  join  lateral  (select  da.close_price  as  close_price  from  eq_tca.daily_analytic_v2  da  where  da.symbol  =  i.activ_symbol  and  da.date_id  >  yc.status_date_id  order  by  date_id  limit  1)  nxt  on  true
--			left  join  fix_capture.fix_message_json  js  on  js.fix_message_id  =  yc.order_fix_message_id  and  js.date_id  =  yc.status_date_id  and  js.date_id  between  in_date_begin  and  in_date_end
--			join  dwh.client_order  co  on  co.order_id  =  yc.order_id  and  co.create_date_id  =  yc.status_date_id  and  co.create_date_id  between  in_date_begin  and  in_date_end
--		where  yc.parent_order_id  is  null
--			and  yc.status_date_id  between  in_date_begin  and  in_date_end
--			and  yc.instrument_type_id  =  'E'
--			and  yc.multileg_reporting_type  =  '1'
--			and  case  when  in_account_ids  <>  '{}'  then  a.account_id  =  any(in_account_ids)  else  true  end
--			and  case  when  in_trading_firm_ids  <>  '{}'  then  a.trading_firm_id  =  any(in_trading_firm_ids)  else  true  end
--			and  case  when  in_client_ids  <>  '{}'  then  c.client_id  =  any(in_client_ids)  else  true  end;
--
/*
	insert  into  pre_pre_fetch_equity_tca	(order_id, client_order_id, date_id, multileg_reporting_type, instrument_type_id, trading_firm_unq_id, trading_firm_name, account_id, account_name, client_id, instrument_id, symbol, parent_routed_time, order_end_time, order_cancel_time, side, buy_or_sell, side_multiplier, parent_order_qty, parent_exec_qty, total_parent_exec_qty, parent_avg_price, principal_amount, target_strategy_id, algorithm, parent_nbbo_bid_price, parent_nbbo_ask_price, parent_nbbo_bid_qty, parent_nbbo_ask_qty, buy_limit_vs_ask_bps, sell_limit_vs_bid_bps, parent_is_marketable, parent_limit_price, routing_table_name, order_arrival_price, order_end_price, vwap_over_life, eligible_vwap_over_life, twap_over_life, eligible_twap_over_life, volume_over_life, eligible_volume_over_life, eligible_pwp_5pc, eligible_pwp_10pc, eligible_pwp_15pc, eligible_pwp_20pc, trade_count, eligible_trade_count, block_volume, eligible_qd_volume, eligible_ix_volume, avg_spread_over_life, day_high_price, day_low_price, open_px, close_px, prev_close_px, next_close_px, routing_time_mid_price, routing_time_spread, aggression_level, activ_symbol, day_order_qty, order_qty)
	select  
		yc.order_id,
		yc.client_order_id,
		yc.date_id,
...
left  join  dwh.d_routing_table  rt  on  rt.routing_table_id  =  yc.routing_table_id  and  rt.is_active  =  true
left  join  eq_tca.algorithmic_order_analytic_v2  tca  on  tca.order_id  =  yc.order_id  and  tca.date_id  =  yc.date_id  
left  join  lateral (select * from eq_tca.daily_analytic_v2  da  where  da.symbol  =  i.activ_symbol  and  da.date_id  =  yc.date_id and  da.date_id between  in_date_begin and in_date_end limit 100500) da on true
left  join  lateral  (select  da.close_price  as  close_price  from  eq_tca.daily_analytic_v2  da  where  da.symbol  =  i.activ_symbol  and  da.date_id  <  yc.date_id  order  by  date_id  desc  limit  1)  prev  on  true
left  join  lateral  (select  da.close_price  as  close_price  from  eq_tca.daily_analytic_v2  da  where  da.symbol  =  i.activ_symbol  and  da.date_id  >  yc.date_id  order  by  date_id  limit  1)  nxt  on  true
join LATERAL(select * from dwh.client_order  co  where  co.order_id  =  yc.order_id  and  co.create_date_id  =  yc.date_id  and  co.create_date_id  between  in_date_begin and in_date_end limit 100500) co on true
left  join  lateral (select fix_message from fix_capture.fix_message_json  fmj  where  fmj.fix_message_id  =  co.fix_message_id  and  fmj.date_id  =  co.create_date_id and  fmj.date_id  between  in_date_begin and in_date_end limit 1) fmj on true;
	...
	(
		...
		f.last_qty  *  case  when  f.side  =  '1'  then  f.ask_price  -  efq_last_px  else  efq_last_px  -  f.bid_price  end  as  dollars_saved_from_far,
		case  when  f.ask_price  -  f.bid_price  <=  0  then  0  else  f.last_qty  *  f.spread  *  (f.efq_last_px  -  execution_time_mid_price)  /  half_spread  end  efq_dollars,
		...
		from    (
			...
			case  when  ftr.last_px  >  ftr.ask_price  then  ftr.ask_price  when  ftr.last_px  <  ftr.bid_price  then  ftr.bid_price  else  ftr.last_px  end  efq_last_px,
			...
			ftr.last_qty  *  case  when  ftr.side  =  '1'  then  ftr.ask_price  -  (case  when  ftr.last_px  >  ftr.ask_price  then  ftr.ask_price  when  ftr.last_px  <  ftr.bid_price  then  ftr.bid_price  else  ftr.last_px  end)  else  (case  when  ftr.last_px  >  ftr.ask_price  then  ftr.ask_price  when  ftr.last_px  <  ftr.bid_price  then  ftr.bid_price  else  ftr.last_px  end)  -  ftr.bid_price  end  as  dollars_saved_from_far,
			case  when  ftr.ask_price  -  ftr.bid_price  <=  0  then  0  else  ftr.last_qty  *  (ftr.ask_price    -  ftr.bid_price)  *  ((case  when  ftr.last_px  >  ftr.ask_price  then  ftr.ask_price  when  ftr.last_px  <  ftr.bid_price  then  ftr.bid_price  else  ftr.last_px  end)  -  (ftr.ask_price  +  ftr.bid_price)  /  2)  /  (0.5  *  (ftr.ask_price  -  ftr.bid_price))  end  efq_dollars,
		...
		--						
			from  dwh.flat_trade_record  ftr  
				...
				left  join  dwh.d_exchange  real_exch  on  real_exch.exchange_id  =  e.real_exchange_id  and  real_exch.is_active
			...
			where  ftr.is_busted  =  'N'
				...
				and  case  when  coalesce(in_trading_firm_ids,  '{}')  <>  '{}'  then  ftr.trading_firm_id  =  any(in_trading_firm_ids)  else  true  end
				and  case  when  coalesce(in_client_ids,  '{}')  <>  '{}'  then  ftr.client_id  =  any(in_client_ids)  else  true  end
				and  case  when  in_sub_strategy  <>  '{}'  then  ftr.sub_strategy  =  any(in_sub_strategy)  else  true  end
				and  ftr.instrument_type_id  =  'E'
				and  ftr.multileg_reporting_type  =  '1'
			)  f
		)
			select  
			tp.order_id,
			...
			case
				when  (case  when  tp.day_order_qty  <  tp.order_qty  then  tp.day_order_qty  else  tp.order_qty  end)  /  dea.average_volume_22day::float  =  0  then  0
				when  (case  when  tp.day_order_qty  <  tp.order_qty  then  tp.day_order_qty  else  tp.order_qty  end)  /  dea.average_volume_22day::float  between  0.0000001  and  0.01  then  1
				when  (case  when  tp.day_order_qty  <  tp.order_qty  then  tp.day_order_qty  else  tp.order_qty  end)  /  dea.average_volume_22day::float  between  0.0099999  and  0.05  then  2
				when  (case  when  tp.day_order_qty  <  tp.order_qty  then  tp.day_order_qty  else  tp.order_qty  end)  /  dea.average_volume_22day::float  between  0.0499999  and  0.1  then  3
				when  (case  when  tp.day_order_qty  <  tp.order_qty  then  tp.day_order_qty  else  tp.order_qty  end)  /  dea.average_volume_22day::float  between  0.0999999  and  0.25  then  4
				when  (case  when  tp.day_order_qty  <  tp.order_qty  then  tp.day_order_qty  else  tp.order_qty  end)  /  dea.average_volume_22day::float  between  0.2499999  and  0.5  then  5
				when  (case  when  tp.day_order_qty  <  tp.order_qty  then  tp.day_order_qty  else  tp.order_qty  end)  /  dea.average_volume_22day::float  between  0.4999999  and  1  then  6
				else  9::float
			end  as  adv_pct,
			dea.sector,
	...
	--
					...
					sum(coalesce(tcce_maker_taker_fee_amount,  0.0)  +  coalesce(tcce_transaction_fee_amount,  0.0))  as  net_fees_rebates_amount,
	...
	--
					...
					from  ftr
					where  ftr.order_id  =  tp.order_id  and  ftr.date_id  =  tp.date_id
					limit  1
			)  tr  on  1=1
			--dea  join
			left  join  lateral
			(
			select  dea.activ_symbol,  dea.date_id  as  activ_analytic_date,  mc.scale_market_cap,  dea.sector,  dea.scale_market_cap_id,  
			dea.average_volume_22day,  dea."5d_avg_bid_ask_spread",  dea."5d_avg_bid_ask_spread_pct"  *  100  as  "5d_avg_spread_bps"
			...
			where  date_id  between  in_date_begin  and  in_date_end  and  date_id  =  tp.date_id  and  dea.activ_symbol  =  tp.activ_symbol  
			limit  1
	                )  dea  on  1=1;
...
/*	create  temp  table  if  not  exists  pre_fetch_equity_tca_venue  (
		order_id  				int8  ,
		day_order_qty  			int4,
		...
		efq_last_px	  			numeric,
		observed_spread_value	numeric,
		dollars_saved_from_far  	numeric,
		efq_dollars  			numeric,
		midpont_stability		int4,
		net_fees_rebates_amount	numeric,
		clearing_amount			numeric,
		commission_amount		numeric,		
		rn  int8  NULL
	...
	truncate  table  pre_fetch_equity_tca_venue;*/
	...
	select
                ...
                ftr.last_qty  *  (ftr.ask_price    -  ftr.bid_price)  as  observed_spread_value,
                ftr.last_qty  *  case
                                                        when  ftr.side  =  '1'  then  ftr.ask_price  -  (case  when  ftr.last_px  >  ftr.ask_price  then  ftr.ask_price  when  ftr.last_px  <  ftr.bid_price  then  ftr.bid_price  else  ftr.last_px  end)
                                                        else  (case  when  ftr.last_px  >  ftr.ask_price  then  ftr.ask_price  when  ftr.last_px  <  ftr.bid_price  then  ftr.bid_price  else  ftr.last_px  end)  -  ftr.bid_price
                                                end  as  dollars_saved_from_far,
                case
                        when  ftr.ask_price  -  ftr.bid_price  <=  0  then  0
                        else  ftr.last_qty  *  (ftr.ask_price    -  ftr.bid_price)  *  ((case  when  ftr.last_px  >  ftr.ask_price  then  ftr.ask_price  when  ftr.last_px  <  ftr.bid_price  then  ftr.bid_price  else  ftr.last_px  end)  -  (ftr.ask_price  +  ftr.bid_price)  /  2)  /  (0.5  *  (ftr.ask_price  -  ftr.bid_price))
                end  efq_dollars,
	        case
        ...
        from  data_marts.f_yield_capture  yc
                left  join  dwh.flat_trade_record  ftr  on  (ftr.street_order_id  =  yc.order_id  and  ftr.date_id  between  in_date_begin  and  in_date_end)
                ...
                left  join  eq_tca.eq_rev_ft  rev  on  (rev.trade_record_id  =  ftr.trade_record_id  and  num_nulls(p0ms_bid,  p0ms_ask,  p10ms_bid,  p10ms_ask)  =  0  and  rev.date_id  between  in_date_begin  and  in_date_end)
                left  join  dwh.d_target_strategy  dts  on  dts.target_strategy_id  =  yc.sub_strategy_id  
	...
	select  public.load_log(l_load_id,  l_step_id,  'pre_fetch_equity_tca_venue  insertd  in  temp  table  ====',  l_row_cnt,  'I')
	into  l_step_id;
