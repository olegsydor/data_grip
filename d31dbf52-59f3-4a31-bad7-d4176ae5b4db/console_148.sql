...
select yc.order_id,
       ...
       case
           when yc.side = '1' then 10000 * (yc.order_price - yc.nbbo_ask_price) / coalesce(yc.nbbo_ask_price, 0, null)
           else null end                                                                     as buy_limit_vs_ask_bps,
       case
           when yc.side <> '1' then 10000 * (yc.order_price - yc.nbbo_bid_price) / coalesce(yc.nbbo_bid_price, 0, null)
...
where yc.parent_order_id is null
  and yc.status_date_id between 20250601 and 20250930
  and yc.instrument_type_id = 'E'
  and yc.multileg_reporting_type = '1';

analyze trash.tmp_fyc;

drop table if exists trash.pre_pre_fetch_equity_tca;
create temp table if not exists trash.pre_pre_fetch_equity_tca
(
    order_id                  int8           null,
...
insert into trash.pre_pre_fetch_equity_tca (order_id, client_order_id, date_id, multileg_reporting_type, instrument_type_id,
                                      trading_firm_unq_id, trading_firm_name, account_id, account_name, client_id,
                                      instrument_id, symbol, parent_routed_time, order_end_time, order_cancel_time,
                                      side, buy_or_sell, side_multiplier, parent_order_qty, parent_exec_qty,
                                      total_parent_exec_qty, parent_avg_price, principal_amount, target_strategy_id,
                                      algorithm, parent_nbbo_bid_price, parent_nbbo_ask_price, parent_nbbo_bid_qty,
                                      parent_nbbo_ask_qty, buy_limit_vs_ask_bps, sell_limit_vs_bid_bps,
                                      parent_is_marketable, parent_limit_price, routing_table_name, order_arrival_price,
                                      ...
                                      eligible_ix_volume, avg_spread_over_life, day_high_price, day_low_price, open_px,
...
from trash.tmp_fyc yc
         ...
         left join lateral (select *
                            ...
                            where da.symbol = i.activ_symbol
                              ...
                              and da.date_id between 20250601 and 20250930
         ...
         join LATERAL (select *
                       ...
                       where co.order_id = yc.order_id
                         ...
                         and co.create_date_id between 20250601 and 20250930
         ...
         left join lateral (select fix_message
                            ...
                            where fmj.fix_message_id = co.fix_message_id
                              ...
                              and fmj.date_id between 20250601 and 20250930
                            limit 1) fmj on true;

drop table if exists trash.pre_fetch_equity_tca;
create table if not exists trash.pre_fetch_equity_tca
(
    order_id                  int8,
	...
	truncate  table  trash.pre_fetch_equity_tca;
	...
	(
		...
		--
			...
			where  ftr.is_busted  =  'N'
				and  ftr.date_id  between  20250601  and  20250930
	...
	--
			...
			where  date_id  between  20250601  and  20250930  and  date_id  =  tp.date_id  and  dea.activ_symbol  =  tp.activ_symbol
			limit  1
	                )  dea  on  1=1;
...
/*	create  temp  table  if  not  exists  pre_fetch_equity_tca_venue  (
	...
	select
        ...
        from  data_marts.f_yield_capture  yc
                left  join  dwh.flat_trade_record  ftr  on  (ftr.street_order_id  =  yc.order_id  and  ftr.date_id  between  20250601  and  20250930)
                ...
                left  join  eq_tca.eq_rev_ft  rev  on  (rev.trade_record_id  =  ftr.trade_record_id  and  num_nulls(p0ms_bid,  p0ms_ask,  p10ms_bid,  p10ms_ask)  =  0  and  rev.date_id  between  20250601  and  20250930)
                left  join  dwh.d_target_strategy  dts  on  dts.target_strategy_id  =  yc.sub_strategy_id
                join  tmp_fyc  par  on  par.order_id  =  yc.parent_order_id
        where  yc.status_date_id  between  20250601  and  20250930
                and  yc.instrument_type_id  =  'E'
