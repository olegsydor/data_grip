-- DROP FUNCTION dash360.report_compliance_order_blotter_reg(int4, int4, text, bpchar, _int4, _varchar, _int8, _varchar);

CREATE OR REPLACE FUNCTION trash.report_compliance_order_blotter_reg(in_start_date_id integer DEFAULT public.get_dateid(CURRENT_DATE),
                                                                     ...
                                                                     in_street_client_order_ids character varying[] DEFAULT '{}'::character varying[])
    RETURNS TABLE
            (
                ret_row text
            )
...
-- 20250225 SY improvement after PG16 upgrade
-- 20250507 MB https://dashfinancial.atlassian.net/browse/DS-9912 Decommission of data_marts.d_sub_strategy
-- 20250718 MB https://dashfinancial.atlassian.net/browse/DS-10230 Order Blotter for Regulatory Inquiry - Duped Child Row
-- 20251205 SO https://dashfinancial.atlassian.net/browse/DEVREQ-7255  in_street_client_order_ids added
declare
    l_load_id                  int;
    l_step_id                  int;
    l_row_cnt                  int;
    l_add_parent_client_orders character varying[];
...
begin
    ...
    if coalesce(in_street_client_order_ids, '{}') != '{}' then
    ...
    create temp table order_ids_cte as
    ...
    where po.create_date_id between in_start_date_id and in_end_date_id
      ...
      and case
              when coalesce(l_add_parent_client_orders, '{}') = '{}' then true
              else po.client_order_id = any (l_add_parent_client_orders) end
      and case
              when coalesce(in_symbols, '{}') = '{}' then true
              else di.symbol = any (in_symbols) end;

    get diagnostics l_row_cnt = row_count;
    select public.load_log(l_load_id, l_step_id, 'order_ids_cte  created ', l_row_cnt, 'C')
    into l_step_id;

    analyze order_ids_cte;

    drop table if exists co;
    create temp table co as
    select co.*, pyc.client_order_id as pyc_client_order_id, oic.instrument_type_id
    from order_ids_cte oic
        ...
        pyc.create_date_id between in_start_date_id and in_end_date_id
            and pyc.create_date_id = co.create_date_id
...
--        co.order_id,
                                   coalesce(co.pyc_client_order_id, co.client_order_id) , -- as "Parent Cl Ord ID",
                                   ...
                                   case when co.multileg_reporting_type = '1' then 'N' else 'Y' end , -- as "Is Mleg",
...
into l_client_order_ids_street --Parent IDs from street
from dwh.client_order so
join dwh.client_order po on po.create_date_id = so.create_date_id and po.order_id = so.parent_order_id and po.parent_order_id is null
where so.create_date_id between 20251203 and 20251203
	and so.multileg_reporting_type in ('1','2')
	and so.parent_order_id is not null
	and so.client_order_id in (
		'GVAB3334-20251203',
		'B744AAAA069620251203'
	)
group by po.client_order_id;

-- Populate order_ids_cte
 and case
	when coalesce(in_client_order_ids, '{}') = '{}' and coalesce(l_client_order_ids_street, '{}') = '{}' then true --Both empty
	when coalesce(in_client_order_ids, '{}') = '{}' then po.client_order_id = any (l_client_order_ids_street) --Parent client_order_ids empty
	when coalesce(l_client_order_ids_street, '{}') = '{}' then po.client_order_id = any (in_client_order_ids) --Street client_order_ids empty
	else po.client_order_id = any (in_client_order_ids) and po.client_order_id = any(l_client_order_ids_street) --Both not empty
end


select distinct co.client_order_id, po.client_order_id --co.account_id, co.multileg_reporting_type, po.client_order_id, *--distinct(par.client_order_id)
--         into l_add_parent_client_orders
from dwh.client_order co
         join dwh.client_order po on po.order_id = co.parent_order_id
where true
--           and str.client_order_id = any (in_street_client_order_ids)
  and co.parent_order_id is not null
  and case
          when coalesce(:in_account_ids, '{}') = '{}' then true
          else co.account_id = any (:in_account_ids) end
...
select coalesce(:in_street_client_order_ids, '{}') != '{}'

select *
from trash.report_compliance_order_blotter_reg(in_start_date_id := 20251204,
                                               in_end_date_id := 20251204,
                                               in_account_ids := '{63706, 63109}',
                                               in_client_order_ids := '{"L231732512040000003","L231732512040000005","K231732512040000011","L231732512040000020","L231732512040000037"}',
                                               in_street_client_order_ids := '{"FWAA000220251204","FWAA000320251204","FWAA000420251204","FWAA000520251204","FWAA000620251204"}');
...
from trash.report_compliance_order_blotter_reg(in_start_date_id := 20251204,
                                               in_end_date_id := 20251204,
                                               in_account_ids := '{63706, 63109}',
                                               in_client_order_ids := '{"L231732512040000003"}',
                                               in_street_client_order_ids := '{"FWAA000220251204","FWAA000320251204"}');
