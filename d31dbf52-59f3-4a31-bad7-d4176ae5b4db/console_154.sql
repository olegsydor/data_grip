...
CREATE OR REPLACE FUNCTION trash.report_isi_bill_changes_monthly_2(p_start_date_id integer DEFAULT NULL::integer,
...
--   l_gtc_date_id := to_char((to_date(l_start_date_id::varchar, 'YYYYMMDD') - interval '6 months'), 'YYYYMMDD')::integer;
    ...
    from compliance.blaze_execution cbe
             left join lateral (select parentid
                                from trash.so_liquidpoint_edw_treports lp
                                where true
                                  and lp.id = cbe.treports_id
                                limit 1) bcl on true
    where true
...
--ON COMMIT drop
    ...
    from dwh.flat_trade_record tr
             ...
             left join lateral (select treports_id
                                ...
                                where true
                                  ...
                                  and str.parentid = par.order_id ) str on true
    ...
    RETURN QUERY
        ...
        union all
        select coalesce(s.date_id::varchar, '') || ',' || --
        ...
        --coalesce(s.street_tag_17::varchar, '')                         --
                   ...
                   as roe
        from (select date_id, order_id, report_id, tag_17
