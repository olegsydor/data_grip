-- DROP FUNCTION trash.report_gtc_recon_ofp0038(varchar, _int4, int4, int4);

CREATE or replace FUNCTION trash.report_gtc_recon_ofp0038(in_instrument_type_id character varying DEFAULT 'O'::character varying,
...
begin
    ...
    return query
        select 'OrderID,Side,PutOrCall,OpenClose,OpenQty,Qty,Symbol,Suffix,ExpirationDate,StrikePrice,LimitPrice,StopPrice,ExecInst';
...
-- the code below has been added to provide the same performance in the case we use the report for CURRENT date
                ...
                and case
                        ...
                        else gos.account_id = any (in_account_ids) end
                and case
                        when in_instrument_type_id is null then true
                        else i.instrument_type_id = in_instrument_type_id end
...
select *
from trash.report_gtc_recon_ofp0038(in_start_date_id := 20251229, in_end_date_id := 20251229,
                                    in_account_ids := '{58091}');

select array_agg(account_id)
from dwh.d_account
where trading_firm_id = 'OFP0038'
and is_active
