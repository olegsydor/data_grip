...
CREATE OR REPLACE FUNCTION dash360.get_active_child_gtc_orders(in_account_ids integer[] DEFAULT NULL::integer[], in_start_date_id integer DEFAULT (to_char((CURRENT_DATE)::timestamp with time zone, 'YYYYMMDD'::text))::integer, in_end_date_id integer DEFAULT (to_char((CURRENT_DATE)::timestamp with time zone, 'YYYYMMDD'::text))::integer, in_trading_firm_ids character varying[] DEFAULT '{}'::character varying[])
...
begin
    ...
    return query
        select t_base.account_name              as account_name,
               ...
               t_base.client_order_id          as parent_cl_ord_id,
