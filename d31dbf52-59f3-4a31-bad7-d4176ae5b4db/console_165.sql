...
--    raise notice 't_all_hist_o - %', clock_timestamp();
    select public.load_log(l_load_id, l_step_id,
                           'order_chain_for_order_id' || in_order_id::text || ' calculation t_all_hist_co', l_row_count,
                           'O')
    into l_step_id;
...
--    raise notice 't_cte_client_order - %', clock_timestamp();
    ...
    create temp table order_status as
    select order_status
    from t_cte_client_order co
             ...
             join dwh.execution e on co.order_id = e.order_id
    where co.order_id = any (string_to_array((select string_agg(order_id::text, ',') from t_all_hist_o), ',')::bigint[])
      and e.exec_time < co2.process_time
      and e.exec_type = 'F';

    get diagnostics l_row_count = row_count;
    select public.load_log(l_load_id, l_step_id,
                           'order_chain_for_order_id' || in_order_id::text || ' calculation order_status', l_row_count,
                           'O')
    into l_step_id;
    ...
    from (
             ...
             select DISTINCT ON (ex.order_id, co.trans_type) null::bigint                                                              EXEC_ID,
...
select * from trash.so_order_chain_for_order_id(438097887071425807, 'N')