...
--          on commit drop
    ...
    from dwh.client_order cl
    where true
      and cl.client_order_id = '1_gd260127'
      and cl.create_date_id between :in_start_date_id and :in_end_date_id
...
select * from dwh.execution
    ...
    select date_id,
           ...
           last_qty,
           coalesce(tr.tcce_transaction_fee_amount, 0) + coalesce(tr.tcce_maker_taker_fee_amount, 0) +
...
-- DROP FUNCTION trash.report_rps_s3(int4, int4, _int4, bpchar, _varchar, bool);
    select *
    from trash.report_rps_s3(in_start_date_id := 20260127, in_end_date_id := 20260127,
                             in_cl_ord_ids := '{1_gd260127}',
                             in_is_multi_leg := 'Y',
                             in_exclude_blaze := false);
    ...
    CREATE FUNCTION trash.report_rps_s3(in_start_date_id integer, in_end_date_id integer,
                                        ...
                                        in_cl_ord_ids varchar(256)[] default null::varchar(256)[])
        RETURNS TABLE
                (
                    ret_row text
                )
...
--          on commit drop
    ...
    where true
      ...
      and case when in_exclude_blaze then coalesce(cl.ex_destination, '') not ilike 'blaze' else true end
      and case when in_exclude_blaze then coalesce(cl.exchange_id, '') not ilike 'blaze' else true end
    and case when coalesce(in_cl_ord_ids, '{}') = '{}' then true else cl.client_order_id = any(in_cl_ord_ids) end;
    get diagnostics l_row_cnt = row_count;
