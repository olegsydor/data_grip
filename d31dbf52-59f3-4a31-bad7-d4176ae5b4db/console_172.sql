select * from trash.so_report_fintech_s3_master_file(in_start_date_id := 20260622, in_end_date_id := 20260622, in_strategies := '{SENSOR}', in_cl_ord_id := '{G231512606220002007}');

CREATE FUNCTION trash.so_report_fintech_s3_master_file(in_start_date_id integer, in_end_date_id integer, in_account_ids bigint[] DEFAULT '{}'::bigint[], in_instrument_type character DEFAULT 'E'::bpchar, in_trading_firm_ids character varying[] DEFAULT '{}'::character varying[], in_strategies character varying[] DEFAULT NULL::character varying(128)[], in_cl_ord_id varchar[] default null)
...
AS $function$
    ...
    -- SO 20260429 https://dashfinancial.atlassian.net/browse/DS-11295 S3 EOD Equity File
declare
    l_data_firm_id     text;
    l_account_ids      int8[];
    l_load_id          int;
...
begin
    ...
    and case when in_cl_ord_id is null then true else cl.client_order_id = any(in_cl_ord_id) end ;
...
--                               case when cl.time_in_force_id = '5' then '1' else '0' end, -- POST_MARKET_IND
                               ...
                               fc.fix_comp_id, --	CLIENT_TEXT1 (39)
                               null, --	CLIENT_TEXT2 (40)
                               null, --	CLIENT_TEXT3
                               null, --	CLIENT_TEXT4
                               null, --	CLIENT_TEXT5
...
--                               tag_9000, --	ALGO
                               case
                                   when cl.multileg_reporting_type = '3' then tag_9000
                                   when cl.parent_order_id is null then tag_9000
                                   else sdr.wave_type_name
                                   end , -- ALGO
...
--     insert into t_orders
    drop table if exists t_orders_gtc_open;
    create table if not exists t_orders_gtc_open as
    select cl.create_date_id,
           cl.parent_order_id,
           cl.order_id,
           cl.process_time,
    ...
    drop table if exists t_orders_gtc_close;
    create table if not exists t_orders_gtc_close as
    select cl.create_date_id,
           cl.parent_order_id,
           cl.order_id,
           cl.process_time,
    ...
    insert into t_orders
    select *
    from t_orders_gtc_open;

    insert into t_orders
    select *
    from t_orders_gtc_close;
