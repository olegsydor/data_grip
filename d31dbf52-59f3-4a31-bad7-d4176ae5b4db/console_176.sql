...
select * from trash.report_fintech_adh_parent_order_count_review(in_start_date_id := 20260101,
                                                                              in_end_date_id := 20260702,
                                                                              in_instrument_type_id := 'O');

select * from trash.report_fintech_adh_parent_order_count_review(in_start_date_id := 20260101,
                                                                              in_end_date_id := 20260630,
                                                                              in_instrument_type_id := 'O');


CREATE OR REPLACE FUNCTION trash.report_fintech_adh_parent_order_count_review(in_start_date_id integer,
...
begin
    ...
    select oc.period                as "Period",
           oc.trading_firm_name     as "Trading Firm",
           oc.account_name          as "Account",
           oc.customer_or_firm_name as "Capacity",
           sum(oc.cum_qty)          as "Qty",
           sum(oc.order_count)      as "Parent Order Count"
    from trash.fintech_adh_parent_order_count oc
    where oc.status_date_id between in_start_date_id and in_end_date_id
    and case when in_instrument_type_id is null then true else oc.instrument_type_id = in_instrument_type_id end
    group by oc.period, oc.trading_firm_name, oc.account_name, oc.customer_or_firm_name;
