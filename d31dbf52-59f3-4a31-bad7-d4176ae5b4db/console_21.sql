...
create temp table tmp_suppl as
	...
	case
		when str.multileg_reporting_type = '1' then 'N' else 'Y'
	end||'|'||
	'Y'||'|'||--BD Flag
	case
...
--
left join fix_capture.fix_message_json as fm on (tr.street_order_fix_message_id = fm.fix_message_id and fm.date_id = tr.date_id )
...
select
  case
	when REC_TYPE = 'HDR' then REC||lpad((REC_NUM-1)::varchar,10,' ')
	else REC
  end
  from (
    select REC, REC_TYPE, OUT_ORD_FLAG, row_number() OVER () as REC_NUM
    from(
 ...
 union all
  ...
  to_char(in_date, 'MM/DD/YY')

  as REC

 union all
  select  'TITLE' as REC_TYPE, 2 as OUT_ORD_FLAG,
