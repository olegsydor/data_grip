CREATE TABLE db_management.table_retention (
	...
	CONSTRAINT table_retention_check CHECK (((cleanup_schedule)::text = ANY (ARRAY[('DAY'::character varying)::text, ('MONTH'::character varying)::text, ('YEAR'::character varying)::text]))),
...
begin
	...
	for tbl in (select schema_name, table_name from db_management.table_retention where is_active and retention_type = 'D')
		loop
