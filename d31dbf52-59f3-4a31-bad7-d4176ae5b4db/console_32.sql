-- DROP FUNCTION dash360.report_rps_ml_supplemental(int4, int4);
...
--                3       as OUT_ORD_FLAG, --tr.*,
               ...
               tr.open_close,
               tr.last_qty::text ,
               coalesce(tr.street_order_qty, tr.order_qty, 0)::text ,
               to_char(tr.last_px, 'FM99999990.0099'),
               oc.opra_symbol ,
               ui.symbol ,
               to_char(tr.order_process_time, 'HH24:MI:SS') ,
               to_char(tr.trade_record_time, 'HH24:MI:SS') ,
               acc.eq_order_capacity,
               coalesce(tr.street_order_id, tr.order_id)::text ,
               tr.exec_id::varchar, -- column 14
               'DASH' ,
               'DASH' ,
               case tr.subsystem_id
                   when 'HFTDMA' then 'BEBS949C'
               else coalesce(fm.fix_message ->> '440', tr.street_account_name)
               ...
               '' ,--Exchange Access Fee
               case
                   when tr.exchange_id = 'BATS' then tr.contra_broker
                   when tr.exchange_id in ('ARCA', 'AMEX') then tr.trade_exec_broker
                   else ''
                   end ,
               case
        ...
        where tr.date_id between in_start_date_id and in_end_date_id
          ...
          and tr.is_busted = 'N'
          and tr.order_id > 0
--         and tr.street_order_id = 14137311097
          ;
...
select 'n', *
from trash.report_rps_ml_supplemental____(20240111, 20240111)

insert into t_os
select 'o', * from dash360.report_rps_ml_supplemental(20240111, 20240111);

select ret_row from t_os
where "?column?" = 'n'
and ret_row is null
except
select ret_row from t_os
