-- DROP FUNCTION dash360.report_gtc_gtd_schwab_blue_open_order(int4, _varchar, _int4, _varchar, _varchar);
select *
from dash360.report_gtc_gtd_schwab_blue_open_order(
        p_account_ids := '{26586}', in_start_date_id := 20240106, in_end_date_id := 20240107
     );
drop function dash360.report_gtc_gtd_schwab_blue_open_order;

CREATE or replace FUNCTION dash360.report_gtc_gtd_schwab_blue_open_order(--p_date_id integer DEFAULT NULL::integer,
...
begin
  ...
  l_step_id:=1;
    ...
    select public.load_log(l_load_id, l_step_id,
                           'dash360.report_gtc_gtd_schwab_blue_open_order for ' || in_start_date_id::text || '-' ||
  ...
  from
    (
      select
          co.client_order_id as order_number
        , case
        ...
        and gtc.create_date_id <= in_start_date_id -- created today or earlier -- 20221005 --
        ...
        and case when l_account_ids is not null then gtc.account_id = any(l_account_ids) else true end
          ...
          and close_date_id > in_end_date_id
        and gtc.create_date_id <= in_start_date_id-- created today or earlier -- 20221005 --
        and gtc.time_in_force_id in ('1', '6')
        and case when l_account_ids is not null then gtc.account_id = any(l_account_ids) else true end
        ) gtc
        ...
        left join lateral
          (
            ...
            where ex.order_id = gtc.order_id
              ...
              and ex.exec_date_id <= in_start_date_id -- open for today -- 20221005 --
...
--   l_row_cnt := coalesce((select count(1) from tmp_gtc_schwab_roe), 0);
   ...
   return query
       select case
              ...
              to_char(to_date(in_start_date_id::varchar, 'YYYYMMDD'), 'MMDDYYYY'); -- File Creation Date
