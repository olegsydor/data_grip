...
--    ACC.OPT_IS_FIX_EXECBROK_PROCESSED,
       ...
       CASE
           ...
           END                                                                               "LastMkt",                    --52
       CO.ORDER_QTY - EX.CUM_QTY + coalesce(eq.sum_last_qty, 0) as "DayOrderQty",                --53
       coalesce(eq.sum_last_qty, 0) as "DayCumQty",                  --54
       eq.sum_last_qty_last_px/coalesce(sum_last_qty, 1) as "DayAvgPx",
...
FROM cte_client_order CO
         JOIN dwh.EXECUTION EX ON (EX.ORDER_ID = CO.ORDER_ID and ex.exec_date_id >= co.create_date_id and ex.exec_date_id > 20220101)  -- remove this hardcode!!!
--          JOIN
...
--      ) HSD ON (HSD.INSTRUMENTID = CO.INSTRUMENT_ID)
     join lateral (SELECT I.INSTRUMENT_ID         InstrumentID,
      ...
      FROM dwh.d_INSTRUMENT I
               ...
               LEFT JOIN dwh.d_OPTION_SERIES OS on (OC.OPTION_SERIES_ID = OS.OPTION_SERIES_ID)
               LEFT JOIN dwh.d_INSTRUMENT U ON (OS.UNDERLYING_INSTRUMENT_ID = U.INSTRUMENT_ID)
      WHERE I.INSTRUMENT_ID = CO.INSTRUMENT_ID and I.INSTRUMENT_TYPE_ID IN ('E', 'O', 'M') limit 1
         --AND I.is_active
     ) HSD ON true
...
left join lateral (SELECT FM.fix_message ->> '11' as tag_11,
                     ...
                     WHERE EX.FIX_MESSAGE_ID = FM.FIX_MESSAGE_ID limit 1) fmj on true
...
group by eq,order_id limit 1) eq on true
WHERE 1 = 1
  --and EX.ORDER_STATUS <> '3'
  --AND CO.MULTILEG_REPORTING_TYPE IN ('1','2')
  AND co.order_id in (select order_id from all_hist_o)
  AND co.create_date_id in (select create_date_id from all_hist_o)
