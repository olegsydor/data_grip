CREATE or replace FUNCTION dash360.report_fintech_ml_pro_200byte(in_start_date_id integer, in_end_date_id integer, in_account_ids integer[] DEFAULT '{}'::integer[], in_instrument_type character DEFAULT NULL::bpchar)
...
begin
    ...
    with base as (select
                      ...
                      round(sum(
                  		case
                      		when a.trading_firm_id = 'ctceq' then 0.0007 * tr.last_qty
...
select
    tr.date_id, tr.instrument_type_id, hsd.display_instrument_id,
    --AA Record
    'AA'                                                       as "AA_RECORD_TYPE",
    row_number() over (
        partition by tr.date_id
