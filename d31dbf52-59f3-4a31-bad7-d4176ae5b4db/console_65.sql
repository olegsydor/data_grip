-- DROP FUNCTION dash360.get_active_parent_gtc_orders(_int4, int4, int4);
...
{11881,14765,17287,19250,20805,26209,26270,26271,26272,26273,26454,26484,26546,26636,29609,30150,30429,50084,52465,57000,61790,63904,64998}
select array_agg(distinct trading_firm_id) from (select ac.account_id, ac.trading_firm_id
                        from dwh.gtc_order_status gtc
 ...
 select array_agg(trading_firm_id)
...
select *
from dash360.get_active_child_gtc_orders(--in_account_ids := '{11881,14765,17287}',
                                        in_start_date_id := 20240509,
                                        in_end_date_id := 20240509,
                                        in_trading_firm_ids := '{"greatpnt","OFP0001"}'
     )
except
select *
from dash360.get_active_child_gtc_orders(--in_account_ids := '{11881,14765,17287}',
...
-- DROP FUNCTION dash360.get_active_child_gtc_orders(_int4, int4, int4);

CREATE FUNCTION dash360.get_active_child_gtc_orders(in_account_ids integer[] DEFAULT NULL::integer[],
                                                               in_start_date_id integer DEFAULT (to_char((CURRENT_DATE)::timestamp with time zone, 'YYYYMMDD'::text))::integer,
                                                               in_end_date_id integer DEFAULT (to_char((CURRENT_DATE)::timestamp with time zone, 'YYYYMMDD'::text))::integer,
                                                               in_trading_firm_ids character varying[] default '{}'::character varying[])
    RETURNS TABLE
