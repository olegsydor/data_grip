call trash.get_consolidator_eod_pg_8(20240621);
-- DROP PROCEDURE trash.get_consolidator_eod_pg_8(int4);

CREATE OR REPLACE PROCEDURE trash.get_consolidator_eod_pg_8(IN in_date_id integer)
 LANGUAGE plpgsql
AS $procedure$
declare
    l_load_id int;
...
select CONSTR.CROSS_ORDER_ID, CONSTR.ORDER_ID, array_agg(ALP.LP_DEMO_MNEMONIC) as t_alp_agg
...
group by CONSTR.CROSS_ORDER_ID, CONSTR.ORDER_ID;

create index on t_alp (CROSS_ORDER_ID);
...
-----
    ...
    from dwh.client_order cl
             ...
             left join lateral (select cnl.no_legs
                                ...
                                limit 1) lnb on true
/*
                                left join lateral (select t_alp.lp_demo_mnemonic as contr_str
...
--                              group by pcon.fix_connection_id
             ...
             left join lateral (select t_alp.lp_demo_mnemonic as contr
...
--                              group by pcon.fix_connection_id
                                limit 1) cc on true
*/
             left join lateral(select * from t_alp as cc_str
             where  cc_str.cross_order_id = STR.CROSS_ORDER_ID and cc_str.order_id <> STR.ORDER_ID
                                  and cl.PARENT_ORDER_ID is null
                                  and STR.CROSS_ORDER_ID is not null
                                  limit 1) cc_str on true
             left join lateral (select * from t_alp as cc
...
/*
        -- amex
             left join lateral (select ask_price, bid_price, ask_qty as ask_quantity, bid_qty as bid_quantity
                                from dwh.get_routing_market_data(in_transaction_id := cl.transaction_id,
                                in_exchange_id := 'AMEX',
                                in_multileg_reporting_type := cl.multileg_reporting_type,
                                in_instrument_id := cl.instrument_id,
