...
select char_length(ret_row), ret_row from dash360.report_fintech_eod_pershing_ps_trade_file(in_start_date_id := 20241107, in_end_date_id:= 20241107, in_account_ids := '{8112}');

select rpad(:symb, 9) -- as cusip;


    create temp drop table t_rec
--          on commit drop
    as
    select tr.trade_record_id,
           case
...
--                                rpad(tr.exchange_id, 4), -- as exchange_mnemonic, [12-15]
                               rpad('', 4), -- as exchange_mnemonic, [12-15]
                               lpad('', 2), -- as filler1, [16-17]
                               rpad(case when tr.instrument_type_id = 'E' then di.symbol else 'OPTION' end,
                                    9), -- as cusip_symbol, --Use Symbol? We don't provide CUISP [18-26]
                               '8', -- as market, [27]
                               '1', -- as blotter, [28]
                               lpad(tr.last_qty::text, 8, '0'), -- as quantity, [29-36]
...
--                                rpad(tr.exchange_id, 4), -- as exchange_mnemonic, [12-15]
                               ...
                               lpad('', 10), -- as trading_account,[64-73]
--                               lpad('', 4), -- as execution_time_hhmm,[74-77]
                               lpad(to_char(tr.trade_record_time, 'HH24MI'), 4), -- as execution_time_hhmm,[74-77]
                               '3', -- as control_field, [78]
                               lpad('', 2) -- as execution_time_ss [79-80]
                               ], '', '')                   as ret_row
    from dwh.flat_trade_record tr
...
select ret_row, trade_record_time from t_rec
order by trade_record_id, record_num;
