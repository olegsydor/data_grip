...
select * from dash360.report_fintech_eod_pershing_ps_trade_file(in_start_date_id := 20241107, in_end_date_id:= 20241107, in_account_ids := '{35871}');
...
-- DROP FUNCTION dash360.report_fintech_eod_pershing_ps_trade_file(int4, int4, _text, _int4);

CREATE OR REPLACE FUNCTION dash360.report_fintech_eod_pershing_ps_trade_file(in_start_date_id integer, in_end_date_id integer, in_trading_firm_ids text[] DEFAULT '{}'::text[], in_account_ids integer[] DEFAULT '{}'::integer[])
...
begin
    ...
    select public.load_log(l_load_id, l_step_id,
                           'report_fintech_eod_pershing_ps_trade_file for ' || in_start_date_id::text || '-' ||
    ...
    return query
        select array_to_string(ARRAY [
                                   ...
                                   rpad(l_batch_id, 5), -- as batch_id, --Should we hardcode?
                                   lpad('', 48), -- as filler,
...
--                                rpad(tr.exchange_id, 4), -- as exchange_mnemonic, [12-15]
    ...
    return query
        select array_to_string(ARRAY [
                                   ...
                                   lpad(l_batch_id, 5), -- as batch_id, --Should we hardcode? [7-11]
                                   lpad('', 4), -- as filler, [12-15]
                                   lpad(l_row_cnt::text, 7, '0'), -- RECORD COUNT [16-22]
                                   lpad('', 3), -- as filler, [23-25]
                                   lpad(coalesce(l_sum_buy, 0)::text, 17, '0'), -- as Total Buy Quantity [26-42]
                                   lpad('', 3), -- as filler, [43-45]
                                   lpad(coalesce(l_sum_sell, 0)::text, 17, '0'), -- as Total Sell Quantity [46-62]
