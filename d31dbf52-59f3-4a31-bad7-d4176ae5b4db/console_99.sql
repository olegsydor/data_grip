...
select * from t_old
order by 1;
-- order_id in (10619764231,10619764244,10619764245,10619764255,10619764270,10619764288,10619764578)


-- new version
with order_ids_cte as (select co.create_date_id, co.order_id, di.instrument_type_id, di.display_instrument_id
                       from dwh.client_order po
                                join dwh.client_order co on (co.create_date_id = po.create_date_id and
                                                             (co.order_id = po.order_id or co.parent_order_id = po.order_id))
                           --                                         join dwh.historic_security_definition_all hsd
...
select --array_to_string(ARRAY [
       ...
       ftr.day_cum_qty::text                                               as "Ex Qty",
       ftr.leaves_qty::text                                                as "Lvs Qty",
       hsd.display_instrument_id                                           as "Symbol",
       to_char(hsd.maturity_date, 'MM/DD/YYYY')                            as "Expiration Day",
       round(co.price, 6)::text                                            as "Price",
       round(ftr.avg_px, 6)::text                                          as "Avg Px",
       tif.tif_name                                                        as "TIF",
       ot.order_type_name                                                  as "Ord Type",
       case
           when co.open_close = 'O' then 'Open'
...
--                                               co.create_date_id = yc.status_date_id and co.order_id = yc.order_id)
         ...
         left join lateral
    (
    select os.order_status_description, ex.exec_text, ex.fix_message_id, exec_type_description, exec_broker, exec_time
    from dwh.execution ex
             left join dwh.d_order_status os on (os.order_status = ex.order_status and os.is_active)
             left join dwh.d_exec_type et on (et.exec_type = ex.exec_type)
    ...
    limit 1
         ...
         left join lateral (
    select max(ftr.date_id)                                                                     as date_id,
           max(ftr.cmta)                                                                        as cmta,
           ftr.open_close,
           ...
           sum(last_px)                                                                         as last_px,
           avg(last_qty),
           --avg(last_px) as avg_px,
    ...
    where ftr.date_id between :in_start_date_id and :in_end_date_id
      ...
      and is_busted = 'N'
      and ftr.order_id > 0
    group by ftr.open_close, ftr.order_id, ftr.instrument_id, ftr.account_id, ftr.side
    ) ftr on true
         left join dwh.d_fix_connection fc on fc.fix_connection_id = co.fix_connection_id --and fc.is_active
         left join dwh.client_order2auction coa
                   on coa.create_date_id between :in_start_date_id and :in_end_date_id and
...
and co.order_id in (10619764231,10619764244,10619764245,10619764255,10619764270,10619764288,10619764578)
