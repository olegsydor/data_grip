alter table dwh.clearing_instruction
...
begin
    /*
        ...
        17-11-2025 - OS  - DS-10726 - [DB] Extend dash360.executions_blotter_child_executions procedure to return new parameter SocGenContributorID
        11-12-2025 - OS  - DS-10839 - [DB] Extend dash360.executions_blotter_child_executions procedure to return new parameter pillarDealId
        07-01-2026 - OS  - D360-16941 Return Total CCRU Amount for Execution Blotter, Change Clearing Pop-up, Allocations
        12-02-2026 - AA  - DS-11046 - [DB] Extend dash360.executions_blotter_child_executions procedure to return Client Algorithm Alias parameter
        16-02-2026 - OS - https://dashfinancial.atlassian.net/browse/DS-10860
        18-03-2026 - OS - https://dashfinancial.atlassian.net/browse/DS-11272 Return "Clearing Submitted Away" parameter for Executions. Procedure - dash360.executions_blotter_child_executions
    */

    LOOP
        SELECT trim(FROM split_part(coalesce(user_filter, ''), 'and', n))
        INTO part_where; -- the first entry is allways '' if user filter starts from 'and'
    ...
    select ' '
               ...
               || case
                      when start_status_date is not null and end_status_date is not null
                          then ' and tr.trade_record_time between $4 and $5 '
                      else '' end
               || case
                      when user_filter is not null then replace(user_filter, ' and CLIENT_ORDER_ID',
                                                                ' and tr.CLIENT_ORDER_ID')
                      else '' end
...
--    coalesce(e.real_exchange_id, e.exchange_id) real_exchange_id,
    e.exchange_id as real_exchange_id,
    tr.principal_amount,
    tr.ask_price,
    tr.bid_price,
    tr.ask_qty,
    ...
    tr.client_commission_rate * tr.last_qty as client_commission_amount,
		fmj2.fix_message_jsonb ->> ''9310'' AS client_algorithm_alias,
    ci.clearing_submitted_away::character
...
--              ) cin on tr.trade_record_id = cin.trade_record_id
    ...
    left join lateral (select fix_message_jsonb from fix_capture.fix_message_json fmj where fmj.date_id = tr.date_id and fmj.fix_message_id = tr.trade_fix_message_id) fmj on true
    left join lateral (select fix_message_jsonb from fix_capture.fix_message_json fmj2 where fmj2.date_id = tr.date_id and fmj2.fix_message_id = tr.order_fix_message_id) fmj2 on true
where
    tr.is_busted = ''N''
