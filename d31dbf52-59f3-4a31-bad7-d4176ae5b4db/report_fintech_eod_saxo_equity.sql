...
CREATE OR REPLACE FUNCTION fintech_reports.adh_order_blotter_reg(in_start_date_id integer DEFAULT get_dateid(CURRENT_DATE), in_end_date_id integer DEFAULT get_dateid(CURRENT_DATE), in_row_type text DEFAULT NULL::text, in_instrument_type character DEFAULT NULL::bpchar, in_account_ids integer[] DEFAULT '{}'::integer[], in_client_order_ids character varying[] DEFAULT '{}'::character varying[], in_symbols character varying[] DEFAULT '{}'::character varying[])
 RETURNS TABLE("Parent Cl Ord ID" character varying, "Row Type" character varying, "Account" character varying, "Creation Date" text, "Event Date" text, "Creation Time" text, "Routed Time" text, "Event Time" text, "Order Status" character varying, "Cl Ord ID" character varying, "Orig Cl Ord ID" character varying, "Side" text, "Ord Qty" integer, "Ex Qty" integer, "Lvs Qty" integer, "Symbol" character varying, "Expiration Day" text, "Price" numeric, "Avg Px" numeric, "TIF" character varying, "Ord Type" character varying, "O/C" text, "Security Type" text, "Root Symbol" character varying, "Client ID" character varying, "Capacity" character varying, "Sub Strategy" character varying, "Ex Dest" character varying, "Sending Firm" character varying, "Event Type" character varying, "Free Text" character varying, "Reject Reason" text, "Max Floor" bigint, "Exec Broker" character varying, "CMTA" character varying, "Is Mleg" text, "Is Cross" text, "ATS Auction ID" bigint, "Cross Ord Type" character varying, "Fee Sensitivity" smallint, "Handle Inst" character varying, "Locate Broker" character varying, "Leg ID" character varying, "MPID" character varying, "OCC Opt Data" character varying, "Ord Capacity" character varying, "OSI Symbol" character varying, "Sub System" character varying, "Session Eligibility" character varying, "Sweep Style" character varying, "Trading Firm" character varying)
 LANGUAGE plpgsql
AS $function$
declare
begin
    ...
    return query
    with order_ids_cte as (
    	select co.create_date_id, co.order_id
    	from dwh.client_order po
    	join dwh.client_order co on (co.create_date_id = po.create_date_id and (co.order_id = po.order_id or co.parent_order_id = po.order_id))
    	join dwh.historic_security_definition_all hsd on (hsd.instrument_id = po.instrument_id)
    	where po.create_date_id between in_start_date_id and in_end_date_id
			...
			and case when in_instrument_type is null then true else hsd.instrument_type_id = in_instrument_type end
			and case when coalesce(in_account_ids, '{}') = '{}' then true else co.account_id = any(in_account_ids) end
			and case when coalesce(in_client_order_ids, '{}') = '{}' then true else po.client_order_id = any(in_client_order_ids) end
			and case when coalesce(in_symbols, '{}') = '{}' then true else hsd.symbol = any(in_symbols) end
    )
	select coalesce(pyc.client_order_id , yc.client_order_id)                  as "Parent Cl Ord ID",
	       (case when yc.parent_order_id is null then 'Parent' else 'Child' end)::varchar as "Row Type",
	       ...
	       round(yc.order_price, 6)                                            as "Price",
	       ...
	       --cro.cross_type                                                      as "Cross Ord Type",
	       case
	...
	from order_ids_cte oic
			 join data_marts.f_yield_capture yc on (yc.status_date_id between in_start_date_id and in_end_date_id and yc.status_date_id = oic.create_date_id and yc.order_id = oic.order_id)
		     left join data_marts.f_yield_capture pyc on (pyc.status_date_id between in_start_date_id and in_end_date_id and pyc.status_date_id = yc.status_date_id and pyc.order_id = yc.parent_order_id)
	    ...
	    limit 1
	         ...
	         left join dwh.client_order2auction coa on coa.create_date_id between in_start_date_id and in_end_date_id and coa.create_date_id = co.create_date_id and coa.order_id = co.order_id
