 create temp table t_stg_rept_locked_markets as
...
select * from t_stg_rept_locked_markets;
...
create temp table t_recurs as
        ...
        select first_value(m.market_transact_time) over w as lock_begin,
          last_value(m.market_transact_time) over w as lock_end,
          last_value(m.market_transact_time) over w -
            first_value(m.market_transact_time) over w as lock_duration
          , m.*
        from hierarch_cte m
 ...
 update t_stg_rept_locked_markets m
    ...
    from
     t_recurs src
    where true -- and src.is_block is not null
      and src.start_date_id = m.start_date_id
      and src.transaction_id = m.transaction_id
      and src.exchange_id = m.exchange_id
      and src.instrument_id = m.instrument_id;
 ...
 create temp table tmp_fyc3 as
 select m.start_date_id
      , m.transaction_id
      , m.instrument_id
      , m.bid_price
      , m.ask_price
 ...
 from t_stg_rept_locked_markets m
     ...
     where y.status_date_id between :l_start_date_id and :l_end_date_id -- = 20171219
       ...
       and case when y.side in ('1', '3') then m.ask_price <= y.order_price -- buy order is marketable
         when y.side not in ('1', '3') then m.bid_price >= y.order_price end-- sell order is marketable
...
create temp table tmp_fyc2 as
 select m.start_date_id
      ...
      , to_date(status_date_id::varchar, 'YYYYMMDD')                                       as status_date
 from t_stg_rept_locked_markets m
          left join data_marts.f_yield_capture y
     on (y.status_date_id between :l_start_date_id and :l_end_date_id -- = 20171219
       and y.status_date_id = m.start_date_id
       and y.parent_order_id is not null                                -- street level
       and y.multileg_reporting_type in ('1', '2')
       and y.instrument_id = m.instrument_id
...
select * from tmp_fyc2
except
select * from tmp_fyc3
