...
select * from staging.find_in_load_timing('insert into dwh.flat_trade_record date_id', 60*24*1)
where true
order by 1 desc, 2 desc;

-- DROP FUNCTION staging.find_in_load_timing(text, int4, timestamp);

CREATE OR REPLACE FUNCTION staging.find_in_load_timing(in_text text, in_minutes integer DEFAULT 120, in_from timestamp without time zone DEFAULT clock_timestamp())
 RETURNS TABLE(load_timing_id bigint, step integer, table_name character varying, row_count integer, seconds integer, log_date timestamp without time zone, operation character)
 LANGUAGE plpgsql
AS $function$
...
begin
    return query
        with c_ids as
                 (
                     select distinct x.log_date, x.load_timing_id as ids
                     from public.load_timing x
                     where x.log_date >= in_from - (interval '1 minute') * in_minutes
                       and x.log_date < in_from
                       and x.table_name ilike ('%' || in_text || '%')
                 )
        select lt.load_timing_id::bigint, lt.step, lt.table_name, lt.row_count, lt.seconds, lt.log_date, lt.operation
        from public.load_timing lt
        where lt.load_timing_id in (select ids from c_ids)
    --order by 1, 2
    ;
end;
...
select str2au.auction_id, str2au.create_date_id,
        CASE
            WHEN str2au.auction_id IS NULL THEN NULL::text
            WHEN exists (SELECT null
                         FROM request_for_quote r
 ...
 from staging.trade_record tr
                     LEFT JOIN lateral (select auction_id, create_date_id from client_order2auction str2au
                               where str2au.order_id = tr.street_order_id AND
                                  str2au.create_date_id = to_char(tr.street_order_process_time, 'YYYYMMDD')::int limit 1) str2au on true
where true
and tr.client_order_id ='13152000000035'
and tr.street_client_order_id ='DPAA0012-20260116'
and tr.date_id = 20260116