...
create function dash360.get_trade_records_for_allocation(in_account_ids bigint[] default '{}'::bigint[],
                                                         in_date_id integer,
    ...
    returns table
            (
                "tradeDate"            date,                        -- date_id
                "tradeId"              bigint,                      -- trade_record_id
                "accId"                integer,                     -- account_id
                "instrId"              bigint,                      -- instrument_id
                "side"                 character,
                "oc"                   character,                   -- open_close
                "symbol"               character varying,
                "execQty"              integer,                     -- exec_qty
                "avgPx"                numeric,                     -- avg_px
                "expDate"              timestamp without time zone, -- expiration_date
                "secType"              character,                   -- instrument_type_id
                "execTime"             timestamp without time zone, -- street_exec_time
                "notional"             numeric,                     -- ???
                "isAllocated"          boolean,                     -- is_allocated
                "isBundle"             boolean,                     -- is_bundle
                "gup"                  character varying,           -- exec_broker
                "reportedStatus"       boolean,                     --is_prev_reported
                "reportedTime"         timestamp without time zone, -- reported_time
                "expirationDate"       timestamp without time zone, -- last_trade_date
                "customerFirm"         character                    -- opt_customer_firm
...
--     raise notice '2 - %', clock_timestamp();
    ...
    return query
        select tr.date_id::text::date as "tradeDate",
               tr.trade_record_id::int8 as "tradeId",
               tr.account_id::int4 as "accId",
               tr.instrument_id::int8 as "instrId",
               tr.side::character as "side",
               tr.open_close::character as "oc",
               i.symbol as "symbol",
               tr.last_qty::int4                                                                        as "execQty",
               tr.last_px                                                                               as "avgPx",
               i.last_trade_date as "expDate",
               i.instrument_type_id::character as "secType" ,
               coalesce(tr.street_trade_record_time, tr.trade_record_time)::timestamp without time zone as "execTime",
-999 as "notional",
               false                                                                                    as "isAllocated",
               false                                                                                    as "isBundle",
               tr.exec_broker::character varying as "gup",
               coalesce(nullif(tr.is_billed, 'N'), rep.to_report)::character                            as  "reportedStatus",
               case
...
--                           and tri.exch_exec_id = tr.exch_exec_id
                                 ...
                                 order by 1
                                 limit 1),
                                rep.db_create_time) end                                                 as "reportedTime",
               tr.opt_customer_firm::character as "customerFirm",
