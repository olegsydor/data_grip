CREATE FUNCTION trash.dash360_imb_get_unmatched_trade_records(in_start_date_id integer DEFAULT get_dateid(CURRENT_DATE), in_end_date_id integer DEFAULT get_dateid(CURRENT_DATE), in_account_ids integer[] DEFAULT '{}'::integer[], user_filter character varying DEFAULT NULL::character varying)
 RETURNS TABLE(trade_record_time timestamp without time zone, trade_record_id bigint, trading_firm_id character varying, account_id bigint, customer_type character varying, symbol_osi character varying, symbol character varying, root_symbol character varying, exp_date date, strike_price numeric, put_call character, side character, last_qty integer, last_px numeric, secondary_order_id character varying, exec_broker character varying, is_trade_electronic boolean, date_id integer, cmta character varying, exchange_id character varying)
 LANGUAGE plpgsql
 STABLE COST 1
AS $function$
...
select max(log_date) - 20 * interval '1 minute' into l_last_match_time
...
create temp table t_imb_get_unmatched_trade_records on commit drop
 ...
 select tr.trade_record_time,
	       ...
	       to_date(oc.maturity_year::text||lpad(oc.maturity_month::text, 2, '0')||lpad(oc.maturity_day::text,2, '0'), 'YYYYMMDD') as Exp_Dat,
	       oc.strike_price,
	       oc.put_call ,
	       tr.side,
	       tr.last_qty,
	...
	left join occ_data.occ_trade_data_matching otdm on otdm.date_id = tr.date_id and otdm.trade_record_id = tr.trade_record_id and otdm.date_id between in_start_date_id and in_end_date_id
...
select regexp_replace('drop truncate select', '[drop|truncate]', 'nothing')
