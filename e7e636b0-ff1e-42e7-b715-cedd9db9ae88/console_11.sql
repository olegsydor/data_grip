...
CREATE or replace FUNCTION genesis2.auto_allocate_unallocated_trade_1(in_instrument_type_id character, in_allocation_type integer, in_date_id integer DEFAULT get_dateid(CURRENT_DATE))
 RETURNS integer
 LANGUAGE plpgsql
 SET application_name TO 'ETL:  AutoAllocation'
AS $function$
...
-- 																	condition on cmta is not null has been removed from that join.
--  SY:  20240221 https://dashfinancial.atlassian.net/browse/DS-8077	having count(1) has been added
--  SO: 20240404  https://dashfinancial.atlassian.net/browse/DS-8169 "performance improvement"
...
execute 'select max(TRADE_RECORD_ID)  from TRADE_RECORD where is_busted=''N'' and date_id = '||l_date_id
 ...
 create temp table t_pre_trade_for_allocations on commit drop
 as
   ...
   select TR.ACCOUNT_ID, TR.INSTRUMENT_ID,  TR.SIDE,  TR.OPEN_CLOSE, tr.cmta,
    ...
    tr.trade_record_id
    	...
    	select public.load_log(l_load_id, l_step_id, 'create temp pre_table trade_for_allocations', l_cnt_rows, 'I')
		into l_step_id;
...
select TR.ACCOUNT_ID,
       ...
       array_agg(tr.trade_record_id)                                                     as trade_ids,
       nextval('allocation_instruction_alloc_instr_id_seq'::regclass) as alloc_instr_id
       from t_pre_trade_for_allocations tr
