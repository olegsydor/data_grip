CREATE OR REPLACE FUNCTION dash_reporting.get_ebsmlbccequities(in_date timestamp without time zone)
 RETURNS refcursor
 LANGUAGE plpgsql
AS $function$
...
begin
   select nextval('public.load_timing_seq') into l_load_id;
    l_step_id:=1;
	 select public.load_log(l_load_id, l_step_id, 'reporting_get_ebsmlbccequities STARTED ====', 0, 'O')
	 into l_step_id;

	l_date_id:= to_char(in_date, 'YYYYMMDD')::int;
	   select public.load_log(l_load_id, l_step_id, 'l_date_id size is ='||l_date_id, 1, 'O')
		 into l_step_id;
...
-- allocations and orders
		eq_alloc as
		(
			select 3 as ord_flag_1, al.alloc_instr_id as ord_flag_2,'ALLOC' as rec_type,
			al.date_id as trade_date,
			ac.account_id,
			ca.clearing_account_number,
			i.symbol,i.symbol_suffix,
			0 as order_id,
			al.avg_px,
			...
			inner join genesis2.instrument i on (al.instrument_id = i.instrument_id and i.instrument_type_id = 'E' and i.symbol not in (select test_symbol from dash_reporting.test_symbol_tb))
			inner join genesis2.clearing_account ca on (ae.clearing_account_id = ca.clearing_account_id and ca.is_deleted = 'N'  and ca.clearing_account_type <> '3')
			inner join t_active_account_snapshot ac on ca.account_id = ac.account_id and (ac.eq_report_to_mpid = 'MLCB' or (ac.nscc_mpid <> 'DFIN' and ac.eq_real_time_report_to_mpid = 'MLCB'))

			where ae.alloc_qty > 0
			and al.date_id = l_date_id
			and al.create_time::time < '17:15'::time
			and ae.alloc_instr_id in (select alloc_instr_id from genesis2.alloc_instr2trade_record atr where atr.date_id = l_date_id )
			and ac.account_name not in ('U670015','CBOEU670015','ABRAMSOND','JERROLDE','9585X001','JESS1','JESS1-TRAD','4ZJX1309','4ZJX1209','JESS2','JESS2-TRAD','WBULL')
			and ac.trading_firm_id not in ('ebseqtc','equitec02','ioniccap','ionicap02')
			and coalesce(ac.eq_rt_allocation_enabled,'N') <> 'Y'
			-- only allocated trades should be reported
			/*
			union all

			select 4 as ord_flag_1, tr.order_id as ord_flag_2,'ALLOC' as rec_type,
			tr.date_id as trade_date,
			ac.account_id,
			...
			inner join genesis2.instrument i on (i.instrument_id = tr.instrument_id and i.symbol not in (select test_symbol from dash_reporting.test_symbol_tb)	)
			inner join t_active_account_snapshot ac on (ac.account_id  = tr.account_id)
			inner join genesis2.clearing_account dca on (tr.account_id = dca.account_id and dca.is_default = 'Y' and dca.is_deleted = 'N'   and dca.market_type = 'E' and dca.clearing_account_type <> '3')
			where tr.date_id = l_date_id
			and tr.is_busted = 'N'
			and i.instrument_type_id = 'E'
			and tr.order_id >0 --Exlude Blaze manual trades
			...
			group by tr.date_id,ac.account_id,dca.clearing_account_number,dca.clearing_account_id,dca.account_executive_number,tr.instrument_id,tr.side,i.symbol,i.symbol_suffix,tr.order_id
		...
		(
		 ...
		 from (
			...
			case
			  ...
			  else to_char(public.get_settle_date_by_instrument_type(l_date_id::text::date, 'E'), 'YYYYMMDD')
			end||--55-62 Settle Date
			...
			-- commissions !!!
			left join staging.commission_set cse on (cse.account_id = ae.account_id and cse.instrument_type_id = 'E' and cse.is_deleted = 'N'  and coalesce(to_char(end_date, 'YYYYMMDD')::int , l_date_id+1) > l_date_id)
			...
			inner join t_active_account_snapshot ac on (ac.account_id = ae.account_id )
			---ticket charge
			left join staging.commission_set cse on (cse.account_id = ac.account_id and cse.instrument_type_id = 'O' and cse.is_deleted = 'N'  and coalesce(to_char(cse.end_date, 'YYYYMMDD')::int, l_date_id+1) > l_date_id)
			left join t_misc_fee mf on (mf.commission_set_id = cse.commission_set_id and mf.misc_fee_name_id = 4 and mf.is_deleted = 'N')
