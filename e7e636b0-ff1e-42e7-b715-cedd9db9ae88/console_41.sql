...
-- DROP FUNCTION dash360.bofa_allocation_report_v2(int4, int4, text, bool, _int4);

CREATE OR REPLACE FUNCTION dash360.bofa_allocation_report_v2(in_start_date_id integer, in_end_date_id integer,
    ...
    with cte_ai as (select ai.alloc_instr_id
                    ...
                    where true
                      ...
                      and exists (select ar.alloc_instr_id
                                  ...
                                  having count(*) = 1))
    insert
    into dash_reporting.bofa_allocation_report
...
AS $function$
	...
	-- OS 20260123 https://dashfinancial.atlassian.net/browse/DS-10914 added client_commission_amount
declare
    l_date_id integer;
begin
...
CREATE OR REPLACE FUNCTION dash360.allocations_instruction_commission_rate(in_alloc_instr_id integer, in_date_id integer DEFAULT get_dateid(CURRENT_DATE))
 RETURNS TABLE(date_id integer, trade_record_id bigint, account_id integer, instrument_id bigint, side character, open_close character, avg_px numeric, exec_qty integer, display_instrument_id character varying, last_trade_date date, instrument_type_id character, alloc_instr_id integer, alloc_time timestamp without time zone, is_allocated boolean, is_bundle boolean, cmta character varying, exec_broker character varying, principal_amount numeric, client_commission_rate numeric, username character varying, client_commission_amount numeric)
 LANGUAGE plpgsql
 COST 1
AS $function$
begin
...
left join lateral (select sum(/*coalesce(*/l1.rate /*,0)*/*tr.last_qty)/nullif(sum(tr.last_qty),0) as rate, sum(amount) as amount -- Coalesce commented due to DS-977 .
                   from  alloc_instr2trade_record  alt
                   inner join trade_record tr on alt.trade_record_id = tr.trade_record_id and alt.date_id = tr.date_id
                   left join lateral (select rate, tl.amount,  row_number() over (partition by tl.trade_record_id , book_record_type_id , billing_entity order by cr.priority ) as rn
                                      from trade_level_book_record tl
                                      inner join book_record_creator  cr on tl.book_record_creator_id  = cr.book_record_creator_id
                                      where tl.date_id = in_date_id
					  		            AND tl.book_record_type_id ='CCRU'
										and tl.trade_record_id  = alt.trade_record_id) l1 on true
