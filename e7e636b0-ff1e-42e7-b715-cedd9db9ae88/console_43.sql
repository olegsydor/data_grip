...
-- SY 20250417 https://dashfinancial.atlassian.net/browse/DS-9872 Updated mapping logic 1 to do not match Manual trades
-- AK 20250430 https://dashfinancial.atlassian.net/browse/DS-9872 implemented flow to manage is_busted values
-- OS 20250806 https://dashfinancial.atlassian.net/browse/DS-10307 added a condition for strategas (update for mapping_logic third OR condition)
...
--======================================================================================
	...
	drop table if exists v_busted_trade_tmp;
	create temp table v_busted_trade_tmp as
	select '-1'::integer * staging.base32_to_int8(exec_id) as exec_id,
			order_id,
			chain_id,
			leg_ref_id,
			db_create_time,
			...
			is_busted,
			cl_ord_id
	from staging.v_busted_trade;

	GET DIAGNOSTICS row_cnt = ROW_COUNT;

	select public.load_log(l_load_id, l_step_id, 'v_busted_trade_tmp temp table created', row_cnt, 'U')
		into l_step_id;

	analyze v_busted_trade_tmp;

	select to_char(db_create_time,'YYYYMMDD') from v_busted_trade_tmp
		into l_min_date_id;

	update staging.trade_record_blaze7 trb
		set is_busted = tmp.is_busted,
            load_batch_id  = in_load_batch_id -- SY 20250505.
	from v_busted_trade_tmp tmp
		where trb.exec_id = tmp.exec_id
			and trb.date_id >= l_min_date_id
			and coalesce(trb.is_busted,'123')<> coalesce(tmp.is_busted,'123')
	;

	GET DIAGNOSTICS row_cnt = ROW_COUNT;

	select public.load_log(l_load_id, l_step_id, 'is_busted updated', row_cnt, 'U')
		into l_step_id;
	...
	--	join instrument i on i.instrument_id=tr.instrument_id
		...
		--and trml.secondary_exch_exec_id is distinct from  'Manual Report'
	    --and tr.secondary_exch_exec_id  is distinct from  'Manual Report'
		and tr.is_busted='N'
		and trml.is_busted='N'--trml.is_busted is null
		;
		...
		set trade_record_id=tr.trade_record_id,
		    mapping_logic = 3,
		    load_batch_id  = in_load_batch_id--3333--in_load_batch_id
		from genesis2.trade_record tr
		where  tr.date_id  between in_start_date and in_end_date --20250204 and 20250204--
		and date_trunc('second',tr.trade_record_time)=date_trunc('second',trml.trade_record_time)
		and	tr.account_id=trml.account_id
		and tr.date_id=trml.date_id
		...
		and trml.trade_record_id is null
		and tr.is_busted='N'
		and trml.is_busted='N'--and trml.is_busted is null
	;
	...
	-- OMS_EDW case fuzzy matching logic 1
		...
		and trml2.is_busted = 'N'--and trml.is_busted is null
		and trml.load_batch_id  = in_load_batch_id
	    ...
	    and trml.is_busted ='Y'--and trml.is_busted is not null
	   ;
	...
	--				and (trml.is_busted = 'Y' or trml.blaze_account_alias is not null)
	                and ( (tr.is_busted = 'N' and trml.is_busted ='Y')
                        ...
                        or (trml.blaze_account_alias is distinct from tr.blaze_account_alias and
                            trml.blaze_account_alias is not null and tr.is_busted = 'N' and
                            tr.account_id in (select account_id from genesis2.account where trading_firm_id = 'strategas')
