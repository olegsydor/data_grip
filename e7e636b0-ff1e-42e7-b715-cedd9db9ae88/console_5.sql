-- DROP FUNCTION genesis2.reporting_export_get_occ_trade_data(date);

CREATE FUNCTION dash360.report_rps_occ_trade_data(in_start_date_id int4, in_end_date_id int4)
    RETURNS table
            (
                ret_row text
            )
...
BEGIN
    return query
        select 'put_call,symbol,exp_date,strike,side,lastqty,opposing_clear_mbr,last_px,occ_exchange_code,cmta_exec_gvup,account_type,opposing_broker,opposing_sub_account,client_order_id,account,report_id::text,c_canceled::text,contra_give_cmta,trade_date_time,account_name,parent_client_order_id';
    return query
        select array_to_string(ARRAY [
                                   ...
                                   side::text,
                                   lastqty::text,
                                   opposing_clear_mbr::text,
                                   last_px::text,
                                   occ_exchange_code,
                                   cmta_exec_gvup::text,
                                   account_type::text,
                                   opposing_broker,
                                   opposing_sub_account,
                                   client_order_id,
                                   account,
                                   report_id::text,
                                   c_canceled::text,
        ...
        where trade_date_time::date between in_start_date_id::text::date and in_end_date_id::text::date
        order by report_id;
END;
$function$
;
select min(trade_date_time) from genesis2.occ_trade_data
                            dash360.report_rps_occ_trade_data(in_start_date_id := 20231130, in_end_date_id := 20231130)


create index occ_trade_data_trade_date_id_idx on genesis2.occ_trade_data (trade_date_time)



select book_record_type_id, sum(amount)
from (select t.book_record_type_id,
             amount,
             row_number() over (partition by t.trade_record_id, t.book_record_type_id order by priority asc) as drank
      from genesis2.trade_level_book_record t
               inner join genesis2.trade_record ftr
                          on t.trade_record_id = ftr.trade_record_id
                              and t.date_id = ftr.date_id
                              and ftr.date_id between 20231101 and 20231130
--                               and ftr.date_id = 20231101
                              and ftr.is_busted = 'N'
               inner join lateral(select priority from genesis2.book_record_creator cr where t.book_record_creator_id = cr.book_record_creator_id limit 1) cr on true
      where t.date_id between 20231101 and 20231130
--    and ftr.trade_record_id = 3270506230
        and t.book_record_type_id in ('AMTK', 'ARTY', 'ATF', 'ATPF', 'CLRF', 'TADC', 'TMTK', 'TRTY', 'TTF',
                                      'TTPF') -- only those pulled from EDW to TLBR
     ) x
where x.drank = 1
...
WITH
	ftr_clear AS materialized (
	SELECT
		l.date_id, l.cmta, l.open_close, l.order_id, l.instrument_id, l.side, sum(last_qty) AS day_cum_qty,
		CASE sum(last_qty) WHEN 0 THEN NULL ELSE sum(last_qty * last_px )/ sum(last_qty) END AS avg_px,
		max(opt_customer_firm) AS customer_or_firm_id,
		l.opt_is_fix_clfirm_processed, l.opt_customer_or_firm, l.opt_nickel_commission, l.opt_penny_commission, l.opt_is_fix_custfirm_processed /*,
		max(street_account_name) as street_account_name*/
	FROM
		(
		SELECT
			ftr.date_id AS date_id,
			CASE WHEN (ci.clearing_instr_id IS NOT NULL OR acc.opt_is_fix_clfirm_processed = 'Y') THEN ftr.cmta ELSE NULL END AS cmta,
			ftr.open_close,	ftr.order_id AS order_id, ftr.instrument_id, ftr.account_id, ftr.side,
			CASE WHEN ci.clearing_instr_id IS NULL THEN ftr.last_qty ELSE cie.last_qty END AS last_qty,
			CASE WHEN ci.clearing_instr_id IS NULL THEN ftr.last_px ELSE cie.last_px END AS last_px,
			CASE WHEN ci.clearing_instr_id IS NULL THEN ftr.opt_customer_firm ELSE cie.opt_customer_firm END opt_customer_firm,
			CASE WHEN ci.clearing_instr_id IS NULL THEN 0 ELSE 1 END AS is_cleared,
			acc.opt_is_fix_clfirm_processed, acc.opt_customer_or_firm, acc.opt_nickel_commission, acc.opt_penny_commission, acc.opt_is_fix_custfirm_processed /*,
			coalesce(ftr.street_account_name, '') as street_account_name*/
		FROM
			genesis2.trade_record ftr
		JOIN genesis2.account acc ON	(acc.account_id = ftr.account_id AND acc.is_deleted <> 'Y' AND acc.opt_report_to_mpid = 'MLCB' AND acc.trading_firm_id <> 'cantor')
		LEFT JOIN genesis2.clearing_instruction_entry cie ON (cie.date_id = ftr.date_id	AND COALESCE(cie.new_trade_record_id, cie.trade_record_id) = ftr.trade_record_id AND cie.cmta IS NOT NULL)
		LEFT JOIN genesis2.clearing_instruction ci ON (ci.clearing_instr_id = cie.clearing_instr_entry_id AND ci.status = 'D' AND ci.is_deleted <> 'Y')
		WHERE
			ftr.date_id = :l_date_id
			AND is_busted = 'N'
			AND ftr.order_id > 0
			and ftr.exec_broker = :in_exec_broker
			AND NOT EXISTS (SELECT NULL FROM genesis2.alloc_instr2trade_record t
			WHERE date_id = :l_date_id
				AND t.trade_record_id = ftr.trade_record_id
				--and t.is_active
			)
		)l
	GROUP BY l.date_id, l.cmta, l.open_close, l.order_id, l.instrument_id, l.side, l.opt_is_fix_clfirm_processed, l.opt_customer_or_firm,
			l.opt_nickel_commission, l.opt_penny_commission, l.opt_is_fix_custfirm_processed
	)
	SELECT
		'DAS'||','||--Branch
		CASE
...
--		street_account_name ||','||--Client Identifier
		...
		to_char(((CASE coalesce(gen.min_tick_increment, 0.01) WHEN 0.01 THEN gen.opt_penny_commission WHEN 0.05 THEN gen.opt_nickel_commission END) * gen.alloc_qty), 'FM99990D0')||','||--13
		''||','||--Liquidity
		'S'||','||--Single/Basket
		''||','||--Pass Through Fees
		...
		gen.open_close||','||
		CASE (CASE gen.opt_is_fix_custfirm_processed WHEN 'Y' THEN coalesce(gen.opt_customer_firm, gen.opt_customer_or_firm) ELSE gen.opt_customer_or_firm END)
				WHEN '0' THEN 'C'
				WHEN '1' THEN 'F'
				WHEN '2' THEN 'F'
				WHEN '3' THEN 'C'
		...
		FROM
	(
	SELECT alin.side, ae.alloc_qty, alin.avg_px, acc.opt_is_fix_clfirm_processed, ftr.cmta AS ftr_cmta, ca.cmta AS ca_cmta, alin.date_id, os.min_tick_increment,
	acc.opt_penny_commission, acc.opt_nickel_commission, os.root_symbol, oc.put_call, oc.maturity_year, oc.maturity_month, oc.maturity_day, oc.strike_price, alin.open_close,
	acc.opt_is_fix_custfirm_processed, ftr.opt_customer_firm, acc.opt_customer_or_firm, coalesce(ae.occ_actionable_id,'') as occ_actionable_id --, ftr.street_account_name
    ...
    inner join lateral (select tr.cmta, tr.opt_customer_firm --, coalesce(tr.street_account_name,'') street_account_name
                       from alloc_instr2trade_record aitr
                       inner join trade_record tr on aitr.trade_record_id =tr.trade_record_id and aitr.date_id =tr.date_id and tr.is_busted ='N' and tr.exec_broker = :in_exec_broker
                       where aitr.alloc_instr_id = alin.alloc_instr_id
                         and aitr.date_id=alin.date_id
                        limit 1) ftr on true
	JOIN genesis2.clearing_account ca ON (ca.clearing_account_id = ae.clearing_account_id /*AND ca.is_deleted <> 'Y'*/ AND ca.clearing_account_type = '1' AND ca.market_type = 'O')
	JOIN genesis2.account acc ON (acc.account_id = ca.account_id AND acc.is_deleted <> 'Y' AND acc.opt_report_to_mpid = 'MLCB' AND acc.trading_firm_id <> 'cantor')
...
--		ftr.street_account_name ||','||--Client Identifier
        ''||','||--Client Identifier
		to_char(row_number() OVER(), 'FM0000')||','||
		to_char(((CASE coalesce(OS.MIN_TICK_INCREMENT, 0.01) WHEN 0.01 THEN ftr.OPT_PENNY_COMMISSION WHEN 0.05 THEN ftr.OPT_NICKEL_COMMISSION END) *  ftr.day_cum_qty), 'FM99990D0')||','||--13
		''||','||--Liquidity
		'S'||','||--Single/BASket
		''||','||--PASs Through Fees
		...
		ftr.open_close||','||
		CASE (CASE ftr.OPT_IS_FIX_CUSTFIRM_PROCESSED WHEN 'Y' THEN coalesce(ftr.CUSTOMER_OR_FIRM_ID, ftr.OPT_CUSTOMER_OR_FIRM) ELSE ftr.OPT_CUSTOMER_OR_FIRM END)
		WHEN '0' THEN 'C'
		WHEN '1' THEN 'F'
		WHEN '2' THEN 'F'
		WHEN '3' THEN 'C'
