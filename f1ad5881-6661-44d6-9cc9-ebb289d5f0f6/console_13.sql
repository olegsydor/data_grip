                     ...
                     from dwh.flat_trade_record tr
                              ...
                              left join lateral (select alloc_qty, atr.clearing_account_id, alloc_instr_id
                                                 ...
                                                 where atr.trade_record_id = tr.trade_record_id
                                                   ...
                                                   and atr.is_active
                                                 limit 1) at on true
                              left join t_aie aie on aie.alloc_instr_id = at.alloc_instr_id and
                                                     aie.clearing_account_id = at.clearing_account_id
                     where tr.date_id between in_date_begin and in_date_end
                       and is_busted = 'N'
                       and tr.order_id > 0
                       and case
                               when in_trading_firm_id is null then true
                               else acc.trading_firm_id = in_trading_firm_id end
...
--              and tr.trade_record_reason = 'L'
                     group by tr.date_id, tr.open_close, tr.instrument_id, tr.account_id, tr.side, tr.cmta,
                              tr.account_nickname, at.alloc_qty, aie.occ_actionable_id --tr.street_account_name,
        )
        select date(ftr.date_id::text)                                        as "Date",
               ...
               round(ftr.tcce_royalty_fee_amount / ftr.sum_last_qty * coalesce(ftr.alloc_qty, ftr.sum_last_qty),
                     6)                                                       as "Royalty",
               ftr.cmta,
               ftr.occ_actionable_id                                          as "OCC AID"
