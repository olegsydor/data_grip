...
where account_name ilike '%socgen%'
select * from dwh.d_trading_firm
    where trading_firm_id ilike '%socgen%'
and trading_firm_unq_id != all('{4119, 4120,368281}')
...
/*
 We need to create a custom OBO file for Soc Gen.
Trading Firms:
Societe Generale Financial EQD - ID socgeneqdUAT first, then in prod.
Societe Generale Prime Services Clearing - ID socgenps

We need to filter for low touch flow only by only including orders with 21=1 and excluding orders from any DASHOMS compID.
We should create it in UAT first, then in prod.
UAT and PROD DASHOMS compIDs listed in comments.
...
begin
    ...
    select public.load_log(l_load_id, l_step_id,
                           'dash360.report_obo_compliance_sg for ' || l_date_begin_id::text || ' - ' ||
                           l_date_end_id::text ||
                           ' STARTED===', 0, 'O')
    into l_step_id;
...
-- Account Details
               ...
               is_affiliated                                                 as "Is Affiliated",
               solicitation_flag                                             as "Solicitation Flag"
        from (select *
              from t_exs
