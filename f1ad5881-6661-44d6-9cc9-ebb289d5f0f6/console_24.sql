-- DROP FUNCTION trash.load_parent_order_inc(_int8, int4, _int8);
...
AS $function$
    ...
    -- SO: 20260114 https://dashfinancial.atlassian.net/browse/DS-10567 added order_statys
declare
    l_row_cnt int4;
    l_load_id int8;
    l_step_id int4;
...
with ts as (select x,
            ...
            from generate_series('2026-01-13 00:00', '2026-01-14 00:00', '2 minutes'::interval) as x)
-- select x, lead(x) over (order by 1) as y from ts
select date_id, array_agg(load_batch_id) ids
from (select date_id, ts.x, ts.y, load_batch_id
...
--                             and par.create_date_id = get_dateid(base.parent_order_process_time)
         ...
         left join lateral (select order_status
                            from dwh.execution exi
                            where exi.order_id = par.order_id
                              and exi.exec_date_id >= base.create_date_id
--                                   and exi.exec_type in ('F', '0', 'W')
                            order by exec_id desc
                            limit 1) exi on true
where true
  and par.parent_order_id is null
    ...
    min_exec_id,
    max_exec_id,
    parent_order_process_time,
    order_create_date_id,
    (select order_status
      from dwh.execution exi
        where exi.order_id = parent_order_id
          and exi.exec_date_id = :l_date_id
       order by exec_id desc
       limit 1) as order_status
    from ( select cl.parent_order_id,
           min(ex.exec_id)                   as min_exec_id,
           max(ex.exec_id)                   as max_exec_id,
           min(cl.parent_order_process_time) as parent_order_process_time,
           min(ex.order_create_date_id)      as order_create_date_id
