...
--            ,
--            string_agg(ex.order_status, ',' order by exec_id desc)
    from dwh.execution ex
             join dwh.client_order cl on cl.order_id = ex.order_id and cl.create_date_id = ex.order_create_date_id
    where ex.exec_date_id = 20260123
      and cl.parent_order_id = 414905501630994280
...
--     raise notice 't_street_orders create - %', l_row_cnt;
 ...
 insert into data_marts.f_parent_order (parent_order_id, last_exec_id, create_date_id, status_date_id,
    ...
    select tp.parent_order_id,
           ...
           tp.side,
           md5(row (20260123, tp.max_exec_id, tp.street_count, tp.create_date_id, tp.trade_count, tp.last_qty, tp.amount, tp.street_order_qty, tp.parent_order_qty, tp.instrument_id)::text) as check_sum,
           tp.order_status
    from t_parent_orders tp
             left join data_marts.f_parent_order fp
                       on fp.parent_order_id = tp.parent_order_id and fp.status_date_id = 20260123
