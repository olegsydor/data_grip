...
AS SELECT da.account_id,
    ...
    i.instrument_types,
        CASE
            ...
            WHEN da.eq_report_to_mpid::text = 'MLCB'::text AND (COALESCE(cl_acc.eq_clearing_account_number, 'null alternative'::text) <> ALL (ARRAY['3Q800806'::text, '3Q800797'::text, '3Q800809'::text])) THEN 'Y'::text
            WHEN (EXISTS ( SELECT NULL::text
               FROM d_sg_account sg
              WHERE sg.account_id = da.account_id AND sg.is_active AND sg.sg_parent_account_id IS NULL)) THEN 'Y'::text
            ELSE 'N'::text
   ...
   FROM d_account da
     ...
     LEFT JOIN ( SELECT max(
          ...
          WHERE d_clearing_account.is_default = 'Y'::bpchar AND d_clearing_account.is_active
          GROUP BY d_clearing_account.account_id, d_clearing_account.is_visible_for_manual_allocation) cl_acc ON cl_acc.account_id = da.account_id
  WHERE da.is_active;