...
begin
    ...
    from dwh.gtc_order_status gtc
             join dwh.client_order str
                  on str.order_id = gtc.order_id and str.create_date_id = gtc.create_date_id
             join lateral (select to_char(iex.exec_time, 'YYYYMMDD')::int4 as close_date_id,
                                  iex.order_status
                           from dwh.execution iex
                           where true
...
-- DROP FUNCTION trash.so_gtc_update_daily(int4, int4);
...
begin
    ...
    from dwh.gtc_order_status gtc
             join dwh.client_order str
                  on (str.order_id = gtc.order_id and str.create_date_id = gtc.create_date_id and str.create_date_id >= 20220001)
             join lateral (select to_char(iex.exec_time, 'YYYYMMDD')::int4 as close_date_id,
                                  iex.order_status
                           from dwh.execution iex
                           where true
...
-- DROP FUNCTION trash.so_gtc_update_daily(int4, int4);
alter function dwh.gtc_update_daily rename to gtc_update_daily_old;
alter function dwh.gtc_update_daily_old set schema trash;

create or replace function dwh.gtc_update_daily(p_start_date_id integer default null::integer,
