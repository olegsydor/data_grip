...
CREATE OR REPLACE FUNCTION dash360.report_obo_compliance_xls(in_date_begin_id integer,
                 ...
                 RETURNS TABLE
                         (
                             ...
                             "Exec Link ID"              character varying
                         )
                 LANGUAGE plpgsql
             AS
             $function$
...
-- SO: 20260519 https://dashfinancial.atlassian.net/browse/DEVREQ-8358 Add New ExecLinkID Column to the OBO Compliance file
             declare
                 l_load_id           int;
                 l_step_id           int;
             ...
             begin
                 ...
                 if coalesce(in_account_ids, '{}') = '{}' and coalesce(in_trading_firm_ids, '{}') = '{}' then
                     ...
                     where true
                       ...
                       and case
                               when coalesce(in_account_ids, '{}') <> '{}'::integer[]
                                   then account_id = ANY (in_account_ids)
                               else true end;
                 end if;
                 ...
                 from t_base b
                          join ord_type ot
                               on ot.trans_type = b.trans_type and
                                  case when in_include_routes = 'Y' then true else ot.rn = 1 end
                          left join lateral
                     ( select ex.exec_id,
                              ex.order_status,
                              ex.exec_type,
...
-- Account Details
                            ...
                            mleg_exec_id                                                  as "Exec Link ID"
                     from (select *
-- into trash.so_obo
                           from t_exs
                           where case
                                     when exec_type in ('A', '0', '5', 'b') and event_ts is null then false
                                     else true end
