...
-- OS: 20260119 https://dashfinancial.atlassian.net/browse/DS-10976
    select  ' '
...
left join (
                select max(clearing_instr_id) as clearing_instr_id,
                coalesce(new_trade_record_id, trade_record_id) as trade_record_id,
                max(clearing_submitted_away) as clearing_submitted_away
...
select * from clearing_instruction