...
CREATE OR REPLACE FUNCTION trash.get_data_for_allocation_drop(in_alloc_instr_id bigint, in_date_id integer DEFAULT NULL::integer)
    RETURNS jsonb
    LANGUAGE plpgsql
AS
$function$
...
begin
    ...
    from genesis2.allocation_instruction ai
             ...
             join lateral (select count(*) as alloc_cnt,
                                  jsonb_agg(jsonb_build_object('allocAccount', x.opt_occ_id,
                                                               'allocQty', x.alloc_qty,
                                                               'clrFirm', x.clearing_account_number,
                                                               'actionableId', x.occ_actionable_id,
                                                               'brid', x.sg_brid,
                                                               'subAccount', x.sg_sub_account_name,
                                                               'individualAllocID', x.allocation_instruction_entry_id,
                                                               'sgMintAccount', x.sg_mint_account,
                                                               'AllocEntryCCRURate', ccr.rate,
                                                               'AllocEntryCCRUTotalAmount',
                                                               alloc_entry_ccru_total_amount --ccr.amount * 1.0 * x.alloc_qty / total_qty
                                            ))
                                           as entries
                           from (select ac.opt_occ_id,
                                        aie.alloc_qty,
                                        ca.clearing_account_number,
                                        aie.occ_actionable_id,
                                        ca.sg_brid,
                                        ca.sg_sub_account_name,
                                        aie.allocation_instruction_entry_id,
                                        ca.sg_mint_account,
                                        case
                                            when aie.allocation_instruction_entry_id =
                                                 last_value(aie.allocation_instruction_entry_id) over w
                                                then amount -
                                                     (sum(round(ccr.amount * 1.0 * aie.alloc_qty / ai.total_qty, 4))
                                                      over x)
                                            else round(ccr.amount * 1.0 * aie.alloc_qty / ai.total_qty, 4) end as alloc_entry_ccru_total_amount
...
--                                                  and ca.market_type = di.instrument_type_id
                                 ...
                                 where aie.alloc_instr_id = ai.alloc_instr_id
                                   and aie.date_id = ai.date_id
                                 window w as ( partition by aie.alloc_instr_id order by aie.alloc_qty
                                         rows between unbounded preceding and unbounded following ),
                                        x as ( partition by aie.alloc_instr_id order by aie.alloc_qty
                                                rows unbounded preceding exclude current row )) x
                           limit 1) aie on true
