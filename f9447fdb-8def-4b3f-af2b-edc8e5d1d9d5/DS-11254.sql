-- DROP FUNCTION trash.allocations_snapshot(_int8, int4, bpchar);
...
CREATE OR REPLACE FUNCTION trash.allocations_snapshot(in_account_ids bigint[] DEFAULT '{}'::bigint[],
                                                      ...
                                                      in_hide_non_customer_bphops bool default false)
    RETURNS TABLE
            (
                date_id                    integer,
...
--     raise notice '3 - %', clock_timestamp();
    ...
    return query
        select ai.date_id,
               ...
               case when array_length(ccr.client_order_id, 1) > 1 then '-' else ccr.client_order_id[1] end
...
--and false
          ...
          and case
                  ...
                  when :in_reported_status is null then true end
          and case
                  when :in_hide_non_customer_bphops and (chain_id is not null and chain_id != tr.client_order_id) then false
                  else true end;
