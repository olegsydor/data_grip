-- DROP FUNCTION dash360.allocations_snapshot(_int8, int4, bpchar, bool, _bpchar, _bpchar);
SELECT *
FROM dash360.allocations_snapshot(in_date_id := 20260827, in_hide_non_customer_bphops := FALSE);
...
CREATE OR REPLACE FUNCTION dash360.allocations_snapshot(in_account_ids bigint[] DEFAULT '{}'::bigint[],
    ...
    RETURNS TABLE
            (
                ...
                client_order_status        character,                   -- 35
                ...
                lifecycle_order_id         character varying,           -- 40
...
--and false
    ...
    return query
        select ai.date_id,
               ...
               null::character                                   as lifecycle_order_state
...
--                           and tri.exch_exec_id = tr.exch_exec_id
        ...
        from genesis2.trade_record tr
                 ...
                 inner join genesis2.allocation_instruction ai on (ai.alloc_instr_id = ai2tr.alloc_instr_id)
...
-- SY 20260707 https://dashfinancial.atlassian.net/browse/DS-11722 Inherit CCRU using current book_record_creator instead on MAN
    -- SO 20280810 https://dashfinancial.atlassian.net/browse/DS-11870 Adjust allocation procedures to Lifecycle Order ID and Lifecycle Order Status
    -- SO 20280827 https://dashfinancial.atlassian.net/browse/DS-11870 The same ticket once again: removing lifecycle columns from the allocation part (not in this script) and removing client_order_status at all
declare
    l_user_id              int;
    l_system_id            varchar;
...
-- dash360.get_trade_records_for_allocation - no changes
