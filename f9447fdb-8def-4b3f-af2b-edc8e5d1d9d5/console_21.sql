create table if not exists staging.bofa_allocation_report_history
(
    bofa_allocation_report_id int4 not null
        constraint bofa_allocation_report_pk primary key generated by default as identity,
    report_row                text,
    dataset                   int4,
    report_part               bpchar,
    db_create_time            timestamp default clock_timestamp()
);
comment on table staging.bofa_allocation_report_history is 'The table yo save generated reports by dash360.bofa_allocation_report';
comment on column staging.bofa_allocation_report_history.bofa_allocation_report_id is 'PK';
comment on column staging.bofa_allocation_report_history.report_row is 'report''s row irself';
comment on column staging.bofa_allocation_report_history.dataset is 'dataset from the report';
comment on column staging.bofa_allocation_report_history.report_part is 'A for allocations, T for trades';
comment on column staging.bofa_allocation_report_history.db_create_time is 'creation time';

create index on staging.bofa_allocation_report_history (dataset, report_part);
