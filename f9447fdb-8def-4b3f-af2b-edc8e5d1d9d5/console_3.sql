...
-- SY: 20230613 https://dashfinancial.atlassian.net/browse/DS-6846 OMS_EDW trades processing has been blocked (temporary)
...
if in_exec_id is null
 then
	...
	into l_max_exec_id from trade_record;
...
--	SELECT COALESCE(max(exec_id),0) FROM staging.execution where exec_id>=l_max_exec_id into l_foreign_max_exec_id;
...
--	perform pg_sleep(5);
	perform case when clock_timestamp()::time between '09:30'::time and '10:00'::time then pg_sleep(5)
				 else pg_sleep(3)
	end;
	...
	select dimension_etl() into some_var;
...
IF l_foreign_max_exec_id> l_max_exec_id /* 1=1*/ THEN
	...
	insert into trade_record (trade_record_time, db_create_time, date_id,
		    ...
		    street_account_name, street_exec_broker, trade_text, branch_sequence_number, frequent_trader_id, int_liq_source_type, time_in_force,
	...
	, staging.get_trade_liquidity_indicator(TRADE_LIQUIDITY_INDICATOR)
	, SECONDARY_ORDER_ID
	, EXCH_EXEC_ID
    ...
    , case i.instrument_type_id
        when 'O' then genesis2.get_street_exec_broker(v.street_order_fix_message_id, v.date_id::int, v.exchange_id, v.street_client_order_id, v.multileg_reporting_type::smallint, v.street_is_cross_order::char)
        else null
...
if 1=2 and current_time < '06:30'::time
 then
	...
	select COALESCE(max(exec_id),0)
	into l_arch_max_exec_id from trade_record
...
/*	l_status_message:='l_arch_foreign_max_exec_id = ';
	...
	insert into trade_record (trade_record_time, db_create_time, date_id,
		    ...
		    street_client_order_id, fix_comp_id, LEAVES_QTY, street_exec_inst, fee_sensitivity,
		    strategy_decision_reason_code, compliance_id, floor_broker_id, AUCTION_ID,
		    street_opt_customer_firm, multileg_order_id, frequent_trader_id, int_liq_source_type, time_in_force
...
if in_exec_id is null
 then
	select genesis2.load_log(l_load_id, l_step_id, 'AWAY TRADES processing started', 0, 'I')
	into l_step_id;
...
into l_away_trade_ids
from etl_subscriptions es
where not is_processed
and subscription_name='trade_record_away_trade'
and source_table_name ='trade_desk';

if l_away_load_batch is not null or l_away_trade_ids is not null
 then
...
--    and not trml.is_sor_routed /* SY implemented 20200914 https://dashfinancial.atlassian.net/browse/DS-1936 */
--    and date_id = l_date_id
--    and load_batch_id <> -1
--    and load_batch_id < l_away_load_batch
...
--    and trml.is_busted ='N'
    ...
    where trade_record_id is null
    and instrument_id is not null
    and (  (generation = 0 and not is_sor_routed )/* ordinar case non sor routed trades */
		    or (generation = 0 and is_sor_routed and num_firms>1 and coalesce(nullif(secondary_exch_exec_id,''), 'Manual Report') <> 'Manual Report' ) /* routed to sor with company name changes*/
		    or (generation>0 and is_company_name_changed =1 and not is_sor_routed and coalesce(nullif(secondary_exch_exec_id,''), 'Manual Report') <> 'Manual Report' ) /* non-routed to SOR company name chaned in firther generation*/
		    or (generation>0 and is_company_name_changed =1 and is_sor_routed and mx_gen>generation and coalesce(nullif(secondary_exch_exec_id,''), 'Manual Report') <> 'Manual Report' )
		)
    and date_id = l_date_id
    and load_batch_id <> -1
 --   and subsystem_id not in ('OMS_EDW')
    and ( (load_batch_id < l_away_load_batch and subsystem_id not in ('PG_DASH')) or
...
on conflict (date_id,
  ...
  ;
	...
	select genesis2.load_log(l_load_id, l_step_id, 'AWAY TRADES INSERT INSERTED load_batch_id=', l_cnt, 'I')
	into l_step_id;

if cardinality (l_away_trade_ids) >0
  then
   update etl_subscriptions
   set is_processed = true,
       process_time = clock_timestamp()
   where load_batch_id  = any(l_away_trade_ids)
    ...
    select genesis2.load_log(l_load_id, l_step_id, 'TRADE DESK subscriptions closed =', l_cnt, 'U')
	into l_step_id;
   end if; -- cardinality (l_away_trade_ids) >0


end if; -- l_away_load_batch is not null or l_away_trade_ids is not null
...
into l_step_id;
 ...
 exception when others then
   select load_log(l_load_id, l_step_id, left(sqlstate||': '||REPLACE(sqlerrm, ''::text, ''::text),200), 0, 'E')
  into l_step_id;
  ...
  PERFORM load_error_log('trade_record',  'I', REPLACE(sqlerrm, ''::text, ''::text), l_load_id);
  RAISE;
