...
left join lateral (select sum(/*coalesce(*/ l1.rate /*,0)*/ *tr.last_qty)/nullif(sum(tr.last_qty),0) as rate, sum(amount) as amount
...
--					        FROM alloc_instr2trade_record  alt
--					        inner join trade_level_book_record tl on tl.trade_record_id  = alt.trade_record_id
--					        inner join book_record_creator  cr on tl.book_record_creator_id  = cr.book_record_creator_id
--					        WHERE tl.date_id = in_date_id
--					        AND book_record_type_id ='CCRU'
...
--                     from trade_level_book_record tlbr
--                      inner join alloc_instr2trade_record  alt on tlbr.trade_record_id  = alt.trade_record_id
--                      where alt.alloc_instr_id = a.alloc_instr_id
--                      and tlbr.book_record_type_id ='CCRU'
--                      limit 1
--                   ) ccr on true
