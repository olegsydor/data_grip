...
CREATE /*OR REPLACE*/ FUNCTION dash360.bofa_allocation_report_v3(in_start_date_id integer, in_end_date_id integer, in_exec_broker text, in_is_eod boolean DEFAULT false, in_removed_account_ids integer[] DEFAULT '{62939,263022,62810,62887,62923,63787,67949}'::integer[])
...
AS $function$
    ...
    -- 20260713 SO fix of bug of adding busts with\without original alloc_instr_ids
    -- 20260731 SO https://dashfinancial.atlassian.net/browse/DS-11858 busts cannot be shown more than once

declare
    l_load_id                 int;
    l_step_id                 int;
    l_alloc_instr_id_reported int4[];
    l_alloc_instr_id_busted   int4[];
...
--                                                                 and tr.is_busted = 'N'
            ...
            where alin.date_id between in_start_date_id and in_end_date_id
              ...
              -- if this alloc_instr_id was prev reported then false
...
-- ONLY add busts if the original record (not busted) was reported
    insert into dash_reporting.bofa_allocation_report
...
--                                                                 and tr.is_busted = 'N'
    ...
    where alin.date_id between in_start_date_id and in_end_date_id
      and ca.account_id = any (l_account_ids)
      and alin.is_deleted = 'Y'
      and alin.alloc_instr_id = any (l_alloc_instr_id_reported) -- this alloc_instr_id was reported before
      and alin.alloc_instr_id != all (l_alloc_instr_id_busted) -- this alloc_instr_id never was shown as busted before
;
...
--                                    to_char(row_number() OVER () , 'FM0000') , --
                                   to_char(gen.allocation_instruction_entry_id % 100000000,  'FM00000000') , --
