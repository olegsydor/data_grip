...
CREATE OR REPLACE FUNCTION genesis2.load_away_trade(in_min_report_id text DEFAULT NULL::text,
                                                    in_max_report_id text DEFAULT NULL::text)
    RETURNS integer
    LANGUAGE plpgsql
AS
$function$
    -- 20241217 SO added account_name_gvp and mic_code
-- 2026-811 SO https://dashfinancial.atlassian.net/browse/DS-11877 add new column blaze_order_owner
declare
...
begin
    ...
    select into l_last_loaded_report_id coalesce(in_min_report_id, report_id)
    ...
    if l_last_loaded_report_id is null then
        l_last_loaded_report_id := coalesce(in_min_report_id, 'loeuhlgk0000');
    end if;
    ...
    select public.load_log(l_load_id, l_step_id, 'load_away_trade temp table created', l_row_cnt, 'O')
    into l_step_id;
    ...
    insert into staging.trade_record_blaze7(trade_record_time, date_id, is_busted, subsystem_id, account_name, side,
                                            open_close,
                                            trade_liquidity_indicator, secondary_order_id, secondary_exch_exec_id,
                                            ...
                                            opt_customer_firm,
                                            is_cross_order, street_is_cross_order, contra_broker, client_id,
                                            order_price,
                                            order_process_time, remarks, street_client_order_id, fix_comp_id,
                                            leaves_qty,
                                            leg_ref_id,
                                            load_batch_id, strategy_decision_reason_code, is_parent, symbol,
                                            strike_price,
                                            ...
                                            activ_symbol, blaze_account_alias, is_sor_routed, is_company_name_changed,
                                            companyname,
                                            generation, mx_gen, status, transactiondatetime, legnumber, excode,
                                            systemid,
                                            user_id,
                                            systemordertypeid, companyid, exdestination, client_order_id,
                                            parent_client_order_id,
                                            contra_client_order_id, orig_client_order_id, report_id, child_exec_id,
                                            order_id,
                                            chain_id, client_order_id_guid, report_id_guid, exec_id,
                                            account_name_gvp, mic_code, order_id_guid,
                                            commission_rate_unit, real_flex_order, account_id, exchange_id, num_firms,
                                            is_flex_order,
                                            exch_exec_id, manual_broker, lifecycle_order_id, lifecycle_order_id_status,
                                            blaze_order_owner)
    with cte
             as (select coalesce(aw.manualexecutiontime, aw.transactiondatetime)::timestamptz                 as trade_record_time, -- check timezone
...
-- 			else aw.StockQuantity end,0),aw.Quantity) as street_order_qty,
                        ...
                        coalesce(aw.manualexecutiontime, aw.transactiondatetime)::timestamptz                 as order_process_time,
                        coalesce(aw.dashaliasid,
                                 case
                                     when coalesce(us.aorsusername, us.login) = 'BBNTRST' then 'NTRSCBOE'
                                     else coalesce(us.aorsusername, us.login)
                            ...
                            when aw.expirationdate is not null and aw.strike IS NOT NULL
                                THEN replace(
                                    COALESCE(((((regexp_replace(COALESCE(aw.basecode, ''::text), '\.|-'::text, ''::text,
                                                                'g'::text) ||
                                                 ' '::text) ||
                                                to_char(aw.expirationdate::timestamp with time zone,
                                                        'DDMonYY'::text)) ||
                                               ' '::text) || staging.trailing_dot_away(aw.strike)) ||
                                             "left"(aw.typecode, 8),
                                             CASE
                        ...
                        aw.accountalias                                                                       as blaze_account_alias,
                        case
                            when aw.orderid like 'f_%' then true
                            when
                                CASE
                                    WHEN coalesce(los.EDWID, bos.ID, 0) = 151 and aw.OrderReportSpecialType = 'M'
                                        then 156
                                    ...
                                    (select null
                                     ...
                                     where (CatDestinationID = 'DFIN' or ExDestination like '%PAR')
                                       and coalesce(lm.Ex_Destination, aw.ExDestination) = sor.exdestination) then true
                            else false
                        ...
                        first_value(aw.accountalias) over (partition by CASE
                                                                            WHEN aw.OrderReportSpecialType = 'M'
                                                                                THEN 'Manual Report'
                                                                            ELSE aw.ExchangeTransactionID end, aw.lastshares::int--, aw.side
                            order by case
                                         when aw.generation::int = 1 and aw.ParentOrderID is null then 0
                                         else aw.generation::int end desc)                                    as blaze_account_alias_strategas,
...
--                      aw.childorderid                                                                              as child_client_order_id,
                        ...
                        '-1'::integer * staging.base32_to_int8(aw.reportid)                                   AS exec_id,
                        ...
                        case
                            when nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination), '') in
                                 ('CBOE-CRD NO BK', 'PAR', 'CBOIE')
                                then 'XCBO'
                            when nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination), '') in
                                 ('XPAR', 'PLAK', 'PARL') then 'LQPT'
                            when nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination), '') in
                            ...
                            when nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination), '') in
                                 ('C2', 'CBOE2') then 'C2OX'
                            when nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination), '') = 'SMARTR'
                                then 'COWEN'
                            when nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination), '') in
                                 ('ACT', 'BOE', 'OTC', 'lp', 'VOL')
                                then 'BRKPT'
                            when nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination), '') in ('XPSE')
                                then 'ARCO'
                            when nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination), '') = 'TO'
                                then 'AMXO'
                            else nullif(coalesce(den1.mic_code, lm.Ex_Destination, aw.ExDestination),
                                        '') end                                                               as mic_code,
                        staging.clordid_to_guid(aw.orderid)                                                   as order_id_guid,
                        aw.commission_rate_unit,
                        case
                            when replace(replace(aw.BaseCode, '.', '/'), '-', '/') like '[0-9]%' then 1
                            else 0
                            end                                                                               as real_flex_order,
                        is_represented,
                        is_sg,
                        manual_broker,
                        aw.lifecycle_orderid                                                                  as lifecycle_order_id,
                        aw.lifecycle_orderid_status                                                           as lifecycle_order_id_status,
                        aw.blaze_order_owner
-- select *
                 from t_blaze aw --staging.v_away_trade aw
                          ...
                          LEFT JOIN billing.tusers us
                                    on aw.UserID::int = us.USerID and us.SystemID = 2 and
                                       us.EDWActive = '1'::bit -- USER
                          ...
                          left join billing.dash_exchange_names den
                                    on den.mic_code = regexp_replace(coalesce(lm.Ex_Destination, aw.ExDestination),
                                                                     '(DIRECT-| Printer)', '', 'g') and
                                       den.real_exchange_id = den.exchange_id and den.mic_code != '' and den.is_active
                          left join billing.dash_exchange_names den1
                                    on den1.exchange_id =
                                       regexp_replace(coalesce(lm.Ex_Destination, aw.ExDestination),
                                                      '(DIRECT-| Printer)', '', 'g') and
    ...
    select trade_record_time,
           ...
           last_qty,
           last_px,
           coalesce(l1.ex_destination, l2.ex_destination, cte.ex_destination, 'ALGO')         as ex_destination,
           coalesce(l1.ex_destination, l2.ex_destination, 'DMA')                              as sub_strategy,
           street_order_qty,
           order_qty,
           ...
           cte.instrument_type_id,
           activ_symbol,
--	cte.blaze_account_alias,
           coalesce(cte.blaze_account_alias,
                    case
                        when (a.trading_firm_id in ('strategas') or a2.trading_firm_id in ('strategas'))
                            then blaze_account_alias_strategas
                        else cte.blaze_account_alias end)                                     as blaze_account_alias,
           ...
           client_order_id,
           parent_client_order_id,
           contra_client_order_id,
           orig_client_order_id,
           report_id,
           child_exec_id,
           order_id,
           chain_id,
           client_order_id_guid,
           report_id_guid,
           exec_id,
           account_name_gvp,
           cte.mic_code,
           cte.order_id_guid,
           commission_rate_unit,
           real_flex_order--cte.*
            ,
           coalesce(a2.account_id, a.account_id, 16435)                                       as account_id,
           coalesce(de.exchange_id, l1.exchange_id, cte.mic_code)                             as exchange_id,
           sum(cte.is_company_name_changed) over (partition by secondary_exch_exec_id)        as num_firms,
           case when max(real_flex_order) over (partition by order_id) >= 1 then 1 else 0 end as is_flex_order,
           case when secondary_exch_exec_id = 'Manual Report' then report_id else null end    as exch_exec_id,
           manual_broker,
           lifecycle_order_id,
           lifecycle_order_id_status,
           blaze_order_owner
    from cte
             left join genesis2.account a on cte.account_name = a.account_name and a.is_deleted = 'N'
             left join genesis2.account a2 on cte.account_name_gvp = a2.account_name and a2.is_deleted = 'N'
             left join genesis2.exchange de
                       on cte.mic_code = de.mic_code and de.is_deleted = 'N' and de.real_exchange_id = de.exchange_id
             left join genesis2.d_ex_destination dex
                       on dex.exchange_id = de.exchange_id and dex.instrument_type_id = de.instrument_type_id
             left join lateral (select dex.ex_destination_code as ex_destination, de.exchange_id
                                from genesis2.exchange de
                                         join genesis2.d_ex_destination dex on dex.exchange_id = de.exchange_id
                                    and dex.instrument_type_id = de.instrument_type_id
                                where de.is_deleted = 'N'
                                  and dex.is_active
                                  and de.real_exchange_id = de.exchange_id
                                  and dex.ex_destination_code = cte.mic_code
                                limit 1
        ) l1 on true
             left join lateral (select mic_code, de.instrument_type_id, dex.ex_destination_code as ex_destination
                                from genesis2.exchange de
                                         join genesis2.d_ex_destination dex on dex.exchange_id = de.exchange_id
                                    and dex.instrument_type_id = de.instrument_type_id
                                where de.is_deleted = 'N'
                                  and dex.is_active
                                  and de.real_exchange_id = de.exchange_id
                                  and de.mic_code = cte.mic_code
                                limit 1
        ) l2 on true
    where Status in (151, 156, 239)
      and SystemOrderTypeID <> 87
      and not (is_represented is not distinct from 'Y' and is_sg is not distinct from 'SG')
      and (generation = 0
        or (generation > 0
            and is_company_name_changed = 1
            and not is_sor_routed
            and coalesce(nullif(secondary_exch_exec_id, ''), 'Manual Report') <> 'Manual Report'
               ) /* non-routed to SOR company name chaned in firther generation*/
        or (generation > 0
            and is_company_name_changed = 1
            and is_sor_routed
            and mx_gen > generation
            and coalesce(nullif(secondary_exch_exec_id, ''), 'Manual Report') <> 'Manual Report')
        );
