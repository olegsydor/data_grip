...
-- compare 2 UAT
CREATE OR REPLACE FUNCTION dash360.bofa_allocation_report(in_start_date_id integer, in_end_date_id integer, in_exec_broker text, in_is_eod boolean DEFAULT false)
...
begin
    ...
    as
    select distinct on (atr.trade_record_id, br.to_report, br.alloc_instr_id)
        atr.trade_record_id, br.to_report, br.alloc_instr_id, br.db_create_time, 'B' as alloc_rep_type
    from dash_reporting.bofa_allocation_report br
             join genesis2.alloc_instr2trade_record atr
                  on atr.alloc_instr_id = br.alloc_instr_id and atr.date_id = br.date_id
    where br.date_id = in_date_id
...
select * from dash360.so_allocations_snapshot(in_date_id := 20250109);
...
-- SY 20210531 https://dashfinancial.atlassian.net/browse/DS-3420 return type has been changed from id into query. The logic to modify trade_record has been implemented
...
-- OS 20250113 https://dashfinancial.atlassian.net/browse/DS-9407 Add new attrs is_prev_reported, opt_customer_firm
declare
    l_user_id              int;
    l_system_id            varchar;
...
begin
    ...
    with ct as ( update genesis2.allocation_instruction
        set
            is_deleted = 'Y',
            delete_time = 'now'::timestamp,
            deleted_by_user_id = in_user_id
        where alloc_instr_id = in_alloc_instr_id
        returning created_by_user_id, created_by_subsystem_id, date_id)
    select created_by_user_id, created_by_subsystem_id, date_id
...
--  raise info '%: l_user_id=%, l_system_id=%, date_id = %', clock_timestamp(),l_user_id, l_system_id, l_date_id ;
    ...
    then
		...
		inner join trade_record tr on aitr.trade_record_id = tr.trade_record_id and tr.is_busted ='N' and tr.date_id = aitr.date_id and trade_record_reason='L'
		inner join trade_record orig_tr on tr.orig_trade_record_id = orig_tr.trade_record_id  and tr.date_id = orig_tr.date_id
...
select * from dash360.so_allocations_instruction_delete(-54861, 5662);

-- DROP FUNCTION dash360.so_allocations_instruction_delete(int4, int4);
...
select genesis2.load_log(l_load_batch_id::int, l_step_id, 'allocations_instruction_delete STARTED =====', 0, 'S'::char)
	into l_step_id;
   ...
   /* UNALLOCATE MANUAL ALLOCATION with reverting trade_record changes*/
	...
	into l_step_id;
       ...
       raise info 'Revert vector is %', l_revert_vector;
	    ...
	    if l_revert_vector is not null
         then l_new_trade_record_ids:=dash360.ptm_process_trades(l_date_id, in_user_id, l_revert_vector);
--        then l_new_trade_record_ids:=trash.so_ptm_process_trades(l_date_id, in_user_id, l_revert_vector);

    	select genesis2.load_log(l_load_batch_id::int, l_step_id, 'cardinality(l_new_trade_record_ids): ', cardinality(l_new_trade_record_ids), 'S'::char)
    	into l_step_id;
         ...
         else select array_agg(trade_record_id::bigint)
              into l_new_trade_record_ids
              from alloc_instr2trade_record a
...
select genesis2.load_log(l_load_batch_id::int, l_step_id, 'allocations_instruction_delete returning query', 0, 'S'::char)
	into l_step_id;
...
return query
    select tr.date_id,
           ...
           case
               when true --btr.to_report in ('U', 'C')
                   and exists
                        (select null
                         from genesis2.alloc_instr2trade_record aitr
                                  join dash_reporting.bofa_allocation_report br
...
-- 		   into l_revert_vector
		...
		inner join trade_record orig_tr on tr.orig_trade_record_id = orig_tr.trade_record_id  and tr.date_id = orig_tr.date_id
		where true
--       and aitr.alloc_instr_id = -54584
	      and aitr.date_id = :l_date_id;

select '{"2346652518": [{"cmta": "NULL", "user_id": 1, "account_nickname": "NULL", "street_account_name": "NULL", "trade_record_reason": "U", "allocation_avg_price": "NULL", "clearing_account_number": "NULL"}], "2346652519": [{"cmta": "NULL", "user_id": 1, "account_nickname": "NULL", "street_account_name": "NULL", "trade_record_reason": "U", "allocation_avg_price": "NULL", "clearing_account_number": "NULL"}], "2346652520": [{"cmta": "NULL", "user_id": 1, "account_nickname": "NULL", "street_account_name": "NULL", "trade_record_reason": "U", "allocation_avg_price": "NULL", "clearing_account_number": "NULL"}]}'::jsonb

select jsonb_build_object(tr.trade_record_id, jsonb_build_object('clearing_account_number', orig_tr.clearing_account_number,
															  'account_nickname', orig_tr.account_nickname,
															  'street_account_name', orig_tr.street_account_name,
															  'cmta', orig_tr.cmta,
		      												  'allocation_avg_price', null,
															  'trade_record_reason', 'U',
															  'user_id', :in_user_id))
...
where alr.to_report = 'R'
  and alr.date_id = in_date_id
  and alr.alloc_instr_id = in_load_batch_id;


select trade_record_id
from dash_reporting.bofa_trade_record btr
where btr.date_id = in_date_id
  and dataset = in_load_batch_id
...
--     into l_alloc_instr_id_reported
 ...
 select
     alin.alloc_instr_id,
                   ...
                   case
                       when ar.date_id is not null then 'C' --'skip - current alloc_instr_id'
                       when or_ai.alloc_instr_ids && '{-55431}'
                           then 'U' -- 'unable to report - alloc_instr_id has been reported before'
                       else 'R' end as to_report,
     exec_broker,
     is_busted
...
select dash360.bofa_allocation_report(in_start_date_id := 20250116, in_end_date_id := 20250116,
                                                          in_exec_broker := '015',
                                                          in_is_eod := false);
...
select load_batch_id, subscription_id , date_id, *
                 from staging.etl_subscriptions
                 where true
                 and date_id = 20250116
                   and load_batch_id = 13560793
                     and load_batch_id in (2346662122,2346662123,2346662124)
...
;
 select tr.exec_broker,
        ...
        select tr.exec_broker,
               ...
               ui.user_name
        from genesis2.allocation_instruction ai
                 join lateral (select aitr.trade_record_id, bar.* from dash_reporting.bofa_allocation_report bar
                                        join genesis2.alloc_instr2trade_record aitr
                      on (aitr.alloc_instr_id = bar.alloc_instr_id and aitr.date_id = bar.date_id)
                      where ai.alloc_instr_id = bar.alloc_instr_id and ai.date_id = bar.date_id limit 1) bar on true

                 join lateral (select *
                               from genesis2.trade_record tr
                               where tr.trade_record_id = bar.trade_record_id
                                 and tr.date_id = bar.date_id
                                 and tr.exec_broker = :in_exec_broker
                               limit 1) tr on true
                 join genesis2.account ac on tr.account_id = ac.account_id and ac.is_deleted <> 'Y'
                 left join genesis2.user_identifier ui on ui.user_id = ai.deleted_by_user_id and ui.is_deleted <> 'Y'
...
select
    ...
    tr.trade_record_id,
    * from genesis2.trade_record tr
    join genesis2.alloc_instr2trade_record atr using (trade_record_id, date_id)
         join dash_reporting.bofa_allocation_report bar on bar.alloc_instr_id = atr.alloc_instr_id and bar.date_id = atr.date_id
where bar.date_id = 20250127
-- and bar.dataset = 13611072
and bar.alloc_instr_id = -56217;

select staging.all_orig_trade_record_id_today(2346679558, 20250127);
2346679813;


select is_billed, trade_record_id, exch_exec_id, exec_id, orig_trade_record_id, * from genesis2.trade_record tr
where trade_record_id = any('{2346679798,2346679806,2346679808,2346679810,2346679812}');

or
select is_billed, * from genesis2.trade_record tr
where trade_record_id = any('{2346679798,2346679807,2346679809,2346679811,2346679813}');
...
select aitr.trade_record_id, *  from dash_reporting.bofa_allocation_report bar
                 join genesis2.allocation_instruction ai
                      on ai.alloc_instr_id = bar.alloc_instr_id and ai.date_id = bar.date_id
                 join genesis2.alloc_instr2trade_record aitr
                      on (aitr.alloc_instr_id = bar.alloc_instr_id and aitr.date_id = bar.date_id)
...
$fx$;
comment on function staging.zabbix_monitor_ptm_missed_r is 'The script fix the issue when trade_records exist with missed status R';

select * from staging.zabbix_monitor_ptm_missed_r();
select * from staging.fix_ptm_missed_r();

select * from dash360.allocations_snapshot('{262707}', 20250204) WHERE trade_record_id IN ('2346694938');
...
select exec_id, exch_exec_id, * from genesis2.trade_record
where trade_record_id = 2346694938
